...
#ifdef OMX_CORE_TRACE
    #ifdef ANDROID
        #undef TRACE
        #define TRACE(argv) LOGD argv
    #else
        #undef TRACE
        #define TRACE(argv) printf argv
        #define LOGE printf
    #endif
#else
    #undef TRACE
...
char *tComponentName[MAXCOMP][2] = {
	...
	//{"OMX.hantro.G1.video.decoder", "video_decoder.wmv"},
	{NULL, NULL}
};
...
OMX_ERRORTYPE hantro_OMX_Init()
{
    ...
    TRACE(("OMX core: hantro_OMX_Init...\n"));
    ...
    TRACE(("OMX core: hantro_OMX_Init... done\n"));
    return eError;
}
/******************************Public*Routine******************************\
* OMX_GetHandle
    ...
    OMX_PTR pAppData, OMX_CALLBACKTYPE* pCallBacks)
{
    ...
    TRACE(("OMX core: hantro_OMX_GetHandle...\n"));
    TRACE(("OMX core: component name: %s\n", cComponentName));
    ...
    for (refIndex=0; refIndex < MAX_TABLE_SIZE; refIndex++) {
        ...
        if (strcmp(componentTable[refIndex].name, cComponentName) == 0) {
            TRACE(("Found component %s with refCount %d\n",
    ...
    LOGE("OMX core: OMX_ErrorComponentNotFound\n");
    err = OMX_ErrorComponentNotFound;
    goto UNLOCK_MUTEX;
CLEAN_UP:
    LOGD("CLEAN_UP\n");
...
UNLOCK_MUTEX:
    ...
    TRACE(("OMX core: hantro_OMX_GetHandle... done\n"));
    return (err);
}
...
OMX_ERRORTYPE hantro_OMX_FreeHandle (OMX_HANDLETYPE hComponent)
{
    ...
    TRACE(("OMX core: hantro_OMX_FreeHandle...\n"));
    ...
    for (refIndex=0; refIndex < MAX_TABLE_SIZE; refIndex++) {
        for (handleIndex=0; handleIndex < componentTable[refIndex].refCount; handleIndex++){
            ...
            if (componentTable[refIndex].pHandle[handleIndex] == hComponent){
                TRACE(("Found matching pHandle(%p) at index %d with refCount %d",
...
EXIT:
    ...
    TRACE(("OMX core: hantro_OMX_FreeHandle... done\n"));
    return retVal;
}
...
{
    TRACE(("OMX core: hantro_OMX_Deinit...\n"));
    ...
    TRACE(("OMX core: hantro_OMX_Deinit... done\n"));
    ...
    OMX_IN  OMX_U32 nPortInput)
{
    ...
    TRACE(("OMX core: hantro_OMX_SetupTunnel...\n"));
    ...
    TRACE(("OMX core: hantro_OMX_SetupTunnel... done\n"));
    return eError;
}
    ...
    else
    {
        ...
        else
        {
            ...
            LOGE("pNumRoles (%d) is less than actual number (%d) of roles \
                   for this component %s\n", (int)*pNumRoles, componentTable[i].nRoles, cComponentName);
        }
    }
    EXIT:
    return eError;
        ...
        if (*pNumComps < compOfRoleCount)
        {
            ...
            */
            ...
            LOGE("pNumComps (%d) is less than the actual number (%d) of components \
                  supporting role %s\n", (int)*pNumComps, (int)compOfRoleCount, role);
        }
        else
        {
            k = 0;
...
OMX_ERRORTYPE hantro_OMX_BuildComponentTable()
{
    ...
    TRACE(("OMX core: hantro_OMX_BuildComponentTable... done\n"));
    return eError;
}
...

{
    ...
    TRACE(("OMX core: hantro_OMXConfigParser status %d\n", Status));
    return Status;
}
#endif
