...
OMX_ERRORTYPE BOSA_ST_DeInitComponentLoader(BOSA_COMPONENTLOADER *loader) {
  ...
  for(i=0;i<numLib;i++) {
    ...
    if(err!=0) {
      LOGW("In %s Error %d in dlclose of lib %i\n", __func__,err,i);
    }
  }
...
OMX_ERRORTYPE BOSA_ST_CreateComponent(
  ...
  if (componentPosition == -1) {
    LOGE("Component not found with current ST static component loader.\n");
    return OMX_ErrorComponentNotFound;
  }

  //component name matches with general component name field
  LOGV("Found base requested template %s\n", cComponentName);
  ...
  if (eError != OMX_ErrorNone) {
    ...
    LOGE("Error during component construction :0x%x\n",eError);
  ...
  *pHandle = openmaxStandComp;
  ...
  LOGV("Template %s found returning from OMX_GetHandle\n", cComponentName);
  LOGV("Out of %s\n", __func__);
  return OMX_ErrorNone;
}
...
OMX_ERRORTYPE BOSA_ST_ComponentNameEnum(
  ...
  if (!found) {
    LOGE("Out of %s with OMX_ErrorNoMore\n", __func__);
  ...
  *pNumRoles = 0;
  ...
  while (templateList[i]) {
    if(!strcmp(templateList[i]->name, compName)) {
      LOGV("Found requested template %s IN GENERAL COMPONENT\n", compName);
      ...
      *pNumRoles = templateList[i]->name_specific_length;
      ...
      for(j=0;j<templateList[i]->name_specific_length;j++) {
        if(!strcmp(templateList[i]-> name_specific[j], compName)) {
          LOGV("Found requested component %s IN SPECIFIC COMPONENT \n", compName);
          *pNumRoles = 1;
          found = 1;
          if(roles == NULL) {
            return OMX_ErrorNone;
  ...
  if(!found) {
    LOGE("no component match in whole template list has been found\n");
    *pNumRoles = 0;
    return OMX_ErrorComponentNotFound;
  }
  LOGV("Out of %s\n", __func__);
