...
static int ReadVop(u8 * image, i32 nro, char *name, i32 width,
...
int main(int argc, char *argv[])
{
    ...
    nextPict = cmdl.firstVop;
    while(nextPict <= cmdl.lastVop)
    {
        VideoStbResult result;

        if(ReadVop((u8 *) pictureMem.virtualAddress, nextPict, cmdl.input,
                   cmdl.lumWidthSrc, cmdl.lumHeightSrc, cmdl.inputFormat) != 0)
            break;

        if(ReadVop((u8 *) nextPictureMem.virtualAddress, nextPict + 1, cmdl.input,
                   cmdl.lumWidthSrc, cmdl.lumHeightSrc, cmdl.inputFormat) != 0)
            break;
...
int AllocRes(commandLine_s * cmdl, VideoStbInst inst)
{
    ...
    printf("Input format: %d = %s\n", cmdl->inputFormat,
           ...
           10 ? strFormat[cmdl->inputFormat] : "Unknown");
...
/*------------------------------------------------------------------------------
    ...
    Returns:
        ...
        non-zero error code 
------------------------------------------------------------------------------*/
int ReadVop(u8 * image, i32 nro, char *name, i32 width, i32 height, i32 format)
{
    int ret = 0;
    u32 src_img_size;
    u32 luma_size;
