...
{
    if((data->appn.thumbMode && data->appn.thumbEnable) ||
       (!data->appn.thumbMode && !data->appn.thumbEnable))
    {
        ...
        /* APP0 extended header ==> thumbnail */
        if(data->appn.thumbMode && data->appn.thumbEnable)
        {
            EncJpegHeaderPutBits(stream, APP0, 16);
            COMMENT("APP0 Extended");

            /* Length of header will be updated 
             * after thumbnail is encoded */
            EncJpegHeaderPutBits(stream, 0x0008, 16);
            COMMENT("Lp");
            data->appn.targetStart = stream->byteCnt;
            ...
            EncJpegHeaderPutBits(stream, 0x00, 8);
            COMMENT("Ident3");
            EncJpegHeaderPutBits(stream, 0x10, 8);
            COMMENT("Extension code");

            EncJpegHeaderPutBits(stream, SOI, 16);
            COMMENT("SOI Thumbnail");
        }
    }
}
