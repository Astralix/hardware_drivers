...
/*------------------------------------------------------------------------------
    ...
    Write the end of current coding unit (RI / FRAME)
    into stream and write the actual thumbnail size 
	(APP0 header) into stream.
    
------------------------------------------------------------------------------*/
void EndRi(stream_s * stream, jpegData_s * jpeg)
{

#if 0 /* not needed anymore, ASIC generates EOI marker */

    /* Jpeg frame end marker */
    if(jpeg->mbNum == jpeg->mbPerFrame)
    {
        /* set start point before update */
        stream->stream = jpeg->streamStartAddress;

        /* check if slice mode or whole */
        if(jpeg->codingType == ENC_WHOLE_FRAME)
        {
            /* Update stream pointer */
            stream->stream = &(stream->stream[stream->byteCnt]);

            /* end stream (EOI) */
            EncJpegImageEnd(stream, jpeg);
        }
        else
        {
            /* Update stream pointer */
            stream->stream = &(stream->stream[stream->byteCnt - 2]);

            /* replace rst ==> end stream (EOI) */
            EncJpegImageEndReplaceRst(stream, jpeg);
        }
    }

#endif

    /* re-write APP0 header length */
    if(jpeg->appn.thumbMode && jpeg->appn.thumbEnable)
    {
        jpeg->appn.targetEnd = stream->byteCnt;
        jpeg->appn.appExtLp =
            (0x4 + ((jpeg->appn.targetEnd - jpeg->appn.targetStart)));

        /* write actual APP0 size to stream */
        jpeg->appn.pStartOfOutput[0x16] =
            ((jpeg->appn.appExtLp & 0x0000FF00) >> 8);
        jpeg->appn.pStartOfOutput[0x17] = ((jpeg->appn.appExtLp & 0x000000FF));
    }
}
...
void jpegSetNewFrame(jpegInstance_s * inst)
{
    ...
    if(regs->firstFreeBit != 0)
    {
        ...
        const u8 *pTmp = (u8 *) ((size_t) (inst->stream.stream) & (~0x07));
        u32 val;
...
#ifdef TRACE_ASIC
    EWLmemset(inst->asic.controlBase.virtualAddress, 0,
              inst->asic.controlBase.size);
#endif
