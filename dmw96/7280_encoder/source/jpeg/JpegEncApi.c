...
#define JPEGENC_MAJOR_VERSION 2
#define JPEGENC_MINOR_VERSION 2

#define JPEGENC_BUILD_MAJOR 2
#define JPEGENC_BUILD_MINOR 5
...
static bool_e CheckJpegCfg(JpegEncCfg * pEncCfg);
static i32 CheckThumbnailSize(const JpegEncCfgThumb * pCfgThumb);
...
#error Right bit-shifting (>>) does not preserve the sign
    ...
    pEncInst->asic.regs.inputImageFormat = pEncInst->preProcess.inputFormat;
    pEncInst->asic.regs.inputImageRotation = pEncInst->preProcess.rotation;
...
/*******************************************************************************
 Function name : JpegEncSetThumbnailMode
 Description   :
 Return type   : JpegEncRet
 Argument      : JpegEncInst inst
 Argument      : JpegEncCfg * pEncCfg
...
JpegEncRet JpegEncSetThumbnailMode(JpegEncInst inst, JpegEncCfg * pEncCfg)
{
    ...
    APITRACE("JpegEncSetThumbnailMode#");

    /* Check for illegal inputs */
    if((pEncInst == NULL) || (pEncCfg == NULL))
    {
        APITRACE("JpegEncSetThumbnailMode: ERROR null argument");
        return JPEGENC_NULL_ARGUMENT;
    }
    ...
    if(pEncInst->inst != pEncInst)
    {
        APITRACE("JpegEncSetThumbnailMode: ERROR Invalid instance");
        return JPEGENC_INSTANCE_ERROR;
    }

    /* Thumbnail dimensions must be valid */
    if(!pEncInst->jpeg.appn.thumbEnable)
    {
        APITRACE("JpegEncSetThumbnailMode: ERROR Thumbnail disabled");
        return JPEGENC_INVALID_ARGUMENT;
    }

    if(CheckThumbnailSize(&pEncCfg->cfgThumb) != (i32) JPEGENC_OK)
    {
        APITRACE
            ("JpegEncSetThumbnailMode: ERROR Out of range thumbnail dimension(s)");
        return JPEGENC_INVALID_ARGUMENT;
    }

    if(pEncCfg->codingMode == JPEGENC_420_MODE)
    {
        if (pEncInst->preProcess.inputFormat == JPEGENC_YUV422_PLANAR)
        {
            APITRACE("JpegEncSetThumbnailMode: ERROR 4:2:2 planar input in 4:2:0 mode");
            return JPEGENC_INVALID_ARGUMENT;
        }

        pEncInst->jpeg.MCUHeight = 16;
    ...
    else
    {
        ...
        if ((preProcess->inputFormat != JPEGENC_YUV422_INTERLEAVED_YUYV) &&
            (preProcess->inputFormat != JPEGENC_YUV422_INTERLEAVED_UYVY) &&
            (preProcess->inputFormat != JPEGENC_YUV422_PLANAR))
        {
            APITRACE("JpegEncSetThumbnailMode: ERROR 4:2:0 input in 4:2:2 mode");
            return JPEGENC_INVALID_ARGUMENT;
        }
        if (pEncInst->preProcess.rotation != JPEGENC_ROTATE_0)
        {
            APITRACE("JpegEncSetThumbnailMode: ERROR rotation in 4:2:2 mode");
            return JPEGENC_INVALID_ARGUMENT;
        }

        pEncInst->jpeg.MCUHeight = 8;
        pEncInst->jpeg.codingMode = ENC_422_MODE;
    }

    mbTotal = (u32) (((pEncCfg->cfgThumb.codingWidth + 15) / 16) *
                     ((pEncCfg->cfgThumb.codingHeight +
                       pEncInst->jpeg.MCUHeight-1) / pEncInst->jpeg.MCUHeight));

    pEncInst->jpeg.header = ENCHW_YES;
    pEncInst->jpeg.width = pEncCfg->cfgThumb.codingWidth;
    pEncInst->jpeg.height = pEncCfg->cfgThumb.codingHeight;
    pEncInst->jpeg.lastColumn = ((pEncInst->jpeg.width + 15) / 16);
    pEncInst->jpeg.mbPerFrame = mbTotal;

    /* Pre processing */
    pEncInst->preProcess.lumWidthSrc = pEncCfg->cfgThumb.inputWidth;
    pEncInst->preProcess.lumHeightSrc = pEncCfg->cfgThumb.inputHeight;
    pEncInst->preProcess.lumWidth = pEncCfg->cfgThumb.codingWidth;
    pEncInst->preProcess.lumHeight = pEncCfg->cfgThumb.codingHeight;
    pEncInst->preProcess.horOffsetSrc = pEncCfg->cfgThumb.xOffset;
    pEncInst->preProcess.verOffsetSrc = pEncCfg->cfgThumb.yOffset;
    pEncInst->preProcess.mbHeight = pEncInst->jpeg.MCUHeight;

    /* Restart interval (MCU rows converted to macroblocks) */
    pEncInst->jpeg.restart.Ri = 0;

    /* Coding type */
    pEncInst->jpeg.codingType = ENC_WHOLE_FRAME;

    /* Thumbnail mode */
    pEncInst->jpeg.appn.thumbMode = 1;

    /* Check that configuration is valid */
    if(EncPreProcessCheck(&pEncInst->preProcess) == ENCHW_NOK)
    {
        APITRACE
            ("JpegEncSetThumbnailMode: ERROR invalid pre-processing argument");
        return JPEGENC_INVALID_ARGUMENT;
    }

    /* Allocate internal SW/HW shared memories */
    if(EncAsicMemAlloc_V2(&pEncInst->asic,
                          (u32) pEncInst->jpeg.width,
                          (u32) pEncInst->jpeg.height, ASIC_JPEG) != ENCHW_OK)
    {
        APITRACE("JpegEncSetThumbnailMode: ERROR ewl memory allocation");
        return JPEGENC_EWL_MEMORY_ERROR;
    }

    APITRACE("JpegEncSetThumbnailMode: OK");

    return JPEGENC_OK;
}

/*******************************************************************************
 Function name : JpegEncSetFullResolutionMode
 Description   :
 Return type   : JpegEncRet
 Argument      : JpegEncInst inst
 Argument      : JpegEncCfg * pEncCfg
...
JpegEncRet JpegEncSetFullResolutionMode(JpegEncInst inst, JpegEncCfg * pEncCfg)
{
    ...
    APITRACE("JpegEncSetFullResolutionMode#");

    /* Check for illegal inputs */
    if((pEncInst == NULL) || (pEncCfg == NULL))
    {
        APITRACE("JpegEncSetFullResolutionMode: ERROR null argument");
        return JPEGENC_NULL_ARGUMENT;
    }
    ...
    if(pEncInst->inst != pEncInst)
    {
        APITRACE("JpegEncSetFullResolutionMode: ERROR Invalid instance");
        return JPEGENC_INSTANCE_ERROR;
    }
    ...
    {
        APITRACE
            ("JpegEncSetFullResolutionMode: ERROR Out of range image dimension(s)");
        return JPEGENC_INVALID_ARGUMENT;
    }
    ...
    if(pEncCfg->codingMode == JPEGENC_420_MODE)
    {
        if (pEncInst->preProcess.inputFormat == JPEGENC_YUV422_PLANAR)
        {
            APITRACE("JpegEncSetFullResolutionMode: ERROR 4:2:2 planar input in 4:2:0 mode");
            return JPEGENC_INVALID_ARGUMENT;
        }
    ...
    else
    {
        ...
        if ((preProcess->inputFormat != JPEGENC_YUV422_INTERLEAVED_YUYV) &&
            ...
            (preProcess->inputFormat != JPEGENC_YUV422_PLANAR))
        {
            APITRACE("JpegEncSetFullResolutionMode: ERROR 4:2:0 input in 4:2:2 mode");
            return JPEGENC_INVALID_ARGUMENT;
        }
        if (pEncInst->preProcess.rotation != JPEGENC_ROTATE_0)
        {
            APITRACE("JpegEncSetFullResolutionMode: ERROR rotation in 4:2:2 mode");
            return JPEGENC_INVALID_ARGUMENT;
        }
    ...
    if(((pEncCfg->restartInterval * pEncInst->jpeg.MCUHeight) >
                                                pEncCfg->codingHeight) ||
       ((pEncCfg->restartInterval * widthMbs) > 0xFFFF))
    {
        APITRACE("JpegEncSetFullResolutionMode: ERROR restart interval too big");
        return JPEGENC_INVALID_ARGUMENT;
    }
    ...
    if(pEncCfg->codingType == JPEGENC_SLICED_FRAME)
    {
        ...
        {
            APITRACE
                ("JpegEncSetFullResolutionMode: ERROR restart interval not set");
            return JPEGENC_INVALID_ARGUMENT;
        }
        ...
        if((pEncCfg->yOffset % 
                (pEncCfg->restartInterval * pEncInst->jpeg.MCUHeight)) != 0)
        {
            APITRACE("JpegEncSetFullResolutionMode: ERROR yOffset not valid");
            return JPEGENC_INVALID_ARGUMENT;
        }
    }

    mbTotal = (u32) (((pEncCfg->codingWidth + 15) / 16) *
                     ((pEncCfg->codingHeight + pEncInst->jpeg.MCUHeight-1) /
                      pEncInst->jpeg.MCUHeight));
    ...
    else
    {
        ...
        height = (pEncCfg->restartInterval * pEncInst->jpeg.MCUHeight);
    }

    /* Thumbnail mode */
    pEncInst->jpeg.appn.thumbMode = 0;

    /* Check that configuration is valid */
    if(EncPreProcessCheck(&pEncInst->preProcess) == ENCHW_NOK)
    {
        APITRACE
            ("JpegEncSetFullResolutionMode: ERROR invalid pre-processing argument");
        return JPEGENC_INVALID_ARGUMENT;
    }
    ...
    {
        APITRACE("JpegEncSetFullResolutionMode: ERROR ewl memory allocation");
        return JPEGENC_EWL_MEMORY_ERROR;
    }

    APITRACE("JpegEncSetFullResolutionMode: OK");
                         ...
                         JpegEncOut * pEncOut)
{
    ...
    if((pEncIn->pOutBuf == NULL) || (pEncIn->outBufSize < 8))
    {
        APITRACE("JpegEncEncode: ERROR invalid argument");
        return JPEGENC_INVALID_ARGUMENT;
    }
    ...
    if(pEncInst->jpeg.appn.thumbEnable && pEncInst->jpeg.appn.thumbMode)
    {
        pEncOut->jfifSize = 0;
        jpeg->appn.pStartOfOutput = pEncIn->pOutBuf;
    }
    ...
    {
        if(jpeg->appn.thumbEnable && jpeg->appn.thumbMode)
        {
            /* Set stream buffer, the size has been checked */
            if(EncJpegSetBuffer(&pEncInst->stream, (u8 *) pEncIn->pOutBuf,
                                (u32) pEncIn->outBufSize) == ENCHW_NOK)
        ...
        {
            /* Set stream buffer, thumbnail is already in the buffer */
            if(EncJpegSetBuffer(&pEncInst->stream,
                                pEncIn->pOutBuf + pEncOut->jfifSize,
                                (u32) pEncIn->outBufSize) == ENCHW_NOK)
            {
                APITRACE("JpegEncEncode: ERROR Invalid output buffer after TN");
                return JPEGENC_INVALID_ARGUMENT;
            }
            pEncInst->stream.byteCnt = pEncOut->jfifSize;
            pEncInst->stream.bitCnt = pEncOut->jfifSize * 8;
        }
    }
    ...
    if(jpeg->codingType == ENC_WHOLE_FRAME)
    {
        ...
        asic->regs.jpegRestartInterval =
            (jpeg->restart.Ri / (((u32) jpeg->width+15) / 16));
    ...
    if((jpeg->codingType == ENC_PARTIAL_FRAME) &&
       (preProcess->verOffsetSrc >
        (u32) (jpeg->sliceNum * jpeg->sliceRows * jpeg->MCUHeight)))
    {
        jpeg->sliceNum++;
    ...
    /* slice mode not supported for thumbnail */
    if(!jpeg->appn.thumbMode)
    {
    ...
    {
        i32 heightMCUs = (pEncInst->jpeg.height + pEncInst->jpeg.MCUHeight-1) /
                            pEncInst->jpeg.MCUHeight;

        /* Set frame dimensions in slice mode for pre-processing */
        if((jpeg->row + jpeg->sliceRows) <= heightMCUs)
        {
            preProcess->lumHeight = pEncInst->jpeg.MCUHeight *
                ((i32) pEncInst->jpeg.restart.Ri /
                 ((pEncInst->jpeg.width + 15) / 16));
        }
        else
        {
            /* Last slice is smaller than all others */
            preProcess->lumHeight = jpeg->height -
                (jpeg->sliceNum * jpeg->sliceRows * pEncInst->jpeg.MCUHeight);
        }
        ...
        asic->buffering.mbPerFrame =
                (asic->regs.mbsInRow * asic->regs.mbsInCol);
    }
...
i32 CheckFullSize(const JpegEncCfg * pCfgFull)
{
    ...
    if((pCfgFull->codingWidth < 64) || (pCfgFull->codingWidth > (511 * 16)))
    {
        return JPEGENC_ERROR;
    }

    if((pCfgFull->codingHeight < 16) || (pCfgFull->codingHeight > (511 * 16)))
    {
        return JPEGENC_ERROR;
    }
...
/*******************************************************************************
 Function name : CheckThumbnailSize
 Description   : Check that thumbnail size is valid
 Return type   : JPEGENC_OK for success
 Argument      : JpegEncCfg
*******************************************************************************/
i32 CheckThumbnailSize(const JpegEncCfgThumb * pCfgThumb)
{
    if((pCfgThumb->inputWidth > 256) || (pCfgThumb->inputHeight > 256))
    {
        return JPEGENC_ERROR;
    }

    if((pCfgThumb->codingWidth < 64) || (pCfgThumb->codingWidth > 240))
    {
        return JPEGENC_ERROR;
    }

    if((pCfgThumb->codingHeight < 16) || (pCfgThumb->codingHeight > 240))
    {
        return JPEGENC_ERROR;
    }

    if((pCfgThumb->codingWidth & (3)) != 0)
    {
        return JPEGENC_ERROR;
    }

    if((pCfgThumb->codingHeight & (1)) != 0)
    {
        return JPEGENC_ERROR;
    }

    if((pCfgThumb->inputWidth & (15)) != 0)
    {
        return JPEGENC_ERROR;
    }

    if(pCfgThumb->inputWidth < ((pCfgThumb->codingWidth + 15) & (~15)))
    {
        return JPEGENC_ERROR;
    }

    if((pCfgThumb->xOffset > pCfgThumb->inputWidth-pCfgThumb->codingWidth) ||
       (pCfgThumb->yOffset > pCfgThumb->inputHeight-pCfgThumb->codingHeight))
    {
        return JPEGENC_ERROR;
    }
...
/*******************************************************************************
 ...
 Argument      : JpegEncCfg
*******************************************************************************/
bool_e CheckJpegCfg(JpegEncCfg * pEncCfg)
{
    ...
    if(pEncCfg->frameType != JPEGENC_YUV420_PLANAR &&
       ...
       pEncCfg->frameType != JPEGENC_YUV422_PLANAR &&
    ...
    /* Extra limitation for partial encoding: Partial coding + 
     * rotation not supported */
    if(pEncCfg->codingType == JPEGENC_SLICED_FRAME &&
       (pEncCfg->rotation != JPEGENC_ROTATE_0))
        return ENCHW_NOK;
    ...
    if(((i32) pEncCfg->xDensity) < 0 || ((i32) pEncCfg->xDensity) > 0xFFFF ||
       ((i32) pEncCfg->yDensity) < 0 || ((i32) pEncCfg->yDensity) > 0xFFFF)
        return ENCHW_NOK;

    /* COM header length */
    if((pEncCfg->comLength > 0xFFFD) ||
       (((i32) pEncCfg->comLength) < 0) ||
       (pEncCfg->comLength && pEncCfg->pCom == NULL))
        return ENCHW_NOK;
