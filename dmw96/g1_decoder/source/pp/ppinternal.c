...
#include "decapicommon.h"
  ...
  #define LOG_ANDROID(args...)  LOGV(args)
#else
...
i32 PPCheckTiledOutput( PPContainer *ppC, PPConfig * ppCfg )
{
    ...
    if(ppOutImg->width & mask) {
        LOG_ANDROID("PPCheckTiledOutput: %d", __LINE__);
                  ...
                  u32 decLinked, u32 decType)
{
    ...
    {
        LOG_ANDROID("PPCheckConfig: %d", __LINE__);
                                 ...
                                 u32 ablendCrop )
{
    ...
    if(!pixAcc)
    {
        if(ppOutImg->width & (WIDTH_MULTIPLE - 1)) {
            LOG_ANDROID("PPCheckAllWidthParams: %d", __LINE__);
            ret = (i32) PP_SET_OUT_SIZE_INVALID;
        }
    }
    else
