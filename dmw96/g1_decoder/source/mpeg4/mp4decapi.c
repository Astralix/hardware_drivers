...
#ifdef MP4DEC_TRACE
#define MP4_API_TRC(str)   LOGV(str)
#else
...
#define DEC_VOPD ((DecContainer *)decInst)->VopDesc
    ...
    {
        LOGE("MP44DecDecode# PARAM_ERROR\n");
    ...
    {
        LOGE("MP4DecDecode: MP4DEC_FORMAT_NOT_SUPPORTED (line %d)", __LINE__);
    ...
    if(API_STOR.DecStat == UNINIT)
    {
        ...
        LOGE("MP4DecDecode: MP4DEC_NOT_INITIALIZED (line %d)", __LINE__);
    ...
    {
        LOGE("MP4DecDecode: MP4DEC_PARAM_ERROR (line %d)", __LINE__);
        return MP4DEC_PARAM_ERROR;
    }
       ...
       pDecCont->StrmStorage.customStrmHeaders )
    {
        ...
        {
            LOGE("MP4DecDecode: MEMFAIL (%d) (line %d)", internalRet, __LINE__);
            return (internalRet);
        }
    }
 ...
 */
    ...
    do
    {
        ...
        if(API_STOR.DecStat != HW_VOP_STARTED)
        {
            ...
            {
            case DEC_ERROR:
                LOGE("MP4DecDecode: DEC_ERROR (line %d)", __LINE__);
                /*
            ...
            case DEC_ERROR_BUF_NOT_EMPTY:
                LOGE("MP4DecDecode: DEC_ERROR_BUF_NOT_EMPTY (line %d)", __LINE__);
                /*
            ...
            default:
                ...
                pDecCont->StrmStorage.status = STATE_SYNC_LOST;
                API_STOR.DecStat = INITIALIZED;
                LOGE("MP4DecDecode: reached default, return MP4DEC_STRM_ERROR (line %d)", __LINE__);
                ret = MP4DEC_STRM_ERROR;
                break;
            }
        }
        ...
        if(API_STOR.DecStat == HW_VOP_STARTED)
        {
            if(DEC_VOPD.vopCoded)
            {
                ...
                if(asicStatus == X170_DEC_TIMEOUT)
                {
                    LOGE("MP4DecDecode: MP4DEC_HW_TIMEOUT (line %d)", __LINE__);
                    ret = MP4DEC_HW_TIMEOUT;
                }
                else if(asicStatus == X170_DEC_SYSTEM_ERROR)
                {
                    LOGE("MP4DecDecode: MP4DEC_SYSTEM_ERROR (line %d)", __LINE__);
                    ret = MP4DEC_SYSTEM_ERROR;
                }
                else if(asicStatus == X170_DEC_HW_RESERVED)
                {
                    LOGE("MP4DecDecode: MP4DEC_HW_RESERVED (line %d)", __LINE__);
                    ret = MP4DEC_HW_RESERVED;
                }
                else if(asicStatus & MP4_DEC_X170_IRQ_BUS_ERROR)
                {
                    LOGE("MP4DecDecode: MP4DEC_HW_BUS_ERROR (line %d)", __LINE__);
                    ret = MP4DEC_HW_BUS_ERROR;
                }
                        ...
                        asicStatus & MP4_DEC_X170_IRQ_TIMEOUT)
                {
                    ...
                    {
                        LOGE("MP4DecDecode: MP4_DEC_X170_IRQ_STREAM_ERROR (line %d)", __LINE__);
