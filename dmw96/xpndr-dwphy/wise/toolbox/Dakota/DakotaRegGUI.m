function varargout = DakotaRegGUI(varargin)
% DAKOTAREGGUI M-file for DakotaRegGUI.fig
%      DAKOTAREGGUI, by itself, creates aC new DAKOTAREGGUI or raises the existing
%      singleton*.
%
%      H = DAKOTAREGGUI returns the handle to aC new DAKOTAREGGUI or the handle to
%      the existing singleton*.
%
%      DAKOTAREGGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DAKOTAREGGUI.M with the given input arguments.
%
%      DAKOTAREGGUI('Property','Value',...) creates aC new DAKOTAREGGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before DakotaRegGUI_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to DakotaRegGUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help DakotaRegGUI

% Last Modified by GUIDE v2.5 14-Jun-2004 14:43:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @DakotaRegGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @DakotaRegGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%------------------------------------------------------
% --- Executes just before DakotaRegGUI is made visible.
%------------------------------------------------------
function DakotaRegGUI_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for DakotaRegGUI
handles.output = hObject;
handles.ReadRegistersFn = @Dakota_GetRegisters;
handles.WriteRegistersFn = @Dakota_SetRegisters;
handles.ChangesPending = 0;
set(handles.WriteOnChange,'Value',1);

% Setup callbacks
if nargin < 5 
    disp('DakotaRegGUI Running in Test Mode (wiseMex I/O)');
elseif (length(varargin) == 3 & strcmpi(varargin{1},'RegisterReadWriteFns') & length(varargin{2})==2)
    ...
    uiwait(errordlg('Invalid input parameter specification','DakotaRegGUI Error','modal'));
end	  
guidata(hObject, handles);
[handles.RegByField, handles.ValidAddr] = Dakota_RegisterMap;
...
C = get(handles.DakotaRegGUI,'Children');
for i=1:length(C),
    h = C(i);
    style = get(h,'Style');
    tag   = get(h,'Tag');
    if(isfield(handles.RegByField,tag))
        ...
        if(strcmp(style,'edit'))
            P(1) = P(1) + P(3) + 0.5; P(2)=P(2)-0.3; P(3)=1+1.5*length(tag);
            h = uicontrol(handles.DakotaRegGUI,'Style','text','String',tag,'HorizontalAlignment','Left','Units','Characters','Position',P);
...
if(~isfield(handles,'MenuBar'))
    handles.MenuBar = RegGUI_MENU(handles.DakotaRegGUI);
end
guidata(hObject, handles)


% --- Outputs from this function are returned to the command line.
function varargout = DakotaRegGUI_OutputFcn(hObject, eventdata, handles)
...
function DakotaRegGUI_WindowButtonDownFcn(hObject, eventdata, handles)
...
guidata(handles.DakotaRegGUI, handles);
UpdateRegisterList(handles);
switch(style)
...
% --- Executes during object creation, after setting all properties.
function Diversity_CreateFcn(hObject, eventdata, handles)
if ispc
    set(hObject,'BackgroundColor','white');
else
    set(hObject,'BackgroundColor',get(0,'defaultUicontrolBackgroundColor'));
...
for i=1:length(name),
    if(isfield(handles.RegByField,name{i}))
        h = handles.(name{i});
        d = Dakota_GetValueByField(handles.RegList, handles.RegByField, name{i});        
        style = get(h,'Style');
        switch(style)
...
if(~isempty(k0))
    RSSI0 = handles.RegList(k0,2);
    RSSI1 = handles.RegList(k0+1,2);
