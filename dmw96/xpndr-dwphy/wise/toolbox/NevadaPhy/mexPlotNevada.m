function [outS,outY] = mexPlotNevada(t1,t2,tt1,tt2,dt)
...
if nargin<5,
    if isfield(S,'TX') && isfield(S.TX,'TimingShift'),
        dt = S.TX.TimingShift*1e6;
    else
        dt = 50.3165;
    end
end

t20=(k20-1)/20-dt; Y.t20 = t20;
t40=(k40-1)/40-dt; Y.t40 = t40;
t80=(k80-1)/80-dt; Y.t80 = t80;
...
switch InputPlot,
    case 1, % TX Output
        subplot(nPlot,1,1); plot(t80, real(S.RX.r)/S.RX.Ar); grid on;
        A = axis; 
        if(~SetLimit), t1=A(1); end; A(1) = t1;
        if(~SetLimit), t2=A(2); end; A(2) = t2; 
        A(3) = -2.1; A(4) = 2.1; axis(A);
...
switch(SignalSet)
    case 0, % TX
        subplot(nPlot,1,3); plot(t80,Y.MAC_TX_EN+0,t80,Y.TxEnB+2,t80,Y.TxDone+4,t80,Y.SW1+6,t80,Y.PA50+8); grid on;
        set(gca,'YTick',0:2:8); set(gca,'YTickLabel',{'MAC_TX_EN','TxEnB','TxDone','SW1','PA50'});
        axis([t1 t2 0 10]);
    case 1,
    ...
    case 7, % RX
        subplot(nPlot,1,3); 
        plot(t80,Y.PHY_CCA+0, t80,Y.PHY_RX_EN+2, t20,Y.aSigOut.ValidRSSI(k)+4, t80,Y.ClearDCX+6, t20,Y.aSigOut.AGCdone(k)+8, t20,Y.aSigOut.HeaderValid(k)+10,t20,Y.aSigOut.RxDone(k)+12,t80,Y.ADCFCAL+14);
        grid on;
        set(gca,'YTick',0:2:14); set(gca,'YTickLabel',{'PHY_CCA','PHY_RX_EN','ValidRSSI','ClearDCX','AGCdone','HeaderValid','RxDone','ADCFCAL'});
        axis([t1 t2 0 16]);
end
