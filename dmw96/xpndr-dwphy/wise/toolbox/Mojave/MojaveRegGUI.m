function varargout = MojaveRegGUI(varargin)
% MOJAVEREGGUI M-file for MojaveRegGUI.fig
%      MOJAVEREGGUI, by itself, creates aC new MOJAVEREGGUI or raises the existing
%      singleton*.
%
%      H = MOJAVEREGGUI returns the handle to aC new MOJAVEREGGUI or the handle to
%      the existing singleton*.
%
%      MOJAVEREGGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MOJAVEREGGUI.M with the given input arguments.
%
%      MOJAVEREGGUI('Property','Value',...) creates aC new MOJAVEREGGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before MojaveRegGUI_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to MojaveRegGUI_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help MojaveRegGUI

% Last Modified by GUIDE v2.5 01-Nov-2007 20:06:56

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @MojaveRegGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @MojaveRegGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%------------------------------------------------------
% --- Executes just before MojaveRegGUI is made visible.
%------------------------------------------------------
function MojaveRegGUI_OpeningFcn(hObject, eventdata, handles, varargin)

% Choose default command line output for MojaveRegGUI
handles.output = hObject;
handles.ReadRegistersFn = @Mojave_GetRegisters;
handles.WriteRegistersFn = @Mojave_SetRegisters;
...
if nargin < 5,
    ...
    if UseDwPhyLab,
        ...
        disp('MojaveRegGUI Running in Test Mode (wiseMex I/O)');
    end
...
elseif (length(varargin) == 3 && strcmpi(varargin{1},'RegisterReadWriteFns') && length(varargin{2})==2)
    ...
    uiwait(errordlg('Invalid input parameter specification','MojaveRegGUI Error','modal'));
end	  
guidata(hObject, handles);
setptr(handles.MojaveRegGUI,'watch');
[handles.RegByField, handles.ValidAddr] = Mojave_RegisterMap;
handles.RegList = feval(handles.ReadRegistersFn);
guidata(hObject, handles)
UpdateRegisterList(handles);
SetParameterValues(handles);
setptr(handles.MojaveRegGUI,'arrow');

% Add text to edit boxes
AddTextToRegisterFields(handles, handles.MojaveRegGUI);
UpdateInfoTextBoxes(handles);
if(~isfield(handles,'MenuBar'))
    handles.MenuBar = RegGUI_MENU(handles.MojaveRegGUI);
end
guidata(hObject, handles)

% --- Outputs from this function are returned to the command line.
function varargout = MojaveRegGUI_OutputFcn(hObject, eventdata, handles)
...
setptr(handles.MojaveRegGUI,'watch');
h = waitbar(0,'Reading Registers...','Name','Mojave','WindowStyle','modal');
handles.RegList = feval(handles.ReadRegistersFn);
...
function MojaveRegGUI_WindowButtonDownFcn(hObject, eventdata, handles)
...
guidata(handles.MojaveRegGUI, handles);
UpdateRegisterList(handles);
switch(style)
...
for i=1:length(name),
    if(isfield(handles.RegByField,name{i}))
        h = handles.(name{i});
        d = Mojave_GetValueByField(handles.RegList, handles.RegByField, name{i});        
        style = get(h,'Style');
        switch(style)
...
if(~isempty(n))
    switch(n)
        ...
        case  4, msg = 'Dakota 4';
        case  5, msg = 'Mojave 1a';
        case  6, msg = 'Mojave 1b';
