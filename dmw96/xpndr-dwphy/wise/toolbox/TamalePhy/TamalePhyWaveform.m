function varargout = TamalePhyWaveform(varargin)
% TamalePhyWaveform -- Waveform display tool for the WiSE TamalePHY model
%
%      To use the waveform display, process a transmit/receive operation with the
%      TamalePHY configured with Tamale.EnableTrace = 1 and Tamale.EnableModelIO = 1.
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @TamalePhyWaveform_OpeningFcn, ...
                   'gui_OutputFcn',  @TamalePhyWaveform_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
%% --- Executes just before TamalePhyWaveform is made visible.
function TamalePhyWaveform_OpeningFcn(hObject, eventdata, handles, varargin) %#ok<INUSL>
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to TamalePhyWaveform (see VARARGIN)

% Choose default command line output for TamalePhyWaveform
handles.output = hObject;

if(~isfield(handles,'MenuBar')) 
    handles.MenuBar = TamalePhy_Menu(handles.TamalePhyWaveform);
end
...
Z = get(0,'ScreenSize');
P = get(handles.TamalePhyWaveform,'Position');
P(2) = Z(4) - P(4);
set(handles.TamalePhyWaveform,'Position',P);

% UIWAIT makes TamalePhyWaveform wait for user response (see UIRESUME)
% uiwait(handles.TamalePhyWaveform);


%% --- Outputs from this function are returned to the command line.
function varargout = TamalePhyWaveform_OutputFcn(hObject, eventdata, handles)  %#ok<INUSL,INUSL>
...
if ~any(Y.traceValid)
    error('Unable to update: trace data is invalid; Tamale TX/RX may not have run');
end
...
Key = get(handles.TamalePhyWaveform,'CurrentCharacter');
