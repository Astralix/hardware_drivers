//
//  Nevada.h -- 802.11n Single Spatial Stream Handheld Baseband
...
//
//     NEVADA_VERSION_1  -- Original DMW96 design
//     NEVADA_VERSION_1B -- Metal fix for DMW96B
//
#if !defined(NEVADA_VERSION_1) && !defined(NEVADA_VERSION_1B)
    #define  NEVADA_VERSION_1B
#endif
//////////////////////////////////////////////////////////////////////////////////////////////
...
//
#ifdef NEVADA_VERSION_1B
    #define NEVADA_PART_ID 9
#else
    #define NEVADA_PART_ID 8
#endif
...
//  ------------------
//  NEVADA_FFT_OFFSET     - delay from peak found to the start of an FFT (RX control)
//  NEVADA_FFT_DELAY      - processing/pipeline delay for the (I)FFT
//  NEVADA_PLL_DELAY      - delay due to the PLL phase corrector
//  NEVADA_SIG_LATENCY    - latency from L-SIG FFT start to the decoded output
//  NEVADA_GF_SIG_LATENCY - latency from the HT-SIG FFT start to decoded output (Greenfield)
//  NEVADA_DPD_DELAY      - signal delay for DPD vs bypass
//
#define NEVADA_FFT_OFFSET      2
#define NEVADA_FFT_DELAY      82
#define NEVADA_PLL_DELAY       0
#define NEVADA_SIG_LATENCY    70
#define NEVADA_HTSIG_LATENCY  76 
#define NEVADA_DPD_DELAY       4

//////////////////////////////////////////////////////////////////////////////////////////////
//
//  ARRAY LENGTHS
//  -------------
//  NEVADA_MAX_N_SYM      - maximum number of OFDM symbols. 2560 --> 10.2 ms which is greater
//                          than any 802.11a packet or 802.11n aPPDUMaxTime (10.0 ms)
//  NEVADA_MAC_DV_TX      - maximum number of MAC_TX_D arrays stored in DvState
//
#define NEVADA_MAX_N_SYM 2560
#define NEVADA_MAX_DV_TX    4 

//////////////////////////////////////////////////////////////////////////////////////////////
//
//  Enumerate Subcarrier Type Labels
//
enum NevadaSubcarrierTypeE
{
    NEVADA_NULL_SUBCARRIER  = 0,
    NEVADA_DATA_SUBCARRIER  = 1,
    NEVADA_PILOT_SUBCARRIER = 2
};
...
typedef struct
{
    ...
    wiWORD CC110Real[64][8], CC110Imag[64][8]; // ...defined in Nevada_b_Init()

} Nevada_LookupTable_t;
...
typedef struct
{
    ...
    int      bResamplePhase;         // phase between 44/40 MHz for Nevada_b_TX_Resample
    ...
    Nevada_traceTxType *traceTx;     // trace for general purpose signals/markers
...
typedef union
{
    ...
    struct
    {
        ...
        unsigned int RxIsCCK     :1; // <20> Indicates DSSS/CCK reception in DFE (added for version 1b)
    };
} Nevada_aSigState_t;
...
typedef union // ----------------- Registers in Top-Level Control -----------         
{
    ...
    struct
    {
        ...
        unsigned int bPathMux      : 1; // <19> mux control for bNevada input
        unsigned int bRXEnB        : 1; // <20> bNevada TopCtrl enable
...
typedef union // ----------------- RX Exception Event Flags ----------------------         
{
    ...
    struct
    {
        ...
        unsigned int rLimit_SIGNAL   : 1; // <  4> Overrun of array "r" detected in Nevada_RX2_Signal()
        unsigned int xLimit_SIGNAL   : 1; // <  5> Overrun of array "x" detected in Nevada_RX2_Signal()
        unsigned int xLimit_DataDemod: 1; // <  6> Overrun of array "x" detected in Nevada_RX2_Signal()
        unsigned int kLimit_FrontCtrl: 1; // <  7> Overrun of "k44" counter in Nevada_b_RX_Top()
        unsigned int Truncated_N_SYM : 1; // <  8> OFDM "N_SYM" forced to 1 to avoid array overrun in Nevada_RX2_Signal()
    };
} Nevada_RTL_ExceptionFlags_t;
...
typedef struct
{
    ...
    wiBoolean               BreakCheck;       // indicates NevadaDV_CheckScript should not continue to later waveforms
    ...
    wiUWORD   *MAC_TX_D[NEVADA_MAX_DV_TX];     // Pointer to data bytes to be transmitted
    wiUInt     N_PHY_TX_RD[NEVADA_MAX_DV_TX];  // Length of array in pMAC_TX_D (includes PHY header)
...
typedef struct  // ----------------- Nevada_Downmix State --------------------------
{
...
typedef struct  // ----------------- Nevada_DownsamFIR State -----------------------
{
...
typedef struct  // ----------------- Nevada_WeightedPilotPhaseTrack State ----------
{
...
} Nevada_WeightedPilotPhaseTrack_t;

typedef struct  // ----------------- Nevada_PLL State ------------------------------
{
...
typedef struct  // ----------------- Nevada_EqD State ------------------------------
{
...
typedef struct  // ----------------- Nevada_RX2 State ------------------------------
{
...
typedef struct  // ----------------- Nevada_RX_CCA State ---------------------------
{
...
typedef struct  // ----------------- Nevada_RX (Top) State -------------------------
{
    ...
    Nevada_aSigState_t aSigIn, aSigOut;  // signals from OFDM DFE to top level control
    Nevada_bSigState_t bSigIn, bSigOut;  // signals from CCK FrontCtrl to top level control
...
typedef struct
{
    ...
    struct
    {
        ...
        enum
        {
            NEVADA_GAIN_UPDATE_LINEAR      = 1,
            NEVADA_GAIN_UPDATE_THSIGSMALL  = 2,
            NEVADA_GAIN_UPDATE_THSMALLPEAK = 3,
            NEVADA_GAIN_UPDATE_THSIGLARGE  = 4,
            NEVADA_GAIN_UPDATE_LGSIGAFE    = 5
        } GainUpdateType[8];
    ...
    Nevada_RxClockState_t clock;     // clock generator state
    ...
    Nevada_Downmix_t                 Downmix;
    Nevada_DownsamFIR_t              DownsamFIR;
    Nevada_ImageDetector_t           ImageDetector;
    Nevada_IQCalibration_t           IQCalibration;
    Nevada_CFO_t                     CFOState;
    Nevada_FrameSync_t               FrameSync;
    wiReg                            DigitalAmpOut[4];
    Nevada_AGC_t                     AGC;
    Nevada_SignalDetect_t            SignalDetect;
    Nevada_DigitalFrontEnd_t         DigitalFrontEnd;
    Nevada_WeightedPilotPhaseTrack_t WeightedPilotPhaseTrack;
    Nevada_PLL_t                     PLL;
    Nevada_EqD_t                     EqD;
    Nevada_RX2_t                     RX2;
    Nevada_RX_CCA_t                  CCA;
    Nevada_RxTop_t                   RxTop;
    ...
    Nevada_RTL_ExceptionFlags_t RTL_Exception;  // exception event flags used for RTL verification  

    Nevada_aSigState_t         *aSigOut;        // trace "aSigState" register output
    Nevada_bSigState_t         *bSigOut;        // trace "bSigState" register output
    Nevada_traceRxClockType    *traceRxClock;   // trace RX clocks in top-level controller
    Nevada_traceRxStateType    *traceRxState;   // trace RX top state
    Nevada_traceRxControlType  *traceRxControl; // trace top-level RX control
    Nevada_traceDFEType        *traceDFE;       // trace OFDM Digital Front End
    Nevada_traceAGCType        *traceAGC;       // trace signals from OFDM AGC
    Nevada_traceRSSIType       *traceRSSI;      // trace power measurements from OFDM AGC/RSSI
    Nevada_traceDFSType        *traceDFS;       // trace DFS
    Nevada_traceSigDetType     *traceSigDet;    // trace OFDM signal detect
    Nevada_traceFrameSyncType  *traceFrameSync; // trace OFDM frame synchronization
    Nevada_RadioDigitalIO_t    *traceRadioIO;   // trace radio digital interface   
    Nevada_DataConvCtrlState_t *traceDataConv;  // trace data converter control interface
    Nevada_traceRX2Type        *traceRX2;       // trace for OFDM RX2 demodulation/decoding
    Nevada_traceRxType         *traceRx;        // trace for general purpose signals/markers
    Nevada_traceImgDetType     *traceImgDet;    // trace for image detector
    Nevada_traceIQCalType      *traceIQCal;     // trace for IQ calibration engine
...
typedef union // ----------------- Trace CCK Baseband Interface -------------
{
    ...
    struct
    {
        unsigned int bRXEnB     :1; //< 0> input to Nevada_b_RX_top
        unsigned int Run11b     :1; //< 1> input to Nevada_b_RX_top
        unsigned int LgSigAFE   :1; //< 2> input to Nevada_b_RX_top
        unsigned int bRXDone    :1; //< 3> output from Nevada_b_RX_top
        unsigned int AntSel     :1; //< 4> output from Nevada_b_RX_top
        unsigned int __unused5 : 1; //< 5>
        unsigned int CSCCK      :1; //< 6> output from Nevada_b_RX_top
        unsigned int Restart    :1; //< 7> input to Nevada_b_RX_top
        unsigned int RSSICCK    :8; //<8:15> output from Nevada_b_RX_top
        unsigned int LNAGain    :1; //<16> output from Nevada_b_RX_top
        unsigned int AGain      :6; //<17:22> output from Nevada_b_RX_top
        signed   int DGain      :5; //<23:27> output from AGC
        unsigned int LNAGain2   :1; //<28> output from Nevada_b_RX_top
    };
} Nevada_b_traceCCKType;
...
} Nevada_b_traceControlType;

typedef union // ----------------- Trace bNevada RX Interpolator Signals ----
...
typedef struct
{
    ...
    struct NEVADA_RAM_128x32_S { // 128x32, 2-port RAM
...
typedef struct
{
    ...
    Nevada_b_FrontCtrlRegType inReg, outReg; // packed register I/O
...
typedef struct
{
    ...
    wiWORD    dRxFilter[2][16];    // delay line for Nevada_bRX_Filter
    ...
    Nevada_bRX_Interpolate_t     Interpolate; // state for RX interpolator
    Nevada_bBarkerCorrelator_t   BarkerCorrelator;
    Nevada_bEnergyDetect_t       EnergyDetect;
    Nevada_bCorrelationQuality_t CorrelationQuality;
    Nevada_bAGC_t                AGC;
    Nevada_bEstimateCFO_t        EstimateCFO;
    Nevada_bCorrectCFO_t         CorrectCFO;
    Nevada_bChanEst_t            ChanEst;
    Nevada_bRX_SP_t              RX_SP;
    Nevada_bRX_CMF_t             CMF;
    Nevada_bRX_DSSS_t            RX_DSSS;
    Nevada_bRX_CCK_t             RX_CCK;
    Nevada_bRX_SFD_t             RX_SFD;
    Nevada_bRX_Header_t          RX_Header;
    Nevada_bRX_PSDU_t            RX_PSDU;
    Nevada_bRX_DPLL_t            RX_DPLL;
    Nevada_bRX_Descrambler_t     Descrambler;
    Nevada_b_Downsample_t        Downsample;
    Nevada_bRxTop_t              RxTop;

    Nevada_b_traceCCKType      *traceCCK;     // trace signals from CCK interface
    Nevada_b_traceStateType    *traceState;   // trace state from CCK controller
    Nevada_b_traceControlType  *traceControl; // trace signals from CCK controller
    Nevada_b_FrontCtrlRegType  *traceFrontReg;// trace "outReg" from Nevada_b_RX_top
    Nevada_b_traceDPLLType     *traceDPLL;    // trace signals from DPLL
    Nevada_b_traceBarkerType   *traceBarker;  // trace signals from Barker correlator
    Nevada_b_traceDSSSType     *traceDSSS;    // trace signals from DSSS demodulator
    Nevada_b_traceCCKFBFType   *traceCCKInput;// trace CCK input/DFE output
    Nevada_b_traceCCKTVMFType  *traceCCKTVMF; // trace CCK TVMF output
    Nevada_b_traceCCKFWTType   *traceCCKFWT;  // trace CCK FWT/metric output
    Nevada_b_traceResampleType *traceResample;// trace bRX Resample NCOs
...
wiStatus Nevada_SetRegisterMap(wiUInt pRegData[], unsigned int NumRegs);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Transmit functions in Nevada_TX.c
//
wiStatus Nevada_TX_Init(void);
wiStatus Nevada_TX_Restart(void);
wiStatus Nevada_TX(int Length, wiUWORD *MAC_TX_D, int *Nx, wiComplex **x, int M);
wiReal   Nevada_Prms(void);

void     Nevada_DAC       (int N, wiUWORD uR[], wiUWORD uI[], wiComplex s[], int M);
void     Nevada_Upmix     (int N, wiWORD UpmixInR[], wiWORD UpmixInI[], wiWORD UpmixOutR[], wiWORD UpmixOutI[]);
void     Nevada_TxIQSource(int N, wiWORD xR[], wiWORD xI[], wiWORD *yR, wiWORD *yI);
void     Nevada_TxIQ      (int N, wiWORD xR[], wiWORD xI[], wiWORD *yR, wiWORD *yI);
void     Nevada_Prd       (int N, wiWORD PrdInR[], wiWORD PrdInI[], wiUWORD PrdOutR[], wiUWORD PrdOutI[]);

wiStatus Nevada_TX_FreeMemory(Nevada_TxState_t *pTX, wiBoolean ClearPointerOnly);
wiStatus Nevada_TX_AllocateMemory(int N_SYM, int N_BPSC, int N_DBPS, int M, wiBoolean nMode, wiBoolean Greenfield);

void Nevada_Interleaver( wiUWORD InRATE, wiUWORD IntlvIn[], wiUWORD IntlvOut[], int IntlvTxRx, 
                         wiBoolean nMode, wiBoolean Rotated6Mbps );

///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Nevada_TX_DPD.c
//
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Nevada_RX.c
//
wiStatus Nevada_RX( wiBoolean DW_PHYEnB, Nevada_RadioDigitalIO_t *Radio, 
                    Nevada_DataConvCtrlState_t *DataConvIn, Nevada_DataConvCtrlState_t DataConvOut, 
                    Nevada_CtrlState_t *inReg, Nevada_CtrlState_t outReg );
                   
wiStatus Nevada_RX_Init(void);
wiStatus Nevada_RX_Restart(void);
wiStatus Nevada_RX_FreeMemory(Nevada_RxState_t *pRX, wiBoolean ClearPointerOnly);
wiStatus Nevada_RX_AllocateMemory(void);
wiStatus Nevada_RX_EndOfWaveform(void);

void Nevada_RX_ClockGenerator(Nevada_RxState_t *pRX, wiBoolean Reset);

///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Nevada_RX1.c
//
wiStatus Nevada_RX1( wiBoolean DFEEnB, wiBoolean LgSigAFE, wiUWORD *LNAGainOFDM, wiUWORD *AGainOFDM, 
                     ...
                     Nevada_aSigState_t *aSigIn, Nevada_aSigState_t aSigOut, 
                     Nevada_RxState_t *pRX, NevadaRegisters_t *pReg );

wiStatus Nevada_RX1_Restart(void);

void Nevada_ADC( Nevada_DataConvCtrlState_t SigIn, Nevada_DataConvCtrlState_t *SigOut, 
                 Nevada_RxState_t *pRX, wiComplex ADCAIQ, wiComplex ADCBIQ, 
                 wiUWORD *ADCOut1I, wiUWORD *ADCOut1Q, wiUWORD *ADCOut2I, wiUWORD *ADCOut2Q );

void Nevada_DCX( int n, wiWORD DCXIn, wiWORD *DCXOut, wiBoolean DCXClear, Nevada_RxState_t *pRX, 
                 NevadaRegisters_t *pReg );

///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Nevada_RX2.c
//
...
void Nevada_PLL( wiWORD I1[], wiWORD Q1[], wiWORD H2[], wiWORD DI[], wiWORD DQ[], int iFlag, int HoldVCO, int UpdateH,
                 wiWORD *AdvDly, wiWORD *_cSP, wiWORD *_cCP, Nevada_RxState_t *pRX, NevadaRegisters_t *pReg );

void Nevada_WeightedPilotPhaseTrack( int FInit, int HoldVCO, wiUWORD PathSel, wiWORD v0R[], wiWORD v0I[],
                                     wiWORD v1R[], wiWORD v1I[], wiWORD h0R[], wiWORD h0I[],
                                     wiWORD h1R[], wiWORD h1I[], wiWORD cSP, wiWORD AdvDly, wiWORD *cCPP, 
                                     Nevada_RxState_t *pRX, NevadaRegisters_t *pReg, wiBoolean nMode, int i );
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Nevada_DigitalFrontEnd.c
//
void Nevada_DigitalFrontEnd( wiBoolean DFEEnB, wiBoolean DFERestart, wiBoolean CheckRIFS, wiBoolean HoldGain,
                             ...
                             Nevada_RxState_t *pRX, NevadaRegisters_t *pReg, wiBoolean CSCCK, 
                             wiBoolean RestartRX, Nevada_aSigState_t *aSigOut, Nevada_aSigState_t aSigIn );

/////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Nevada_FFT.c
//
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Nevada_b.c
//
wiStatus Nevada_bRX_AllocateMemory(void);
wiStatus Nevada_bRX_FreeMemory(Nevada_bRxState_t *pRX, wiBoolean ClearPointerOnly);
...
void Nevada_bRX_top( Nevada_RxState_t *pRX, Nevada_bRxState_t *RX, NevadaRegisters_t *pReg, wiBoolean bRXEnB, 
