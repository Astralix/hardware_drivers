...
// ================================================================================================
// FUNCTION : Nevada_b_Init()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_b_Init()
{
	Nevada_LookupTable_t *pLut = Nevada_LookupTable();

    Nevada_bRxState()->N44  = 0;
...
wiStatus Nevada_bTX_AllocateMemory(int Na, int Nc, int M)
{
    ...
    {
        pTX->LastAllocation.Fn      = (wiFunction)Nevada_bTX_AllocateMemory;
        pTX->LastAllocation.Size[0] = Na;
        pTX->LastAllocation.Size[1] = Nc;
        pTX->LastAllocation.Size[2] = M;
        pTX->LastAllocation.Size[3] = 0;
    ...
    pTX->traceTx = (Nevada_traceTxType  *)wiCalloc(pTX->Nz, sizeof(Nevada_traceTxType));
...
// end of bNevada_bTX_AllocateMemory()

// ================================================================================================
// FUNCTION : Nevada_bRX_AllocateMemory()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_bRX_AllocateMemory(void)
{
    Nevada_bRxState_t *pbRX = Nevada_bRxState();
    int N80 = (pbRX->N44 * 80/44) + 2; // +2 to cover rounding errors

    Nevada_bRX_FreeMemory(pbRX, 0);
    ...
    {
        pbRX->traceCCK     = (Nevada_b_traceCCKType     *)wiCalloc(pbRX->N22, sizeof(Nevada_b_traceCCKType)     );
        pbRX->traceState   = (Nevada_b_traceStateType   *)wiCalloc(pbRX->N22, sizeof(Nevada_b_traceStateType)   );
        pbRX->traceControl = (Nevada_b_traceControlType *)wiCalloc(pbRX->N22, sizeof(Nevada_b_traceControlType) );
        pbRX->traceFrontReg= (Nevada_b_FrontCtrlRegType *)wiCalloc(pbRX->N22, sizeof(Nevada_b_FrontCtrlRegType) );
        pbRX->traceDPLL    = (Nevada_b_traceDPLLType    *)wiCalloc(pbRX->N22, sizeof(Nevada_b_traceDPLLType)    );
        pbRX->traceBarker  = (Nevada_b_traceBarkerType  *)wiCalloc(pbRX->N22, sizeof(Nevada_b_traceBarkerType)  );
        pbRX->traceDSSS    = (Nevada_b_traceDSSSType    *)wiCalloc(pbRX->N22, sizeof(Nevada_b_traceDSSSType)    );
        pbRX->traceCCKInput= (Nevada_b_traceCCKFBFType  *)wiCalloc( 65536,  sizeof(Nevada_b_traceCCKFBFType)  );
        pbRX->traceCCKTVMF = (Nevada_b_traceCCKTVMFType *)wiCalloc( 65536,  sizeof(Nevada_b_traceCCKTVMFType) );
        pbRX->traceCCKFWT  = (Nevada_b_traceCCKFWTType  *)wiCalloc(262144,  sizeof(Nevada_b_traceCCKTVMFType) );
        pbRX->traceResample= (Nevada_b_traceResampleType*)wiCalloc(N80,     sizeof(Nevada_b_traceResampleType));
        ...
        {
            wiUtil_WriteLogFile("Nevada_bRX_AllocateMemory: N44 = %u, N80 = %u\n",pbRX->N44, N80);
            return STATUS(WI_ERROR_MEMORY_ALLOCATION);
        }
    }
    // ---------------------------------
...
// end of Nevada_bRX_AllocateMemory()

// ================================================================================================
// FUNCTION : Nevada_bRX_FreeMemory()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_bRX_FreeMemory(Nevada_bRxState_t *pbRX, wiBoolean ClearPointerOnly)
{
    #define FREE(ptr)                                        \
    {                                                        \
...
// end of Nevada_bRX_FreeMemory()

// ================================================================================================
// FUNCTION  : Nevada_bTX_DataGenerator()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bTX_DataGenerator(wiUWORD RATE, int NumByte, wiUWORD *PSDU, wiUWORD *a, wiBoolean ModSel)
{
    int b,i,k,n,r;
    Nevada_TxState_t *pTX = Nevada_TxState();
...
// end of Nevada_bTX_DataGenerator()

// ================================================================================================
// FUNCTION  : Nevada_bTX_Scrambler()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bTX_Scrambler(wiUWORD RATE, int Na, wiUWORD *a, wiUWORD *b)
{
    int k; wiUWORD X;
...
// end of Nevada_bTX_Scrambler()

// ================================================================================================
// FUNCTION  : Nevada_bTX_DBPSK()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bTX_DBPSK(int N, wiUWORD *b, wiUWORD *cReal, wiUWORD *cImag, wiUWORD *pp)
{
...
// end of Nevada_bTX_DBPSK()

// ================================================================================================
// FUNCTION  : Nevada_b_DQPSK_Encoding()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
__inline int Nevada_b_DQPSK_Encoding(wiUWORD b0, wiUWORD b1)
{
...
//             pp    -- phase of previous symbol in quadrant
// ================================================================================================
void Nevada_bTX_DQPSK(int N, wiUWORD *b, wiUWORD *cReal, wiUWORD *cImag, wiUWORD *pp)
{
    const wiUInt Barker[11] = {0,1,0,0,1,0,0,0,1,1,1}; // Barker sequence in multiples of pi
    ...
    {
        pp->w2 += Nevada_b_DQPSK_Encoding(b[n], b[n+1]); // differential phase modulation
...
// end of Nevada_bTX_DQPSK()

// ================================================================================================
// FUNCTION  : Nevada_bTX_CCK()
// ------------------------------------------------------------------------------------------------
...
//             pp    -- phase of previous symbol in quadrant
// ================================================================================================
void Nevada_bTX_CCK(wiUWORD p1, wiUWORD p2, wiUWORD p3, wiUWORD p4, wiUWORD *cReal, wiUWORD *cImag)
{
...
// end of Nevada_bTX_CCK()

// ================================================================================================
// FUNCTION  : Nevada_bTX_CCK55()
// ------------------------------------------------------------------------------------------------
...
//             pp    -- phase of previous symbol in quadrant
// ================================================================================================
void Nevada_bTX_CCK55(int N, wiUWORD *b, wiUWORD *cReal, wiUWORD *cImag, wiUWORD *pp)
{
    int i;            // symbol index
    int k;            // sample index
    int n;            // bit index
    ...
    {
        pp->w2+= Nevada_b_DQPSK_Encoding(b[n], b[n+1]); // DQPSK for common phase
        ...
        Nevada_bTX_CCK(*pp, p2, p3, p4, cReal+k, cImag+k);
    }
}
// end of Nevada_bTX_CCK55

// ================================================================================================
// FUNCTION  : Nevada_bTX_CCK110()
// ------------------------------------------------------------------------------------------------
...
//             pp    -- phase of previous symbol in quadrant
// ================================================================================================
void Nevada_bTX_CCK110(int N, wiUWORD *b, wiUWORD *cReal, wiUWORD *cImag, wiUWORD *pp)
{
    int i;            // symbol index
    int k;            // sample index
    int n;            // bit index
...
// end of Nevada_bTX_CCK110

// ================================================================================================
// FUNCTION  : Nevada_bTX_Modulation
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bTX_Modulation(wiUWORD RATE, int NumByte, wiUWORD *b, wiUWORD *cReal, wiUWORD *cImag)
{
    ...
    // Modulation: Preamble
    // ---------------------
    Nevada_bTX_DBPSK((RATE.b2? 72:144), b, cReal, cImag, &phase);
    b     +=     RATE.b2? 72:144;
    ...
    // Modulation: Header
    // -------------------
    if (RATE.b2) Nevada_bTX_DQPSK(24, b, cReal, cImag, &phase); // short preamble
    else         Nevada_bTX_DBPSK(48, b, cReal, cImag, &phase); // long preamble
    ...
    switch (RATE.w2)
    {
        case 0: Nevada_bTX_DBPSK (8*NumByte,   b, cReal, cImag, &phase); break;
        case 1: Nevada_bTX_DQPSK (8*NumByte/2, b, cReal, cImag, &phase); break;
        case 2: Nevada_bTX_CCK55 (8*NumByte/4, b, cReal, cImag, &phase); break;
        case 3: Nevada_bTX_CCK110(8*NumByte/8, b, cReal, cImag, &phase); break;
    }
}
// end of Nevada_bTX_Modulation()

// ================================================================================================
// FUNCTION  : Nevada_bTX_PulseFilter_FIR
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bTX_PulseFilter_FIR(int Nc, wiUWORD *c, wiWORD *x)
{
...
// end of Nevada_bTX_PulseFilter_FIR()

// ================================================================================================
// FUNCTION  : Nevada_bTX_PulseFilter
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bTX_PulseFilter(int Nc, wiUWORD *cReal, wiUWORD *cImag, wiWORD *xReal, wiWORD *xImag)
{
    Nevada_bTX_PulseFilter_FIR(Nc, cReal, xReal);
    Nevada_bTX_PulseFilter_FIR(Nc, cImag, xImag);
}
// end of Nevada_bTX_PulseFilter()

// ================================================================================================
// FUNCTION  : Nevada_bTX_Resample_Core
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bTX_Resample_Core(int N, wiWORD *x, wiWORD *y, int bResamplePhase)
{
...
// end of Nevada_bTX_Resample_Core()

// ================================================================================================
// FUNCTION  : Nevada_bTX_Resample
// ------------------------------------------------------------------------------------------------
// Purpose   : Transmit signal sample rate conversion from 44 to 40 MS/s
// Parameters: Nc    -- Number of input samples
...
// ================================================================================================
void Nevada_bTX_Resample(int N, wiWORD *xReal, wiWORD *xImag, wiWORD *yReal, wiWORD *yImag, wiInt bResamplePhase)
{
    ...
    Nevada_bTX_Resample_Core(N, xReal, yReal, bResamplePhase);
    Nevada_bTX_Resample_Core(N, xImag, yImag, bResamplePhase);
}
// end of Nevada_bTX_Resample()

// ================================================================================================
// FUNCTION  : Nevada_bTX_UpsampleFIR
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bTX_UpsampleFIR(int Ny, wiWORD *x, wiWORD *y)
{
...
// end of Nevada_bTX_UpsampleFIR()

// ================================================================================================
// FUNCTION  : Nevada_bTX_Upsample
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bTX_Upsample(int Nx, wiWORD *xReal, wiWORD *xImag, wiWORD *yReal, wiWORD *yImag)
{
    Nevada_bTX_UpsampleFIR(Nx, xReal, yReal);
    Nevada_bTX_UpsampleFIR(Nx, xImag, yImag);
}
// end of Nevada_bTX_Upsample()
...
// ================================================================================================
wiStatus Nevada_bTX(wiUWORD RATE, wiUWORD LENGTH, wiUWORD *MAC_TX_D, int *Nx, wiComplex **x, int M) 
{
    int NsPSDU, Na, Nc, dv;
    ...
    XSTATUS(Nevada_bTX_AllocateMemory(Na, Nc, M) );

    // ---------------------------------------------------------------
    // Generate bit sequence from PSDU bytes, preamble type, data rate
    // ---------------------------------------------------------------
    Nevada_bTX_DataGenerator(RATE, LENGTH.w12, MAC_TX_D, pTX->a, pTX->SetPBCC);
    Nevada_bTX_Scrambler    (RATE, Na, pTX->a, pTX->b);
    Nevada_bTX_Modulation   (RATE, LENGTH.w12, pTX->b, pTX->cReal, pTX->cImag);
    ...
    Nevada_bTX_PulseFilter(Nc, pTX->cReal, pTX->cImag, pTX->vReal+dv, pTX->vImag+dv);

    // -------------------
    // Resample and Output
    // -------------------
    Nevada_bTX_Resample(pTX->Nv, pTX->vReal, pTX->vImag, pTX->xReal, pTX->xImag, pTX->bResamplePhase);
    Nevada_bTX_Upsample(pTX->Ny, pTX->xReal, pTX->xImag, pTX->yReal, pTX->yImag);
    Nevada_Upmix       (pTX->Ny, pTX->yReal, pTX->yImag, pTX->zReal, pTX->zImag);
    Nevada_TxIQSource  (pTX->Nz, pTX->zReal, pTX->zImag, pTX->qReal, pTX->qImag);
    Nevada_TxIQ        (pTX->Nz, pTX->qReal, pTX->qImag, pTX->rReal, pTX->rImag);
    Nevada_Prd         (pTX->Nz, pTX->rReal, pTX->rImag, pTX->uReal, pTX->uImag);
    Nevada_DAC         (pTX->Nz, pTX->uReal, pTX->uImag, pTX->d, M);
...
// ================================================================================================
// FUNCTION  : Nevada_bTX_ResampleToDAC()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
wiStatus Nevada_bTX_ResampleToDAC(void)
{
    Nevada_TxState_t  *pTX  = Nevada_TxState();
...
// end of Nevada_bTX_ResampleToDAC()

// ================================================================================================
// FUNCTION  : Nevada_bRX_Filter
// ------------------------------------------------------------------------------------------------
...
//             y -- output, 11 bit
// ================================================================================================
void Nevada_bRX_Filter(int n, wiWORD *x, wiWORD *y)
{
    const int h[14]={-1,-2,-2, 1, 6, 12, 16, 16, 12, 6, 1,-2,-2,-1};

    wiWORD *d = &(Nevada_bRxState()->dRxFilter[n][0]);
    int i;
...
// end of Nevada_bRX_Filter()

// ================================================================================================
// FUNCTION  : Nevada_bRX_DigitalGain
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bRX_DigitalGain(wiWORD *qReal, wiWORD *qImag, wiWORD *xReal, wiWORD *xImag, wiWORD DGain)
{
...
// end of Nevada_bRX_DigitalGain

// ================================================================================================
// FUNCTION  : Nevada_bRX_Interpolate()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bRX_Interpolate(wiWORD *LPFOutR, wiWORD *LPFOutI, wiWORD *IntpOutR, wiWORD *IntpOutI,
                            wiWORD cSP, wiBoolean PLLEnB, wiBoolean TransCCK, wiBoolean INTERPinit,
                            wiBoolean *IntpFreeze, Nevada_RxState_t *pRX)
{
    const wiUInt LUT32div11[16] = {0, 3, 6, 9, 12, 15, 17, 20, 23, 26, 29}; // = round(index*32/11)

    Nevada_bRX_Interpolate_t *pState = &(Nevada_bRxState()->Interpolate);
...
// end of Nevada_bRX_Interpolate()

// ================================================================================================
// FUNCTION  : Nevada_b_BarkerCorrelator()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_b_BarkerCorrelator( wiWORD *BCInR22, wiWORD *BCInI22, wiWORD *BCInR11, wiWORD *BCInI11,
                                wiWORD *BCOutR, wiWORD *BCOutI, int BCMode, int BCEnB )
{
    const int Barker[11] = {0,0,0,1,1,1,0,1,1,0,1}; // Time-reversed Barker sequence

    Nevada_bBarkerCorrelator_t *pState = &(Nevada_bRxState()->BarkerCorrelator);
    wiWORD *dR = pState->dR;
    wiWORD *dI = pState->dI;
...
// end of Nevada_b_BarkerCorrelator()

// ================================================================================================
// FUNCTION  : Nevada_b_EnergyDetect()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_b_EnergyDetect(wiWORD *EDInR, wiWORD *EDInI, int EDEnB, wiUWORD EDwindow, wiUReg *EDOut)
{
    Nevada_bEnergyDetect_t *pState = &(Nevada_bRxState()->EnergyDetect);
    ...
    if (pState->dEDEnB.Q.b0 || EDEnB || pState->EDdone.D.b0) 
        ...
        pState->acc.D.w20 = pState->acc.Q.w20 + (pState->zReal.Q.w7)*(pState->zReal.Q.w7) + (pState->zImag.Q.w7)*(pState->zImag.Q.w7);
...
// end of Nevada_b_EnergyDetect()

// ================================================================================================
// FUNCTION  : Nevada_b_CorrelationQuality()
// ------------------------------------------------------------------------------------------------
// Purpose   : 
...
// ================================================================================================
void Nevada_b_CorrelationQuality( wiWORD BCOutR, wiWORD BCOutI, wiBoolean CQEnB, wiBoolean CSEnB, 
                                  wiUWORD CQwindow, wiUWORD SSwindow, wiUReg *CQOut, wiUReg *CQPeak )
{
    Nevada_bCorrelationQuality_t *pState = &(Nevada_bRxState()->CorrelationQuality);
...
// end of Nevada_b_CorrelationQuality()

// ================================================================================================
// FUNCTION  : Nevada_b_AGC()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_b_AGC( wiBoolean AGCEnB, wiBoolean AGCInitGains, wiUWORD EDOut, wiBoolean AGCbounded, 
                   wiBoolean LgSigAFE, wiUReg *LNAGain, wiUReg *AGain, wiReg *DGain, wiUReg *LargeSignal, 
                   wiUReg *GainUpdate, wiUWORD *AGCAbsEnergy, wiUReg *AGCAbsPwr, NevadaRegisters_t *pReg )
{
    Nevada_bAGC_t *pState = &(Nevada_bRxState()->AGC);
    ...
    if (UpdateGain)
    {
        ...
        pState->GainLimit.D.word = (pState->UpdateCount.w4==pReg->bMaxUpdateCount.w4)? 1:0; // check for update count limit
        ...
        pState->negGain.D.w8 = dXGain2.w7 + (signed)(LNAGain2.b1? 0:pReg->bStepLNA2.w5) + (signed)(LNAGain2.b0? 0:pReg->bStepLNA.w5);
...
// ================================================================================================
void Nevada_b_EstimateCFO( wiBoolean CFOeEnB, wiBoolean CFOeGet, wiWORD BCOutR, wiWORD BCOutI, 
                           wiReg *CFO, NevadaRegisters_t *pReg )
{
    ...
    Nevada_bEstimateCFO_t *pState = &(Nevada_bRxState()->EstimateCFO);
    ...
    if (CFOeEnB == 0)
    {
        ...
        if (CFOeGet)
        {
            ...
            //
            if (Nevada_RxState()->NonRTL.Enabled && Nevada_RxState()->NonRTL.DisableCFO)
                CFO->D.word = 0;
        }
        else
...
// end of Nevada_b_EstimateCFO()

// ================================================================================================
// FUNCTION  : Nevada_b_CorrectCFO()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_b_CorrectCFO( wiBoolean CFOcEnB, wiBoolean TransCCK, wiBitDelayLine dDeciFreeze, wiWORD CFO, wiWORD cCP, 
                          wiWORD CFOcInR, wiWORD CFOcInI, wiWORD *CFOcOutR, wiWORD *CFOcOutI, wiBoolean posedgeCLK11MHz )
{
    ...
    Nevada_bCorrectCFO_t *pState = &(Nevada_bRxState()->CorrectCFO);
...
//end of Nevada_b_CorrectCFO()

// ================================================================================================
// FUNCTION  : Nevada_b_ChanEst()
// ------------------------------------------------------------------------------------------------
...
//             RX         -- bNevada RX data structure
//             pReg       -- configuration registers
// ================================================================================================
void Nevada_b_ChanEst( wiBoolean CEEnB, wiBoolean CEMode, wiBoolean GetCE, wiWORD BCOutR, wiWORD BCOutI,
                       wiUWORD DMOut, wiBoolean DMOutValid, wiBoolean DBPSK, wiBoolean *CEDone,
                       Nevada_bRxState_t *RX, NevadaRegisters_t *pReg )
{
    Nevada_bChanEst_t *pState = &(Nevada_bRxState()->ChanEst);
    ...
    if (!CEEnB)
    {
        ...
        if (pState->count>32 || pState->SymBufCount>2) { // check for buffer overrun (model only)
            wiPrintf("*** Nevada_b_ChanEst ERROR: count=%d, SymBufCount=%d\n",pState->count,pState->SymBufCount);
...
// end of Nevada_b_ChanEst()

// ================================================================================================
// FUNCTION  : Nevada_bRX_SP()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bRX_SP(wiWORD *uReal, wiWORD *uImag, wiWORD **wRptr, wiWORD **wIptr, int Np2, 
                   wiBoolean SPEnB, wiBoolean TransCCK, wiBitDelayLine dDeciFreeze)
{
    Nevada_bRxState_t *pbRX = Nevada_bRxState();
    Nevada_bRX_SP_t   *pState = &(pbRX->RX_SP);
...
// end of Nevada_bRX_SP()

// ================================================================================================
// FUNCTION  : Nevada_bRX_CMF()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bRX_CMF( wiBoolean CMFEnB, wiBitDelayLine DMSym, wiBitDelayLine dDeciFreeze, 
                     wiWORD *uReal, wiWORD *uImag, wiWORD **SSMatOutR, wiWORD **SSMatOutI, 
                     int Nh, wiWORD *hReal, wiWORD *hImag )
{
    Nevada_bRX_CMF_t *pState = &(Nevada_bRxState()->CMF);
...
// end of Nevada_bRX_CMF()

// ================================================================================================
// FUNCTION  : Nevada_bRX_DSSS()
// ------------------------------------------------------------------------------------------------
...
//
// ================================================================================================
void Nevada_bRX_DSSS( wiBoolean DMEnB, wiWORD *CFOOutR, wiWORD *CFOOutI, wiWORD *CMFOutR, wiWORD *CMFOutI,
                      ...
                      wiWORD *BFReal, wiWORD *BFImag )
{
    ...
    Nevada_bRX_DSSS_t *pState = &(Nevada_bRxState()->RX_DSSS);
...
// end of Nevada_bRX_DSSS()

// ================================================================================================
// FUNCTION  : Nevada_bRX_CCK()
// ------------------------------------------------------------------------------------------------
// Purpose   : CCK Demodulator
// Parameters: 
// ================================================================================================
void Nevada_bRX_CCK(wiWORD *wR, wiWORD *wI,
                    ...
                    )
{
    ...
    Nevada_bRxState_t *pbRX = Nevada_bRxState();
    Nevada_bRX_CCK_t  *pState = &(pbRX->RX_CCK);
...
// end of Nevada_bRX_CCK()

// ================================================================================================
// FUNCTION  : Nevada_bRX_SFD()
// ------------------------------------------------------------------------------------------------
...
//             SFDone      -- SFD found
// ================================================================================================
void Nevada_bRX_SFD(wiBoolean SFDEnB, wiUWORD DesOut, wiBoolean DesOutValid, 
                    wiUReg *Preamble, wiUReg *SFDDone)
{
    ...
    Nevada_bRX_SFD_t *pState = &(Nevada_bRxState()->RX_SFD);
...
// end of Nevada_bRX_SFD()

// ================================================================================================
// FUNCTION  : Nevada_bRX_Header()
// ------------------------------------------------------------------------------------------------
...
//             Modulation    -- modulation type: {0=CCK, 1=PBCC-not supported by Nevada}
...
// ================================================================================================
void Nevada_bRX_Header(wiBoolean HDREnB, wiBoolean DBPSK, wiBoolean shortPreamble, wiBoolean SSOutValid, 
                       wiUWORD *a, wiBitDelayLine *HDRDone, Nevada_bRxState_t *RX)
{
    ...
    else
    {
        ...
        if (*pNbit == 48)
        {
            ...
            Nevada_Registers()->RX_SERVICE.word = RX->bSERVICE.w8;
            ...
            //
            //  The conditions for CRCpass are different in the Nevada baseband. Here the
...
// end of Nevada_bRX_Header()

// ================================================================================================
// FUNCTION  : Nevada_bRX_PSDU()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bRX_PSDU( wiBoolean PSDUEnB, wiBoolean DesOutValid, wiBitDelayLine *PSDUDone, 
                      Nevada_RxState_t *RX)
{
    wiUWORD *pBitCount = &(Nevada_bRxState()->RX_PSDU.BitCount);
...
// end of Nevada_bRX_PSDU()

// ================================================================================================
// FUNCTION  : Nevada_bRX_DPLL()
// ------------------------------------------------------------------------------------------------
// Purpose   : 
// Parameters: PLLEnB
//             PSDUEnB
...
//             DMSym and cckSym.
// ================================================================================================
void Nevada_bRX_DPLL(wiBoolean PLLEnB, wiWORD *SSMatOutR, wiWORD *SSMatOutI, wiBitDelayLine SSCorOutValid, 
                     ...
                     wiWORD *cCP, wiWORD *cSP, NevadaRegisters_t *pReg)
{
    Nevada_bRX_DPLL_t *pState = &(Nevada_bRxState()->RX_DPLL);
...
// end of Nevada_bRX_DPLL()

// ================================================================================================
// FUNCTION  : Nevada_bRX_Descrambler()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_bRX_Descrambler(wiUWORD SSOut, wiBoolean SSOutValid, wiUWORD cckOut, wiBoolean cckOutValid,
                            wiBoolean DBPSK, wiUReg *DesOut, wiUReg *DesOutValid, Nevada_bRxState_t *RX, wiUInt FrontState)
{
...
// end of Nevada_bRX_Descrambler()

// ================================================================================================
// FUNCTION  : Nevada_b_Downsample()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_b_Downsample( wiBoolean posedgeCLK22MHz, wiBoolean posedgeCLK11MHz, wiUInt DeciOutEnB,
                          ...
                          wiUReg *DeciFreeze)
{
    Nevada_b_Downsample_t *pState = &(Nevada_bRxState()->Downsample);
...
// end of Nevada_b_Downsample()

// ================================================================================================
// FUNCTION  : Nevada_bRX_top()
// ------------------------------------------------------------------------------------------------
...
//             Run11b      --
// ================================================================================================
void Nevada_bRX_top( Nevada_RxState_t *pRX, Nevada_bRxState_t *RX, NevadaRegisters_t *Reg, 
                     ...
                     Nevada_bSigState_t *bSigOut, Nevada_bSigState_t bSigIn)
{
    Nevada_bRxState_t *pbRX = Nevada_bRxState();
    Nevada_bRxTop_t   *pRxTop = &(pbRX->RxTop);

    Nevada_b_FrontCtrlRegType *inReg = &(pRxTop->inReg);
    Nevada_b_FrontCtrlRegType outReg;
    ...
    if (posedgeCLK22MHz)
    {
        ...
        if (bRXEnB)
        {
            ...
            if (!Nevada_RxState()->EnableTrace) return; // skip subsequent processing to speed up simulation
        }
    }
    ...
    if (posedgeCLK40MHz)
    {
        Nevada_bRX_Filter( 0, qReal, RX->xReal+k40 ); // filter #0: real
        Nevada_bRX_Filter( 1, qImag, RX->xImag+k40 ); // filter #1: imag
        Nevada_bRX_DigitalGain( RX->xReal+k40, RX->xImag+k40, RX->yReal+k40, RX->yImag+k40, pRxTop->DGain.Q );
    }
    if (posedgeCLK40MHz || posedgeCLK44MHz)
    {
        Nevada_bRX_Interpolate( RX->yReal+k40, RX->yImag+k40, RX->sReal+k44, RX->sImag+k44, pRxTop->cSP.Q, 
    ...
    if (posedgeCLK44MHz)
    {
        Nevada_b_AGC( outReg.AGCEnB, outReg.AGCInit, pRxTop->EDOut.Q, outReg.AGCbounded, 
        ...
        Nevada_b_Downsample( posedgeCLK22MHz, posedgeCLK11MHz, inReg->CFOcEnB, pRxTop->IntpFreeze, 
			                 outReg.SymPhase, RX->sReal[k44], RX->sImag[k44],
                             RX->zReal+k22, RX->zImag+k22, RX->vReal+k11, RX->vImag+k11, 
							 &pRxTop->DeciFreeze );

        Nevada_bRX_DPLL( outReg.PLLEnB, pRxTop->SSMatOutR, pRxTop->SSMatOutI, pRxTop->posedge_SSCorOutValid, 
    ...
    if (posedgeCLK22MHz)
    {
        Nevada_b_CorrectCFO( outReg.CFOcEnB, outReg.TransCCK, pRxTop->dDeciFreeze, pRxTop->CFO.Q, pRxTop->cCP, 
			                 RX->vReal[k11], RX->vImag[k11], RX->uReal+k11, RX->uImag+k11, posedgeCLK11MHz );

        Nevada_b_BarkerCorrelator( RX->zReal+k22, RX->zImag+k22, RX->uReal+k11, RX->uImag+k11,
                                   RX->rReal+k22, RX->rImag+k22, outReg.BCMode, outReg.BCEnB );

        Nevada_b_EnergyDetect( RX->zReal+k22, RX->zImag+k22, outReg.EDEnB, Reg->EDwindow, &pRxTop->EDOut );

        Nevada_b_CorrelationQuality( RX->rReal[k22], RX->rImag[k22], outReg.CQEnB, outReg.CSEnB, 
			                         Reg->CQwindow, Reg->SSwindow, &pRxTop->CQOut, &pRxTop->CQPeak );

        Nevada_b_EstimateCFO(outReg.CFOeEnB, outReg.CFOeGet, RX->rReal[k22], RX->rImag[k22], &pRxTop->CFO, Reg);
    }
    // ----------------------------------------------------------------------
    //
    // 11 MHz Clock Domain
    ...
    if (posedgeCLK11MHz && (k11 > 22))
    {
        ...
        Nevada_b_ChanEst( outReg.CEEnB, outReg.CEMode, outReg.CEGet, RX->rReal[k22], RX->rImag[k22], 
			              pRxTop->SSOut.Q, pRxTop->SSOutValid.Q.b0, outReg.DBPSK, &pRxTop->CEDone, RX, Reg);

        Nevada_bRX_CMF( outReg.CMFEnB, pRxTop->DMSym, pRxTop->dDeciFreeze, RX->uReal+k11, RX->uImag+k11, 
			            &pRxTop->SSMatOutR, &pRxTop->SSMatOutI, RX->Nh, RX->hReal, RX->hImag );

        Nevada_bRX_DSSS( outReg.DMEnB, RX->uReal+k11, RX->uImag+k11, pRxTop->SSMatOutR, pRxTop->SSMatOutI, 
        ...
        Nevada_bRX_SP( RX->uReal+k11, RX->uImag+k11, &wR, &wI, RX->Np2, 
			           outReg.SPEnB, outReg.TransCCK, pRxTop->dDeciFreeze );

        if (!outReg.PSDUEnB && pRxTop->cckSym.delay_10T)
        {
            Nevada_bRX_CCK( wR-10,wI-10, &(pRxTop->cckOut.D), &(pRxTop->cckWalOutValid.D), 
        ...
        Nevada_bRX_Descrambler( pRxTop->SSOut.Q, pRxTop->SSOutValid.Q.b0, pRxTop->cckOut.Q, pRxTop->cckOutValid.Q.b0, 
			                    outReg.DBPSK, &pRxTop->DesOut, &pRxTop->DesOutValid, RX, pRxTop->state.Q.w6 );

        Nevada_bRX_SFD (outReg.SFDEnB, pRxTop->DesOut.Q, pRxTop->DesOutValid.Q.b0, &pRxTop->Preamble, &pRxTop->SFDDone);

        Nevada_bRX_Header( outReg.HDREnB, outReg.DBPSK, pRxTop->Preamble.Q.b0, 
			               pRxTop->SSOutValid.Q.b0, RX->a, &pRxTop->HDRDone, RX );

        Nevada_bRX_PSDU(outReg.PSDUEnB, pRxTop->DesOutValid.Q.b0, &pRxTop->PSDUDone, pRX);
    }
    // -----------------------
    ...
    //
    // ----------------------------------------------------------------------
    if (posedgeCLK22MHz && Nevada_RxState()->EnableTrace)
    {
