...
//=================================================================================================
//
//  wiseMex Command Dispatch for the Tamale method
...
{
    if (!strstr(cmd, "Tamale_")) 
        return WI_WARN_PARSE_FAILED; // indicate no command match

	#ifdef __TAMALE_H

		//---------------------------------------------------------------------------
		if COMMAND("Tamale_GetDvParameter")
		{
            char *Param;
			Tamale_DvState_t *pState = Tamale_DvState();
            ...
            Param = wiMexUtil_GetStringValue(prhs[1],0);
            ...
            else if MATCH(Param,"N_PHY_TX_RD"      ) wiMexUtil_PutUIntArray   (TAMALE_MAX_DV_TX, pState->N_PHY_TX_RD, &plhs[0]);
                ...
                mexErrMsgTxt("Tamale_GetDvParameter: parameter not available");
		...
		//---------------------------------------------------------------------------
		if COMMAND("Tamale_GetTxParameter")
		{
            char *Param;
			Tamale_TxState_t *pState = Tamale_TxState();
            ...
            Param = wiMexUtil_GetStringValue(prhs[1],0);
                ...
                mexErrMsgTxt("wiTest_GetTxParameter: parameter not available");
		...
		//---------------------------------------------------------------------------
		if COMMAND("Tamale_GetRxParameter")
		{
            char *Param;
            ...
            Param = wiMexUtil_GetStringValue(prhs[1],0);
                ...
                mexErrMsgTxt("wiTest_GetRxParameter: parameter not available");
		...
		//-------------------------------------------------------------------------------
		if COMMAND("Tamale_GetRxFrameCount")
		{
		...
		//-------------------------------------------------------------------------------
		if (COMMAND("bTamale_GetRxState") || COMMAND("Tamale_b_GetRxState"))
		{
			const char *field_names[] = {
		...
		if COMMAND("Tamale_TxPacket")
		{
			...
			MCHK(Tamale_TX(Length, MAC_TX_D, &Nx, &x, 1));
		...
		//-------------------------------------------------------------------------------
		if COMMAND("Tamale_bTX_ResampleToDAC")
		{
			wiMexUtil_CheckParameters(nlhs,nrhs,0,1);
          		
			MCHK( Tamale_bTX_ResampleToDAC() );
			return WI_SUCCESS;
		}
		//-------------------------------------------------------------------------------
		if COMMAND("Tamale_ReadRegister")
		{
			wiUInt Addr, Data;
            ...
            if (Addr > 1023)
                mexErrMsgTxt("Tamale_ReadRegister: Address out of range (0-1023)");

            Data = Tamale_ReadRegister(Addr);
            wiMexUtil_PutUIntValue(Data, &plhs[0]);
			return WI_SUCCESS;
		}
		//-------------------------------------------------------------------------------
		if COMMAND("Tamale_WriteRegister")
		{
			wiUInt Addr, Data;
            ...
            Data = Tamale_WriteRegister(Addr, Data);
			return WI_SUCCESS;
		}
		//-------------------------------------------------------------------------------
		if (COMMAND("Tamale_ReadRegisterMap") || COMMAND("Tamale_PutRegisterMap"))
		{
			wiUInt *pRegMap;
            int NumRegs;
			...
			MCHK(Tamale_SetRegisterMap(pRegMap, NumRegs));
			if (pRegMap) mxFree(pRegMap);

			return WI_SUCCESS;
		}
		//-------------------------------------------------------------------------------
		if (COMMAND("Tamale_WriteRegisterMap") || COMMAND("Tamale_GetRegisterMap"))
		{
			wiUWORD *RegMap;   // register map (Tx/Rx combined)
			wiMexUtil_CheckParameters(nlhs,nrhs,1,1);
			MCHK(Tamale_GetRegisterMap(&RegMap));
			wiMexUtil_PutUWORDArray(256, RegMap, &plhs[0]);

			return WI_SUCCESS;
		}
		//-------------------------------------------------------------------------------
		if COMMAND("Tamale_GetRxHeader")
		{
			Tamale_RxState_t *pRX = Tamale_RxState();
			wiMexUtil_CheckParameters(nlhs,nrhs,1,1);
		...
		//-------------------------------------------------------------------------------
		if COMMAND("Tamale_WriteConfig")
		{
			mexPrintf(" -----------------------------------------------------------------------------------------------\n");
			mexPrintf(" wiseMex (%s): Tamale Configuration\n",__DATE__);
			mexPrintf(" -----------------------------------------------------------------------------------------------\n");
			MCHK( Tamale_WriteConfig(wiMexUtil_Output) );
			mexPrintf(" -----------------------------------------------------------------------------------------------\n");
			return WI_SUCCESS;
		...
		mexErrMsgTxt("wiseMex: Tamale_ command received but Tamale.h not included.");

	#endif  // TAMALE_H

	return WI_WARN_PARSE_FAILED; // indicate no command match
}
// end of wiseMex_Tamale()
