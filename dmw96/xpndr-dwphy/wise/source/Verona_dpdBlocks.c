...
dpdComplexInt Verona_dpdRealMultInt(dpdComplexInt a, dpdRealInt b)
{
...
dpdRealInt Verona_dpdRealInit(dpdInt a, dpdUInt b)
{
...
dpdInt Verona_convert_to_signed(dpdInt indata, int bits)
{
	dpdUInt NORM, HNORM;
	dpdInt out;
...
int Verona_SATURATE(int a, int LIMIT)
{
	a = a>LIMIT? LIMIT : a;
	a = a<-LIMIT? -LIMIT : a;
	return a;
...
int Verona_SATURATE2(int a, int LIMIT)
{
	a = a>LIMIT? LIMIT : a;
...
dpdStatus Verona_dpdBlock_CleanFilters(int dpd_block_id)
{
	int i;
	dpdBlock *cdpd;
	...
	for (i=0; (unsigned)i<cdpd->num_of_firs; i++)
	{
		...
		Verona_clean_delay_line(delay_line);
	}
...
dpdStatus Verona_dpdBlock_Close(int dpd_block_id)
{
	unsigned int i;
	dpdBlock *cdpd;
...
dpdStatus Verona_dpdBlock_SetFilterCoeff(int dpd_block_id, int bank_set)
{
	unsigned int i, j;
	int coeffr, coeffi;
	int precision;
	...
	for (i=0; i<cdpd->num_of_firs; i++)
	{
		...
		for (j=0; j<cdpd->VFP.vfir[i]->taps; j++)
		{
			...
			coeffr = Verona_convert_to_signed(coeffr, precision);
			FILTER_POINTER++;
			coeffi = FILTER_POINTER->word;
			coeffi = Verona_convert_to_signed(coeffi, precision);
			FILTER_POINTER++;
...
dpdStatus Verona_dpdBlock_Init(int dpd_block_id)
{
	unsigned int i;
	dpdBlock *cdpd;
//	void *tmp;
	...
	Verona_dpdVFIRs_Init(cdpd);

	return dpd_success;
}

dpdBlock *Verona_get_dpdBlock(int dpd_block_id)
{
	return DMW_DPD[dpd_block_id];
}

dpdStatus Verona_dpdVFIRs_Init(dpdBlock *cdpd)
{
	unsigned int i;
	dpdComplexInt *dl;
	dpdComplexInt *coeff;
...
dpdStatus Verona_dpdVFP(int dpd_block_id, dpdComplexInt *dpd_in, int plen, dpdComplexInt *dpd_out)
{
	int i, j;
//	int block_size = 1;
//	dpdUInt precision = 0;
	...
	// {
		...
		Verona_dpdHighOrder(cdpd, dpd_in_bank, plen, 0);
	// }
	//for (i=0; i</*plen*/1; i++)
	//{
		for (j=0; (unsigned)j<cdpd->num_of_firs; j++)
		{
			dline = Verona_get_dpdBlock(dpd_block_id)->VFP.vfir[j]->fir;		// pointer to the delay line of corresponeded filter
			tmpbuffer = dpd_in_bank[j];									// pointer to the input data
			Verona_complex_fir(dline, tmpbuffer, fir_output[j], plen );			// filter
		}
			for (i=0; i<plen; i++)
			{
				for (j=0; (unsigned)j<cdpd->num_of_firs; j++)
			...
			//dpd_out[i] = fir_sum;
			dpd_out[i].Real = Verona_SATURATE2(fir_sum.Real, (1<<(cdpd->VFP.dpdHigherOrder.dpd_in_precision-1))-1);
			dpd_out[i].Imag = Verona_SATURATE2(fir_sum.Imag, (1<<(cdpd->VFP.dpdHigherOrder.dpd_in_precision-1))-1);
...
void Verona_clean_delay_line(DELAY_LINE_COMPLEX *filter)
{
    int i;
    for (i=0; i<filter->size; i++)
    {
...
dpdStatus Verona_complex_fir(DELAY_LINE_COMPLEX *localdl, dpdComplexInt *p_newsample, dpdComplexInt *outbuffer, int block_size)
{
    int i;
    int j;
    int ptr;
...
dpdStatus Verona_dpdHighOrder(dpdBlock *DPD, dpdComplexInt **dpd_in_bank, int plen, int start_index)
{
	int i;
	...
	for (i=start_index; i<(start_index+plen); i++)
	{
		...
		ri1 = Verona_dpdRealInit(DPD_COMPLEX_REAL(ci1) + DPD_COMPLEX_IMAG(ci1), 8 /*ci1.Precision+1*/);
		...
		ci2 = Verona_dpdRealMultInt(DPD_IN[i], ri1);
		...
		ci3 = Verona_dpdRealMultInt(DPD_IN[i], ri3);
