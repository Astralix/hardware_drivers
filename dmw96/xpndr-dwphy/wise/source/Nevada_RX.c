...
//=================================================================================================
//
//  Nevada - Baseband Receiver Model
...
// ================================================================================================
// FUNCTION  : Nevada_RX_ClockGenerator()
// ------------------------------------------------------------------------------------------------
...
//             modules. Clock edge indicators are stored in the Nevada RX state.
//
...
void Nevada_RX_ClockGenerator(Nevada_RxState_t *pRX, wiBoolean Reset)
{
    ...
    if (Reset)
    {
        ...
        for (k=0; k<InitialPhase; k++) // advance clock generator specified periods
            Nevada_RX_ClockGenerator(pRX, 0);
...
// end of Nevada_RX_ClockGenerator()

// ================================================================================================
// FUNCTION : Nevada_RX_Init()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
...
// end of Nevada_RX_Init()

// ================================================================================================
// FUNCTION  : Nevada_RX_Restart()
// ------------------------------------------------------------------------------------------------
...
wiStatus Nevada_RX_Restart(void)
{
    ...
    Nevada_RxState_t  *pRX  = Nevada_RxState();
    Nevada_bRxState_t *pRXb = Nevada_bRxState();

    // Call the RX1 module local restart
    //
    XSTATUS( Nevada_RX1_Restart() );
...
// end of Nevada_RX_Restart()

// ================================================================================================
// FUNCTION  : Nevada_RX_EndOfWaveform()
// ------------------------------------------------------------------------------------------------
...
wiStatus Nevada_RX_EndOfWaveform(void)
{
    ...
    //
    Nevada_DCX(-2, Zero, NULL, 0, Nevada_RxState(), Nevada_Registers()); // Load values from DCX accumulators

    return WI_SUCCESS;
}
// end of Nevada_RX_EndOfWaveform()

// ================================================================================================
// FUNCTION : Nevada_RX_FreeMemory()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
...
// end of Nevada_RX_FreeMemory()

// ================================================================================================
// FUNCTION : Nevada_RX_AllocateMemory()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_RX_AllocateMemory(void)
{
    ...
    int N20 = pRX->N20 = N80/4 + 2 + (NEVADA_FFT_DELAY + NEVADA_PLL_DELAY);
    int N44 =           (N80*11/10)/2+1;
    int N22 = pRX->N22 = N44/2 + 1;

    Nevada_RX_FreeMemory(pRX, 0);

    pRX->PHY_RX_D = (wiUWORD *)wiCalloc(65536 + 8,        sizeof(wiUWORD));
    pRX->cSP      = (wiWORD  *)wiCalloc(NEVADA_MAX_N_SYM, sizeof(wiWORD));
    pRX->cCP      = (wiWORD  *)wiCalloc(NEVADA_MAX_N_SYM, sizeof(wiWORD));
    pRX->kFFT     = (wiInt   *)wiCalloc(NEVADA_MAX_N_SYM, sizeof(wiInt));
    ...
    if (pRX->EnableTrace)
    {
        ...
        pRX->aSigOut       = (Nevada_aSigState_t        *)wiCalloc(N20, sizeof(Nevada_aSigState_t        ));
        pRX->bSigOut       = (Nevada_bSigState_t        *)wiCalloc(N22, sizeof(Nevada_bSigState_t        ));
        pRX->traceRxClock  = (Nevada_traceRxClockType   *)wiCalloc(N80, sizeof(Nevada_traceRxClockType   ));
        pRX->traceRxState  = (Nevada_traceRxStateType   *)wiCalloc(N80, sizeof(Nevada_traceRxStateType   ));
        pRX->traceRxControl= (Nevada_traceRxControlType *)wiCalloc(N80, sizeof(Nevada_traceRxControlType ));
        pRX->traceRadioIO  = (Nevada_RadioDigitalIO_t   *)wiCalloc(N80, sizeof(Nevada_RadioDigitalIO_t   ));
        pRX->traceDataConv = (Nevada_DataConvCtrlState_t*)wiCalloc(N80, sizeof(Nevada_DataConvCtrlState_t));
        pRX->traceDFE      = (Nevada_traceDFEType       *)wiCalloc(N20, sizeof(Nevada_traceDFEType       ));
        pRX->traceAGC      = (Nevada_traceAGCType       *)wiCalloc(N20, sizeof(Nevada_traceAGCType       ));
        pRX->traceRSSI     = (Nevada_traceRSSIType      *)wiCalloc(N20, sizeof(Nevada_traceRSSIType      ));
        pRX->traceDFS      = (Nevada_traceDFSType       *)wiCalloc(N20, sizeof(Nevada_traceDFSType       ));
        pRX->traceSigDet   = (Nevada_traceSigDetType    *)wiCalloc(N20, sizeof(Nevada_traceSigDetType    ));
        pRX->traceFrameSync= (Nevada_traceFrameSyncType *)wiCalloc(N20, sizeof(Nevada_traceFrameSyncType ));
        pRX->traceRX2      = (Nevada_traceRX2Type       *)wiCalloc(N20, sizeof(Nevada_traceRX2Type       ));
        pRX->traceRx       = (Nevada_traceRxType        *)wiCalloc(N80, sizeof(Nevada_traceRxType        ));
        pRX->traceImgDet   = (Nevada_traceImgDetType    *)wiCalloc(N20, sizeof(Nevada_traceImgDetType    ));
        pRX->traceIQCal    = (Nevada_traceIQCalType     *)wiCalloc(N20, sizeof(Nevada_traceIQCalType     ));
        ...
        {
            wiUtil_WriteLogFile("Nevada_RX_AllocateMemory: N80 = %u\n",N80);
            return STATUS(WI_ERROR_MEMORY_ALLOCATION);
        }
    }
    /////////////////////////////////////////////////////////////////////////////////////
    //
    //  Allocate RX memory in the Nevada_b module
    //
    XSTATUS( Nevada_bRX_AllocateMemory() );

    return WI_SUCCESS;
}
// end of Nevada_RX_AllocateMemory()

// ================================================================================================
// FUNCTION : Nevada_RX_NuisanceFilter()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_RX_NuisanceFilter( wiBoolean CheckNuisance, wiBoolean MAC_NOFILTER, 
                               Nevada_CtrlState_t *SigOut, Nevada_RxState_t *pRX, NevadaRegisters_t *pReg)
{
...
// end of Nevada_RX_NuisanceFilter

// ================================================================================================
// FUNCTION : Nevada_RX_Header()
// ------------------------------------------------------------------------------------------------
// Purpose  : Construct PHY header for RX packets
// ================================================================================================
wiStatus Nevada_RX_Header(Nevada_CtrlState_t outReg, wiUInt AntSel, wiUWORD RSSI0, wiUWORD RSSI1)
{
    wiUWORD RATE, LENGTH;
...
// end of Nevada_RX_Header()


// ================================================================================================
// FUNCTION : Nevada_RX_ERROR()
// ------------------------------------------------------------------------------------------------
// Purpose  : Generate the PHY_RX_ERROR signal (for unsupported/invalid packets)
// ================================================================================================
void Nevada_RX_ERROR( Nevada_CtrlState_t *inReg, Nevada_CtrlState_t outReg, wiUWORD PktTime)
{
    NevadaRegisters_t *pReg = Nevada_Registers();
    Nevada_RxState_t *pRX = Nevada_RxState();
...
// end of Nevada_RX_ERROR()


// ================================================================================================
// FUNCTION : Nevada_RX_CCA()
// ------------------------------------------------------------------------------------------------
// Purpose  : Generate PHY_CCA
// ================================================================================================
void Nevada_RX_CCA( Nevada_CtrlState_t *inReg, Nevada_CtrlState_t outReg, wiUWORD PktTime, 
                    wiUWORD RSSI, wiBoolean PHY_RX_EN, wiBoolean ValidRSSI )
{
    ...
    NevadaRegisters_t *pReg = Nevada_Registers();
    Nevada_RX_CCA_t *pState = &(Nevada_RxState()->CCA);
...
//            accurate timing. For Nevada, this module contains controls from the top
                    ...
                    Nevada_CtrlState_t *inReg, Nevada_CtrlState_t outReg )
{
    ...
    Nevada_RxState_t  *pRX  = Nevada_RxState();
    NevadaRegisters_t *pReg = Nevada_Registers();
    Nevada_RxTop_t    *p    = &(pRX->RxTop);

    Nevada_aSigState_t aSigOut, *aSigIn = &(p->aSigIn);
    Nevada_bSigState_t bSigOut, *bSigIn = &(p->bSigIn);
    ...
    if (pRX->clock.posedgeCLK80MHz)
    {
        ...
        if (pRX->TX.Enabled)
        {
            ...
            if (p->MAC_TX_EN && !dMAC_TX_EN)
                Nevada_DvState()->LastTxStart = pRX->k80; // record the last assertion of MAC_TX_EN
        }
        ...
        switch (outReg.State)
        {
            ...
            case 12: // RX: Valid Header ----------------------------------------------------------
            {
                ...
                //
                p->PktTime.D.word = outReg.TurnOnCCK ? Nevada_bRxState()->bPacketDuration.w16 : pRX->aPacketDuration.w16;
                ...
                aSigIn->HeaderValid = 0;
                    ...
                    Nevada_RX_Header(outReg, AntSel, p->RSSI0, p->RSSI1); 
                };
                break;
            }
            ...
            case 22: // RX: Mixed-Mode HT-SIG Decoding --------------------------------------------
                ...
                if (HeaderValid)
                {
                    ...
                    if (!Unsupported) Nevada_RX_Header(outReg, AntSel, p->RSSI0, p->RSSI1);
                }
                break;            
    ...
    if (pRX->clock.posedgeCLK40MHz)
    {
        ...
        // ---------------------
        Nevada_RX1( p->DFEEnB, p->d2LgSigAFE, &p->LNAGainOFDM, &p->AGainOFDM, &p->RSSI0OFDM, &p->RSSI1OFDM,
        ...
        {
            if (pRX->EnableTrace) Nevada_DFS(p->DFEEnB, aSigOut, &p->aSigIn, pRX, pReg);
            XSTATUS( Nevada_RX2(aSigIn, aSigOut, outReg, pRX, pReg) );
        }
    }
    //////////////////////////////////////////////////////////////////////////////////////////
    //
    ...
    if ((pRX->clock.posedgeCLK40MHz || pRX->clock.posedgeCLK44MHz) && pReg->RXMode.b1)
    {
        ...
        Nevada_bRX_top( pRX, pRXb, pReg, outReg.bRXEnB, &(p->bInR.Q), &(p->bInI.Q), 
    ...
    //  from the MAC.
    //
    Nevada_RX_NuisanceFilter(outReg.State==13, pRX->MAC_NOFILTER, inReg, pRX, pReg);
    ...
    //  TX/RX MAC/PHY interfaces which are implied by operations in Nevada_TX and Nevada_RX.
    //
    ...
    {     
        ...
        // External LNA and Power Amplifier Enables ____________________________
        {
            ...
            Radio->PA50   = (pReg->sPA50.w4 == 15) ? aSigIn->RxIsCCK : SignalMux[ pReg->sPA50.w4   ]; // modified for version 1b
#else
        ...
        //
        Nevada_RX_ERROR( inReg, outReg, p->PktTime.Q );
        Nevada_RX_CCA  ( inReg, outReg, p->PktTime.Q, p->RSSI, p->PHY_RX_EN, aSigOut.ValidRSSI );
        ...
        //
        #define NEVADA_SELECTSIGNAL( SelectSignal, Signal )                                                 \
        {                                                                                                   \
            switch (SelectSignal)                                                                           \
            {                                                                                               \
        ...
        } // end of NEVADA_SELECTSIGNAL() //////////////////////////////////////////////////////////////////

        NEVADA_SELECTSIGNAL(pReg->SelectSignal1.w4, p->Signal1.D.word);
        NEVADA_SELECTSIGNAL(pReg->SelectSignal2.w4, p->Signal2.D.word);
