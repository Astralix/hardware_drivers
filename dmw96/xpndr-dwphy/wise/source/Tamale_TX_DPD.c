...
//=================================================================================================
//
//  Tamale - Digital TX Predistortion (Nonlinear Equalizer)
...
//=================================================================================================
static int Tamale_wiDataRateDecoder(int, int, int, int, int);
static int Tamale_dpdgetFilterBank(int *);

// ================================================================================================
// FUNCTION : Tamale_TX_DPD()
// ------------------------------------------------------------------------------------------------
...
void Tamale_TX_DPD(int N, wiWORD xR[], wiWORD xI[], wiWORD *yR, wiWORD *yI)
{
	...
	pr = Tamale_get_dpdBlock(0)->VFP.dpdHigherOrder.dpd_in_precision;
	inp_limit = (1<<(pr-1))-1;


	bank_selector = Tamale_dpdgetFilterBank(&dpd_state);			// Select filter bank
	// disable
	if (!dpd_state)
	{
		for (k=0; k<N; k++)
	...
	Tamale_dpdBlock_CleanFilters(0);					// Clean filters
	Tamale_dpdBlock_SetFilterCoeff(0, bank_selector);	// Set coefficients for the current packet

	// Convert to complex vector
    for (k=0; k<N; k++)
    {
		DPD_SET_COMPLEX_INT_PR(&DPD_IN[k], Tamale_SATURATE(xR[k].word, inp_limit), 
                               Tamale_SATURATE(xI[k].word, inp_limit), pr);
    }

	// Predistorion
	Tamale_dpdVFP(0, DPD_IN, N, DPD_OUT);

    for (k=0; k<TAMALE_DPD_DELAY; k++)
        yR[k].word = yI[k].word = 0;
    
    // Convert to real vectors
	for (k=TAMALE_DPD_DELAY; k<N; k++)
	{
		yR[k].word = DPD_COMPLEX_REAL(DPD_OUT[k - TAMALE_DPD_DELAY]);
		yI[k].word = DPD_COMPLEX_IMAG(DPD_OUT[k - TAMALE_DPD_DELAY]);
	}

	wiFree(DPD_OUT);
	wiFree(DPD_IN);
}
// end of Tamale_TX_DPD()

// ================================================================================================
// FUNCTION : Tamale_TX_DPD_Init()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_TX_DPD_Init(void)
{
	if (Tamale_dpdBlock_Init(0)) return WI_SUCCESS;
	else return WI_ERROR;
}
// end of Tamale_TX_DPD_Init()

// ================================================================================================
// FUNCTION : Tamale_TX_DPD_Close()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_TX_DPD_Close(void)
{
	if (Tamale_dpdBlock_Close(0)) return WI_SUCCESS;
	else return WI_ERROR;
}
// end of Tamale_TX_DPD_Close()

// ================================================================================================
// ================================================================================================
static int Tamale_wiDataRateDecoder(int codeword, int TxMode, int rgf_Tx_Rate_OFDM_HT, int rgf_Tx_Rate_OFDM_LL, int rgf_Tx_Rate_OFDM_LM)
{
	/*
	int DataRate;
...
// ================================================================================================
// ================================================================================================
static int Tamale_dpdgetFilterBank(int *dpd_state)
{
	int HighRate;
	int TxMode;
	int dpd_pa_tmp;
	...
	HighRate = Tamale_wiDataRateDecoder(pTX->RATE.word, TxMode, rgf_Tx_Rate_OFDM_HT, rgf_Tx_Rate_OFDM_LL, rgf_Tx_Rate_OFDM_LM);
