...
//=================================================================================================
//
//  Tamale - Baseband Receiver Model
...
// ================================================================================================
// FUNCTION  : Tamale_RX_ClockGenerator()
// ------------------------------------------------------------------------------------------------
...
//             modules. Clock edge indicators are stored in the Tamale RX state.
//
...
void Tamale_RX_ClockGenerator(Tamale_RxState_t *pRX, wiBoolean Reset)
{
    ...
    if (Reset)
    {
        ...
        for (k=0; k<InitialPhase; k++) // advance clock generator specified periods
            Tamale_RX_ClockGenerator(pRX, 0);
...
// end of Tamale_RX_ClockGenerator()

// ================================================================================================
// FUNCTION : Tamale_RX_Init()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
...
// end of Tamale_RX_Init()

// ================================================================================================
// FUNCTION  : Tamale_RX_Restart()
// ------------------------------------------------------------------------------------------------
...
wiStatus Tamale_RX_Restart(void)
{
    ...
    Tamale_RxState_t  *pRX  = Tamale_RxState();
    Tamale_bRxState_t *pRXb = Tamale_bRxState();

    // Call the RX1 module local restart
    //
    XSTATUS( Tamale_RX1_Restart() );
...
// end of Tamale_RX_Restart()

// ================================================================================================
// FUNCTION  : Tamale_RX_EndOfWaveform()
// ------------------------------------------------------------------------------------------------
...
wiStatus Tamale_RX_EndOfWaveform(void)
{
    ...
    //
    Tamale_DCX(-2, Zero, NULL, 0, Tamale_RxState(), Tamale_Registers()); // Load values from DCX accumulators

    return WI_SUCCESS;
}
// end of Tamale_RX_EndOfWaveform()

// ================================================================================================
// FUNCTION : Tamale_RX_FreeMemory()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
...
// end of Tamale_RX_FreeMemory()

// ================================================================================================
// FUNCTION : Tamale_RX_AllocateMemory()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_RX_AllocateMemory(void)
{
    ...
    int N20 = pRX->N20 = N80/4 + 2 + (TAMALE_FFT_DELAY + TAMALE_PLL_DELAY);
    int N44 =           (N80*11/10)/2+1;
    int N22 = pRX->N22 = N44/2 + 1;

    Tamale_RX_FreeMemory(pRX, 0);

    pRX->PHY_RX_D = (wiUWORD *)wiCalloc(65536 + 8,        sizeof(wiUWORD));
    pRX->cSP      = (wiWORD  *)wiCalloc(TAMALE_MAX_N_SYM, sizeof(wiWORD));
    pRX->cCP      = (wiWORD  *)wiCalloc(TAMALE_MAX_N_SYM, sizeof(wiWORD));
    pRX->kFFT     = (wiInt   *)wiCalloc(TAMALE_MAX_N_SYM, sizeof(wiInt));
    ...
    if (pRX->EnableTrace)
    {
        ...
        pRX->aSigOut       = (Tamale_aSigState_t        *)wiCalloc(N20, sizeof(Tamale_aSigState_t        ));
        pRX->bSigOut       = (Tamale_bSigState_t        *)wiCalloc(N22, sizeof(Tamale_bSigState_t        ));
        pRX->traceRxClock  = (Tamale_traceRxClockType   *)wiCalloc(N80, sizeof(Tamale_traceRxClockType   ));
        pRX->traceRxState  = (Tamale_traceRxStateType   *)wiCalloc(N80, sizeof(Tamale_traceRxStateType   ));
        pRX->traceRxControl= (Tamale_traceRxControlType *)wiCalloc(N80, sizeof(Tamale_traceRxControlType ));
        pRX->traceRadioIO  = (Tamale_RadioDigitalIO_t   *)wiCalloc(N80, sizeof(Tamale_RadioDigitalIO_t   ));
        pRX->traceDataConv = (Tamale_DataConvCtrlState_t*)wiCalloc(N80, sizeof(Tamale_DataConvCtrlState_t));
        pRX->traceDFE      = (Tamale_traceDFEType       *)wiCalloc(N20, sizeof(Tamale_traceDFEType       ));
        pRX->traceAGC      = (Tamale_traceAGCType       *)wiCalloc(N20, sizeof(Tamale_traceAGCType       ));
        pRX->traceRSSI     = (Tamale_traceRSSIType      *)wiCalloc(N20, sizeof(Tamale_traceRSSIType      ));
        pRX->traceDFS      = (Tamale_traceDFSType       *)wiCalloc(N20, sizeof(Tamale_traceDFSType       ));
        pRX->traceSigDet   = (Tamale_traceSigDetType    *)wiCalloc(N20, sizeof(Tamale_traceSigDetType    ));
        pRX->traceFrameSync= (Tamale_traceFrameSyncType *)wiCalloc(N20, sizeof(Tamale_traceFrameSyncType ));
        pRX->traceRX2      = (Tamale_traceRX2Type       *)wiCalloc(N20, sizeof(Tamale_traceRX2Type       ));
        pRX->traceRx       = (Tamale_traceRxType        *)wiCalloc(N80, sizeof(Tamale_traceRxType        ));
        pRX->traceImgDet   = (Tamale_traceImgDetType    *)wiCalloc(N20, sizeof(Tamale_traceImgDetType    ));
        pRX->traceIQCal    = (Tamale_traceIQCalType     *)wiCalloc(N20, sizeof(Tamale_traceIQCalType     ));
        ...
        {
            wiUtil_WriteLogFile("Tamale_RX_AllocateMemory: N80 = %u\n",N80);
            return STATUS(WI_ERROR_MEMORY_ALLOCATION);
        }
    }
    /////////////////////////////////////////////////////////////////////////////////////
    //
    //  Allocate RX memory in the Tamale_b module
    //
    XSTATUS( Tamale_bRX_AllocateMemory() );

    return WI_SUCCESS;
}
// end of Tamale_RX_AllocateMemory()

// ================================================================================================
// FUNCTION : Tamale_RX_NuisanceFilter()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_RX_NuisanceFilter( wiBoolean CheckNuisance, wiBoolean MAC_NOFILTER, 
                               Tamale_CtrlState_t *SigOut, Tamale_RxState_t *pRX, TamaleRegisters_t *pReg)
{
...
// end of Tamale_RX_NuisanceFilter

// ================================================================================================
// FUNCTION : Tamale_RX_Header()
// ------------------------------------------------------------------------------------------------
// Purpose  : Construct PHY header for RX packets
// ================================================================================================
wiStatus Tamale_RX_Header(Tamale_CtrlState_t outReg, wiUInt AntSel, wiUWORD RSSI0, wiUWORD RSSI1)
{
    wiUWORD RATE, LENGTH;
...
// end of Tamale_RX_Header()


// ================================================================================================
// FUNCTION : Tamale_RX_ERROR()
// ------------------------------------------------------------------------------------------------
// Purpose  : Generate the PHY_RX_ERROR signal (for unsupported/invalid packets)
// ================================================================================================
void Tamale_RX_ERROR( Tamale_CtrlState_t *inReg, Tamale_CtrlState_t outReg, wiUWORD PktTime)
{
    TamaleRegisters_t *pReg = Tamale_Registers();
    Tamale_RxState_t *pRX = Tamale_RxState();
...
// end of Tamale_RX_ERROR()


// ================================================================================================
// FUNCTION : Tamale_RX_CCA()
// ------------------------------------------------------------------------------------------------
// Purpose  : Generate PHY_CCA
// ================================================================================================
void Tamale_RX_CCA( Tamale_CtrlState_t *inReg, Tamale_CtrlState_t outReg, wiUWORD PktTime, 
                    wiUWORD RSSI, wiBoolean PHY_RX_EN, wiBoolean ValidRSSI )
{
    ...
    TamaleRegisters_t *pReg = Tamale_Registers();
    Tamale_RX_CCA_t *pState = &(Tamale_RxState()->CCA);
...
//            accurate timing. For Tamale, this module contains controls from the top
                    ...
                    Tamale_CtrlState_t *inReg, Tamale_CtrlState_t outReg )
{
    ...
    Tamale_RxState_t  *pRX  = Tamale_RxState();
    TamaleRegisters_t *pReg = Tamale_Registers();
    Tamale_RxTop_t    *p    = &(pRX->RxTop);

    Tamale_aSigState_t aSigOut, *aSigIn = &(p->aSigIn);
    Tamale_bSigState_t bSigOut, *bSigIn = &(p->bSigIn);
    ...
    if (pRX->clock.posedgeCLK80MHz)
    {
        ...
        if (pRX->TX.Enabled)
        {
            ...
            if (p->MAC_TX_EN && !dMAC_TX_EN)
                Tamale_DvState()->LastTxStart = pRX->k80; // record the last assertion of MAC_TX_EN
        }
        ...
        switch (outReg.State)
        {
            ...
            case 12: // RX: Valid Header ----------------------------------------------------------
            {
                ...
                //
                p->PktTime.D.word = outReg.TurnOnCCK ? Tamale_bRxState()->bPacketDuration.w16 : pRX->aPacketDuration.w16;
                ...
                aSigIn->HeaderValid = 0;
                    ...
                    Tamale_RX_Header(outReg, AntSel, p->RSSI0, p->RSSI1); 
                };
                break;
            }
            ...
            case 22: // RX: Mixed-Mode HT-SIG Decoding --------------------------------------------
                ...
                if (HeaderValid)
                {
                    ...
                    if (!Unsupported) Tamale_RX_Header(outReg, AntSel, p->RSSI0, p->RSSI1);
                }
                break;            
    ...
    if (pRX->clock.posedgeCLK40MHz)
    {
        ...
        // ---------------------
        Tamale_RX1( p->DFEEnB, p->d2LgSigAFE, &p->LNAGainOFDM, &p->AGainOFDM, &p->RSSI0OFDM, &p->RSSI1OFDM,
        ...
        {
            XSTATUS( Tamale_RX2(aSigIn, aSigOut, outReg, pRX, pReg) );
        }
    }
    //////////////////////////////////////////////////////////////////////////////////////////
    //
    ...
    if ((pRX->clock.posedgeCLK40MHz || pRX->clock.posedgeCLK44MHz) && pReg->RXMode.b1)
    {
        ...
        Tamale_bRX_top( pRX, pRXb, pReg, outReg.bRXEnB, &(p->bInR.Q), &(p->bInI.Q), 
    ...
    //  from the MAC.
    //
    Tamale_RX_NuisanceFilter(outReg.State==13, pRX->MAC_NOFILTER, inReg, pRX, pReg);
    ...
    //  TX/RX MAC/PHY interfaces which are implied by operations in Tamale_TX and Tamale_RX.
    //
    ...
    {     
        ...
        //
        Tamale_RX_ERROR( inReg, outReg, p->PktTime.Q );
        Tamale_RX_CCA  ( inReg, outReg, p->PktTime.Q, p->RSSI, p->PHY_RX_EN, aSigOut.ValidRSSI );
        ...
        //
        #define TAMALE_SELECTSIGNAL( SelectSignal, Signal )                                                 \
        {                                                                                                   \
            switch (SelectSignal)                                                                           \
            {                                                                                               \
        ...
        } // end of TAMALE_SELECTSIGNAL() //////////////////////////////////////////////////////////////////

        TAMALE_SELECTSIGNAL(pReg->SelectSignal1.w4, p->Signal1.D.word);
        TAMALE_SELECTSIGNAL(pReg->SelectSignal2.w4, p->Signal2.D.word);
