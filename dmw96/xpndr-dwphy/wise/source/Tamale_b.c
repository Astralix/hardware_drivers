...
// ================================================================================================
// FUNCTION : Tamale_b_Init()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_b_Init()
{
	Tamale_LookupTable_t *pLut = Tamale_LookupTable();

    Tamale_bRxState()->N44  = 0;
...
wiStatus Tamale_bTX_AllocateMemory(int Na, int Nc, int M)
{
    ...
    {
        pTX->LastAllocation.Fn      = (wiFunction)Tamale_bTX_AllocateMemory;
        pTX->LastAllocation.Size[0] = Na;
        pTX->LastAllocation.Size[1] = Nc;
        pTX->LastAllocation.Size[2] = M;
        pTX->LastAllocation.Size[3] = 0;
...
// end of bTamale_bTX_AllocateMemory()

// ================================================================================================
// FUNCTION : Tamale_bRX_AllocateMemory()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_bRX_AllocateMemory(void)
{
    Tamale_bRxState_t *pbRX = Tamale_bRxState();
    int N80 = (pbRX->N44 * 80/44) + 2; // +2 to cover rounding errors

    Tamale_bRX_FreeMemory(pbRX, 0);
    ...
    if (Tamale_RxState()->EnableTrace)
    {
        pbRX->traceCCK     = (Tamale_b_traceCCKType     *)wiCalloc(pbRX->N22, sizeof(Tamale_b_traceCCKType)     );
        pbRX->traceState   = (Tamale_b_traceStateType   *)wiCalloc(pbRX->N22, sizeof(Tamale_b_traceStateType)   );
        pbRX->traceControl = (Tamale_b_traceControlType *)wiCalloc(pbRX->N22, sizeof(Tamale_b_traceControlType) );
        pbRX->traceFrontReg= (Tamale_b_FrontCtrlRegType *)wiCalloc(pbRX->N22, sizeof(Tamale_b_FrontCtrlRegType) );
        pbRX->traceDPLL    = (Tamale_b_traceDPLLType    *)wiCalloc(pbRX->N22, sizeof(Tamale_b_traceDPLLType)    );
        pbRX->traceBarker  = (Tamale_b_traceBarkerType  *)wiCalloc(pbRX->N22, sizeof(Tamale_b_traceBarkerType)  );
        pbRX->traceDSSS    = (Tamale_b_traceDSSSType    *)wiCalloc(pbRX->N22, sizeof(Tamale_b_traceDSSSType)    );
        pbRX->traceCCKInput= (Tamale_b_traceCCKFBFType  *)wiCalloc( 65536,  sizeof(Tamale_b_traceCCKFBFType)  );
        pbRX->traceCCKTVMF = (Tamale_b_traceCCKTVMFType *)wiCalloc( 65536,  sizeof(Tamale_b_traceCCKTVMFType) );
        pbRX->traceCCKFWT  = (Tamale_b_traceCCKFWTType  *)wiCalloc(262144,  sizeof(Tamale_b_traceCCKTVMFType) );
        pbRX->traceResample= (Tamale_b_traceResampleType*)wiCalloc(N80,     sizeof(Tamale_b_traceResampleType));
        ...
        {
            wiUtil_WriteLogFile("Tamale_bRX_AllocateMemory: N44 = %u, N80 = %u\n",pbRX->N44, N80);
            return STATUS(WI_ERROR_MEMORY_ALLOCATION);
        }
    }
    // ---------------------------------
...
// end of Tamale_bRX_AllocateMemory()

// ================================================================================================
// FUNCTION : Tamale_bRX_FreeMemory()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_bRX_FreeMemory(Tamale_bRxState_t *pbRX, wiBoolean ClearPointerOnly)
{
    #define FREE(ptr)                                        \
    {                                                        \
...
// end of Tamale_bRX_FreeMemory()

// ================================================================================================
// FUNCTION  : Tamale_bTX_DataGenerator()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bTX_DataGenerator(wiUWORD RATE, int NumByte, wiUWORD *PSDU, wiUWORD *a, wiBoolean ModSel)
{
    int b,i,k,n,r;
    Tamale_TxState_t *pTX = Tamale_TxState();
...
// end of Tamale_bTX_DataGenerator()

// ================================================================================================
// FUNCTION  : Tamale_bTX_Scrambler()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bTX_Scrambler(wiUWORD RATE, int Na, wiUWORD *a, wiUWORD *b)
{
    int k; wiUWORD X;
...
// end of Tamale_bTX_Scrambler()

// ================================================================================================
// FUNCTION  : Tamale_bTX_DBPSK()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bTX_DBPSK(int N, wiUWORD *b, wiUWORD *cReal, wiUWORD *cImag, wiUWORD *pp)
{
...
// end of Tamale_bTX_DBPSK()

// ================================================================================================
// FUNCTION  : Tamale_b_DQPSK_Encoding()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
__inline int Tamale_b_DQPSK_Encoding(wiUWORD b0, wiUWORD b1)
{
...
//             pp    -- phase of previous symbol in quadrant
// ================================================================================================
void Tamale_bTX_DQPSK(int N, wiUWORD *b, wiUWORD *cReal, wiUWORD *cImag, wiUWORD *pp)
{
    const wiUInt Barker[11] = {0,1,0,0,1,0,0,0,1,1,1}; // Barker sequence in multiples of pi
    ...
    {
        pp->w2 += Tamale_b_DQPSK_Encoding(b[n], b[n+1]); // differential phase modulation
...
// end of Tamale_bTX_DQPSK()

// ================================================================================================
// FUNCTION  : Tamale_bTX_CCK()
// ------------------------------------------------------------------------------------------------
...
//             pp    -- phase of previous symbol in quadrant
// ================================================================================================
void Tamale_bTX_CCK(wiUWORD p1, wiUWORD p2, wiUWORD p3, wiUWORD p4, wiUWORD *cReal, wiUWORD *cImag)
{
...
// end of Tamale_bTX_CCK()

// ================================================================================================
// FUNCTION  : Tamale_bTX_CCK55()
// ------------------------------------------------------------------------------------------------
...
//             pp    -- phase of previous symbol in quadrant
// ================================================================================================
void Tamale_bTX_CCK55(int N, wiUWORD *b, wiUWORD *cReal, wiUWORD *cImag, wiUWORD *pp)
{
    int i;            // symbol index
    int k;            // sample index
    int n;            // bit index
    ...
    {
        pp->w2+= Tamale_b_DQPSK_Encoding(b[n], b[n+1]); // DQPSK for common phase
        ...
        Tamale_bTX_CCK(*pp, p2, p3, p4, cReal+k, cImag+k);
    }
}
// end of Tamale_bTX_CCK55

// ================================================================================================
// FUNCTION  : Tamale_bTX_CCK110()
// ------------------------------------------------------------------------------------------------
...
//             pp    -- phase of previous symbol in quadrant
// ================================================================================================
void Tamale_bTX_CCK110(int N, wiUWORD *b, wiUWORD *cReal, wiUWORD *cImag, wiUWORD *pp)
{
    int i;            // symbol index
    int k;            // sample index
    int n;            // bit index
...
// end of Tamale_bTX_CCK110

// ================================================================================================
// FUNCTION  : Tamale_bTX_Modulation
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bTX_Modulation(wiUWORD RATE, int NumByte, wiUWORD *b, wiUWORD *cReal, wiUWORD *cImag)
{
    ...
    // Modulation: Preamble
    // ---------------------
    Tamale_bTX_DBPSK((RATE.b2? 72:144), b, cReal, cImag, &phase);
    b     +=     RATE.b2? 72:144;
    ...
    // Modulation: Header
    // -------------------
    if (RATE.b2) Tamale_bTX_DQPSK(24, b, cReal, cImag, &phase); // short preamble
    else         Tamale_bTX_DBPSK(48, b, cReal, cImag, &phase); // long preamble
    ...
    switch (RATE.w2)
    {
        case 0: Tamale_bTX_DBPSK (8*NumByte,   b, cReal, cImag, &phase); break;
        case 1: Tamale_bTX_DQPSK (8*NumByte/2, b, cReal, cImag, &phase); break;
        case 2: Tamale_bTX_CCK55 (8*NumByte/4, b, cReal, cImag, &phase); break;
        case 3: Tamale_bTX_CCK110(8*NumByte/8, b, cReal, cImag, &phase); break;
    }
}
// end of Tamale_bTX_Modulation()

// ================================================================================================
// FUNCTION  : Tamale_bTX_PulseFilter_FIR
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bTX_PulseFilter_FIR(int Nc, wiUWORD *c, wiWORD *x)
{
...
// end of Tamale_bTX_PulseFilter_FIR()

// ================================================================================================
// FUNCTION  : Tamale_bTX_PulseFilter
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bTX_PulseFilter(int Nc, wiUWORD *cReal, wiUWORD *cImag, wiWORD *xReal, wiWORD *xImag)
{
    Tamale_bTX_PulseFilter_FIR(Nc, cReal, xReal);
    Tamale_bTX_PulseFilter_FIR(Nc, cImag, xImag);
}
// end of Tamale_bTX_PulseFilter()

// ================================================================================================
// FUNCTION  : Tamale_bTX_Resample_Core
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bTX_Resample_Core(int N, wiWORD *x, wiWORD *y, int bResamplePhase)
{
...
// end of Tamale_bTX_Resample_Core()

// ================================================================================================
// FUNCTION  : Tamale_bTX_Resample
// ------------------------------------------------------------------------------------------------
// Purpose   : Transmit signal sample rate conversion from 44 to 40 MS/s
// Parameters: Nc    -- Number of input samples
...
// ================================================================================================
void Tamale_bTX_Resample(int N, wiWORD *xReal, wiWORD *xImag, wiWORD *yReal, wiWORD *yImag, wiInt bResamplePhase)
{
    ...
    Tamale_bTX_Resample_Core(N, xReal, yReal, bResamplePhase);
    Tamale_bTX_Resample_Core(N, xImag, yImag, bResamplePhase);
}
// end of Tamale_bTX_Resample()

// ================================================================================================
// FUNCTION  : Tamale_bTX_UpsampleFIR
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bTX_UpsampleFIR(int Ny, wiWORD *x, wiWORD *y)
{
...
// end of Tamale_bTX_UpsampleFIR()

// ================================================================================================
// FUNCTION  : Tamale_bTX_Upsample
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bTX_Upsample(int Nx, wiWORD *xReal, wiWORD *xImag, wiWORD *yReal, wiWORD *yImag)
{
    Tamale_bTX_UpsampleFIR(Nx, xReal, yReal);
    Tamale_bTX_UpsampleFIR(Nx, xImag, yImag);
}
// end of Tamale_bTX_Upsample()
...
// ================================================================================================
wiStatus Tamale_bTX(wiUWORD RATE, wiUWORD LENGTH, wiUWORD *MAC_TX_D, int *Nx, wiComplex **x, int M) 
{
    int NsPSDU, Na, Nc, dv;
    ...
    XSTATUS(Tamale_bTX_AllocateMemory(Na, Nc, M) );

    // ---------------------------------------------------------------
    // Generate bit sequence from PSDU bytes, preamble type, data rate
    // ---------------------------------------------------------------
    Tamale_bTX_DataGenerator(RATE, LENGTH.w12, MAC_TX_D, pTX->a, pTX->SetPBCC);
    Tamale_bTX_Scrambler    (RATE, Na, pTX->a, pTX->b);
    Tamale_bTX_Modulation   (RATE, LENGTH.w12, pTX->b, pTX->cReal, pTX->cImag);
    ...
    Tamale_bTX_PulseFilter(Nc, pTX->cReal, pTX->cImag, pTX->vReal+dv, pTX->vImag+dv);

    // -------------------
    // Resample and Output
    // -------------------
    Tamale_bTX_Resample(pTX->Nv, pTX->vReal, pTX->vImag, pTX->xReal, pTX->xImag, pTX->bResamplePhase);
    Tamale_bTX_Upsample(pTX->Ny, pTX->xReal, pTX->xImag, pTX->yReal, pTX->yImag);
    Tamale_Upmix       (pTX->Ny, pTX->yReal, pTX->yImag, pTX->zReal, pTX->zImag);
...
// ================================================================================================
// FUNCTION  : Tamale_bTX_ResampleToDAC()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
wiStatus Tamale_bTX_ResampleToDAC(void)
{
    Tamale_TxState_t  *pTX  = Tamale_TxState();
...
// end of Tamale_bTX_ResampleToDAC()

// ================================================================================================
// FUNCTION  : Tamale_bRX_Filter
// ------------------------------------------------------------------------------------------------
...
//             y -- output, 11 bit
// ================================================================================================
void Tamale_bRX_Filter(int n, wiWORD *x, wiWORD *y)
{
    const int h[14]={-1,-2,-2, 1, 6, 12, 16, 16, 12, 6, 1,-2,-2,-1};

    wiWORD *d = &(Tamale_bRxState()->dRxFilter[n][0]);
    int i;
...
// end of Tamale_bRX_Filter()

// ================================================================================================
// FUNCTION  : Tamale_bRX_DigitalGain
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bRX_DigitalGain(wiWORD *qReal, wiWORD *qImag, wiWORD *xReal, wiWORD *xImag, wiWORD DGain)
{
...
// end of Tamale_bRX_DigitalGain

// ================================================================================================
// FUNCTION  : Tamale_bRX_Interpolate()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bRX_Interpolate(wiWORD *LPFOutR, wiWORD *LPFOutI, wiWORD *IntpOutR, wiWORD *IntpOutI,
                            wiWORD cSP, wiBoolean PLLEnB, wiBoolean TransCCK, wiBoolean INTERPinit,
                            wiBoolean *IntpFreeze, Tamale_RxState_t *pRX)
{
    const wiUInt LUT32div11[16] = {0, 3, 6, 9, 12, 15, 17, 20, 23, 26, 29}; // = round(index*32/11)

    Tamale_bRX_Interpolate_t *pState = &(Tamale_bRxState()->Interpolate);
...
// end of Tamale_bRX_Interpolate()

// ================================================================================================
// FUNCTION  : Tamale_b_BarkerCorrelator()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_b_BarkerCorrelator( wiWORD *BCInR22, wiWORD *BCInI22, wiWORD *BCInR11, wiWORD *BCInI11,
                                wiWORD *BCOutR, wiWORD *BCOutI, int BCMode, int BCEnB )
{
    const int Barker[11] = {0,0,0,1,1,1,0,1,1,0,1}; // Time-reversed Barker sequence

    Tamale_bBarkerCorrelator_t *pState = &(Tamale_bRxState()->BarkerCorrelator);
    wiWORD *dR = pState->dR;
    wiWORD *dI = pState->dI;
...
// end of Tamale_b_BarkerCorrelator()

// ================================================================================================
// FUNCTION  : Tamale_b_EnergyDetect()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_b_EnergyDetect(wiWORD *EDInR, wiWORD *EDInI, int EDEnB, wiUWORD EDwindow, wiUReg *EDOut)
{
    Tamale_bEnergyDetect_t *pState = &(Tamale_bRxState()->EnergyDetect);
...
// end of Tamale_b_EnergyDetect()

// ================================================================================================
// FUNCTION  : Tamale_b_CorrelationQuality()
// ------------------------------------------------------------------------------------------------
// Purpose   : 
...
// ================================================================================================
void Tamale_b_CorrelationQuality( wiWORD BCOutR, wiWORD BCOutI, wiBoolean CQEnB, wiBoolean CSEnB, 
                                  wiUWORD CQwindow, wiUWORD SSwindow, wiUReg *CQOut, wiUReg *CQPeak )
{
    Tamale_bCorrelationQuality_t *pState = &(Tamale_bRxState()->CorrelationQuality);
...
// end of Tamale_b_CorrelationQuality()

// ================================================================================================
// FUNCTION  : Tamale_b_AGC()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_b_AGC( wiBoolean AGCEnB, wiBoolean AGCInitGains, wiUWORD EDOut, wiBoolean AGCbounded, 
                   wiBoolean LgSigAFE, wiUReg *LNAGain, wiUReg *AGain, wiReg *DGain, wiUReg *LargeSignal, 
                   wiUReg *GainUpdate, wiUWORD *AGCAbsEnergy, wiUReg *AGCAbsPwr, TamaleRegisters_t *pReg )
{
    Tamale_bAGC_t *pState = &(Tamale_bRxState()->AGC);
...
// ================================================================================================
void Tamale_b_EstimateCFO( wiBoolean CFOeEnB, wiBoolean CFOeGet, wiWORD BCOutR, wiWORD BCOutI, 
                           wiReg *CFO, TamaleRegisters_t *pReg )
{
    ...
    Tamale_bEstimateCFO_t *pState = &(Tamale_bRxState()->EstimateCFO);
    ...
    if (CFOeEnB == 0)
    {
        ...
        if (CFOeGet)
        {
            ...
            //
            if (Tamale_RxState()->NonRTL.Enabled && Tamale_RxState()->NonRTL.DisableCFO)
                CFO->D.word = 0;
        }
        else
...
// end of Tamale_b_EstimateCFO()

// ================================================================================================
// FUNCTION  : Tamale_b_CorrectCFO()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_b_CorrectCFO( wiBoolean CFOcEnB, wiBoolean TransCCK, wiBitDelayLine dDeciFreeze, wiWORD CFO, wiWORD cCP, 
                          wiWORD CFOcInR, wiWORD CFOcInI, wiWORD *CFOcOutR, wiWORD *CFOcOutI, wiBoolean posedgeCLK11MHz )
{
    ...
    Tamale_bCorrectCFO_t *pState = &(Tamale_bRxState()->CorrectCFO);
...
//end of Tamale_b_CorrectCFO()

// ================================================================================================
// FUNCTION  : Tamale_b_ChanEst()
// ------------------------------------------------------------------------------------------------
...
//             RX         -- bTamale RX data structure
//             pReg       -- configuration registers
// ================================================================================================
void Tamale_b_ChanEst( wiBoolean CEEnB, wiBoolean CEMode, wiBoolean GetCE, wiWORD BCOutR, wiWORD BCOutI,
                       wiUWORD DMOut, wiBoolean DMOutValid, wiBoolean DBPSK, wiBoolean *CEDone,
                       Tamale_bRxState_t *RX, TamaleRegisters_t *pReg )
{
    Tamale_bChanEst_t *pState = &(Tamale_bRxState()->ChanEst);
    ...
    if (!CEEnB)
    {
        ...
        if (pState->count>32 || pState->SymBufCount>2) { // check for buffer overrun (model only)
            wiPrintf("*** Tamale_b_ChanEst ERROR: count=%d, SymBufCount=%d\n",pState->count,pState->SymBufCount);
...
// end of Tamale_b_ChanEst()

// ================================================================================================
// FUNCTION  : Tamale_bRX_SP()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bRX_SP(wiWORD *uReal, wiWORD *uImag, wiWORD **wRptr, wiWORD **wIptr, int Np2, 
                   wiBoolean SPEnB, wiBoolean TransCCK, wiBitDelayLine dDeciFreeze)
{
    Tamale_bRxState_t *pbRX = Tamale_bRxState();
    Tamale_bRX_SP_t   *pState = &(pbRX->RX_SP);
...
// end of Tamale_bRX_SP()

// ================================================================================================
// FUNCTION  : Tamale_bRX_CMF()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bRX_CMF( wiBoolean CMFEnB, wiBitDelayLine DMSym, wiBitDelayLine dDeciFreeze, 
                     wiWORD *uReal, wiWORD *uImag, wiWORD **SSMatOutR, wiWORD **SSMatOutI, 
                     int Nh, wiWORD *hReal, wiWORD *hImag )
{
    Tamale_bRX_CMF_t *pState = &(Tamale_bRxState()->CMF);
...
// end of Tamale_bRX_CMF()

// ================================================================================================
// FUNCTION  : Tamale_bRX_DSSS()
// ------------------------------------------------------------------------------------------------
...
//
// ================================================================================================
void Tamale_bRX_DSSS( wiBoolean DMEnB, wiWORD *CFOOutR, wiWORD *CFOOutI, wiWORD *CMFOutR, wiWORD *CMFOutI,
                      ...
                      wiWORD *BFReal, wiWORD *BFImag )
{
    ...
    Tamale_bRX_DSSS_t *pState = &(Tamale_bRxState()->RX_DSSS);
...
// end of Tamale_bRX_DSSS()

// ================================================================================================
// FUNCTION  : Tamale_bRX_CCK()
// ------------------------------------------------------------------------------------------------
// Purpose   : CCK Demodulator
// Parameters: 
// ================================================================================================
void Tamale_bRX_CCK(wiWORD *wR, wiWORD *wI,
                    ...
                    )
{
    ...
    Tamale_bRxState_t *pbRX = Tamale_bRxState();
    Tamale_bRX_CCK_t  *pState = &(pbRX->RX_CCK);
...
// end of Tamale_bRX_CCK()

// ================================================================================================
// FUNCTION  : Tamale_bRX_SFD()
// ------------------------------------------------------------------------------------------------
...
//             SFDone      -- SFD found
// ================================================================================================
void Tamale_bRX_SFD(wiBoolean SFDEnB, wiUWORD DesOut, wiBoolean DesOutValid, 
                    wiUReg *Preamble, wiUReg *SFDDone)
{
    ...
    Tamale_bRX_SFD_t *pState = &(Tamale_bRxState()->RX_SFD);
...
// end of Tamale_bRX_SFD()

// ================================================================================================
// FUNCTION  : Tamale_bRX_Header()
// ------------------------------------------------------------------------------------------------
...
//             Modulation    -- modulation type: {0=CCK, 1=PBCC-not supported by Tamale}
...
// ================================================================================================
void Tamale_bRX_Header(wiBoolean HDREnB, wiBoolean DBPSK, wiBoolean shortPreamble, wiBoolean SSOutValid, 
                       wiUWORD *a, wiBitDelayLine *HDRDone, Tamale_bRxState_t *RX)
{
    ...
    else
    {
        ...
        if (*pNbit == 48)
        {
            ...
            Tamale_Registers()->RX_SERVICE.word = RX->bSERVICE.w8;
            ...
            //
            //  The conditions for CRCpass are different in the Tamale baseband. Here the
...
// end of Tamale_bRX_Header()

// ================================================================================================
// FUNCTION  : Tamale_bRX_PSDU()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bRX_PSDU( wiBoolean PSDUEnB, wiBoolean DesOutValid, wiBitDelayLine *PSDUDone, 
                      Tamale_RxState_t *RX)
{
    wiUWORD *pBitCount = &(Tamale_bRxState()->RX_PSDU.BitCount);
...
// end of Tamale_bRX_PSDU()

// ================================================================================================
// FUNCTION  : Tamale_bRX_DPLL()
// ------------------------------------------------------------------------------------------------
// Purpose   : 
// Parameters: PLLEnB
//             PSDUEnB
...
//             DMSym and cckSym.
// ================================================================================================
void Tamale_bRX_DPLL(wiBoolean PLLEnB, wiWORD *SSMatOutR, wiWORD *SSMatOutI, wiBitDelayLine SSCorOutValid, 
                     ...
                     wiWORD *cCP, wiWORD *cSP, TamaleRegisters_t *pReg)
{
    Tamale_bRX_DPLL_t *pState = &(Tamale_bRxState()->RX_DPLL);
...
// end of Tamale_bRX_DPLL()

// ================================================================================================
// FUNCTION  : Tamale_bRX_Descrambler()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_bRX_Descrambler(wiUWORD SSOut, wiBoolean SSOutValid, wiUWORD cckOut, wiBoolean cckOutValid,
                            wiBoolean DBPSK, wiUReg *DesOut, wiUReg *DesOutValid, Tamale_bRxState_t *RX, wiUInt FrontState)
{
...
// end of Tamale_bRX_Descrambler()

// ================================================================================================
// FUNCTION  : Tamale_b_Downsample()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Tamale_b_Downsample( wiBoolean posedgeCLK22MHz, wiBoolean posedgeCLK11MHz, wiUInt DeciOutEnB,
                          ...
                          wiUReg *DeciFreeze)
{
    Tamale_b_Downsample_t *pState = &(Tamale_bRxState()->Downsample);
...
// end of Tamale_b_Downsample()

// ================================================================================================
// FUNCTION  : Tamale_bRX_top()
// ------------------------------------------------------------------------------------------------
...
//             Run11b      --
// ================================================================================================
void Tamale_bRX_top( Tamale_RxState_t *pRX, Tamale_bRxState_t *RX, TamaleRegisters_t *Reg, 
                     ...
                     Tamale_bSigState_t *bSigOut, Tamale_bSigState_t bSigIn)
{
    Tamale_bRxState_t *pbRX = Tamale_bRxState();
    Tamale_bRxTop_t   *pRxTop = &(pbRX->RxTop);

    Tamale_b_FrontCtrlRegType *inReg = &(pRxTop->inReg);
    Tamale_b_FrontCtrlRegType outReg;
    ...
    if (posedgeCLK22MHz)
    {
        ...
        if (bRXEnB)
        {
            ...
            if (!Tamale_RxState()->EnableTrace) return; // skip subsequent processing to speed up simulation
        }
    }
    ...
    if (posedgeCLK40MHz)
    {
        Tamale_bRX_Filter( 0, qReal, RX->xReal + k40 ); // filter #0: real
        Tamale_bRX_Filter( 1, qImag, RX->xImag + k40 ); // filter #1: imag
        Tamale_bRX_DigitalGain( RX->xReal+k40, RX->xImag+k40, RX->yReal+k40, RX->yImag+k40, pRxTop->DGain.Q );
    }
    if (posedgeCLK40MHz || posedgeCLK44MHz)
    {
        Tamale_bRX_Interpolate( RX->yReal+k40, RX->yImag+k40, RX->sReal+k44, RX->sImag+k44, pRxTop->cSP.Q, 
    ...
    if (posedgeCLK44MHz)
    {
        Tamale_b_AGC( outReg.AGCEnB, outReg.AGCInit, pRxTop->EDOut.Q, outReg.AGCbounded, 
        ...
        Tamale_b_Downsample( posedgeCLK22MHz, posedgeCLK11MHz, inReg->CFOcEnB, pRxTop->IntpFreeze, 
			                 outReg.SymPhase, RX->sReal[k44], RX->sImag[k44],
                             RX->zReal+k22, RX->zImag+k22, RX->vReal+k11, RX->vImag+k11, 
							 &pRxTop->DeciFreeze );

        Tamale_bRX_DPLL( outReg.PLLEnB, pRxTop->SSMatOutR, pRxTop->SSMatOutI, pRxTop->posedge_SSCorOutValid, 
    ...
    if (posedgeCLK22MHz)
    {
        Tamale_b_CorrectCFO( outReg.CFOcEnB, outReg.TransCCK, pRxTop->dDeciFreeze, pRxTop->CFO.Q, pRxTop->cCP, 
			                 RX->vReal[k11], RX->vImag[k11], RX->uReal+k11, RX->uImag+k11, posedgeCLK11MHz );

        Tamale_b_BarkerCorrelator( RX->zReal+k22, RX->zImag+k22, RX->uReal+k11, RX->uImag+k11,
                                   RX->rReal+k22, RX->rImag+k22, outReg.BCMode, outReg.BCEnB );

        Tamale_b_EnergyDetect( RX->zReal+k22, RX->zImag+k22, outReg.EDEnB, Reg->EDwindow, &pRxTop->EDOut );

        Tamale_b_CorrelationQuality( RX->rReal[k22], RX->rImag[k22], outReg.CQEnB, outReg.CSEnB, 
			                         Reg->CQwindow, Reg->SSwindow, &pRxTop->CQOut, &pRxTop->CQPeak );

        Tamale_b_EstimateCFO(outReg.CFOeEnB, outReg.CFOeGet, RX->rReal[k22], RX->rImag[k22], &pRxTop->CFO, Reg);
    }
    // ----------------------------------------------------------------------
    //
    // 11 MHz Clock Domain
    ...
    if (posedgeCLK11MHz && (k11 > 22))
    {
        ...
        Tamale_b_ChanEst( outReg.CEEnB, outReg.CEMode, outReg.CEGet, RX->rReal[k22], RX->rImag[k22], 
			              pRxTop->SSOut.Q, pRxTop->SSOutValid.Q.b0, outReg.DBPSK, &pRxTop->CEDone, RX, Reg);

        Tamale_bRX_CMF( outReg.CMFEnB, pRxTop->DMSym, pRxTop->dDeciFreeze, RX->uReal+k11, RX->uImag+k11, 
			            &pRxTop->SSMatOutR, &pRxTop->SSMatOutI, RX->Nh, RX->hReal, RX->hImag );

        Tamale_bRX_DSSS( outReg.DMEnB, RX->uReal+k11, RX->uImag+k11, pRxTop->SSMatOutR, pRxTop->SSMatOutI, 
        ...
        Tamale_bRX_SP( RX->uReal+k11, RX->uImag+k11, &wR, &wI, RX->Np2, 
			           outReg.SPEnB, outReg.TransCCK, pRxTop->dDeciFreeze );

        if (!outReg.PSDUEnB && pRxTop->cckSym.delay_10T)
        {
            Tamale_bRX_CCK( wR-10,wI-10, &(pRxTop->cckOut.D), &(pRxTop->cckWalOutValid.D), 
        ...
        Tamale_bRX_Descrambler( pRxTop->SSOut.Q, pRxTop->SSOutValid.Q.b0, pRxTop->cckOut.Q, pRxTop->cckOutValid.Q.b0, 
			                    outReg.DBPSK, &pRxTop->DesOut, &pRxTop->DesOutValid, RX, pRxTop->state.Q.w6 );

        Tamale_bRX_SFD (outReg.SFDEnB, pRxTop->DesOut.Q, pRxTop->DesOutValid.Q.b0, &pRxTop->Preamble, &pRxTop->SFDDone);

        Tamale_bRX_Header( outReg.HDREnB, outReg.DBPSK, pRxTop->Preamble.Q.b0, 
			               pRxTop->SSOutValid.Q.b0, RX->a, &pRxTop->HDRDone, RX );

        Tamale_bRX_PSDU(outReg.PSDUEnB, pRxTop->DesOutValid.Q.b0, &pRxTop->PSDUDone, pRX);
    }
    // -----------------------
    ...
    //
    // ----------------------------------------------------------------------
    if (posedgeCLK22MHz && Tamale_RxState()->EnableTrace)
    {
