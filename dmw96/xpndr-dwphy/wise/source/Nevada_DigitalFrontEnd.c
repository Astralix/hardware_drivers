//--< Nevada_DigitalFrontEnd.c >-------------------------------------------------------------------
//=================================================================================================
//
//  Nevada - OFDM Digital Front End
...
// ================================================================================================
// FUNCTION  : Nevada_CFO_Angle
// ------------------------------------------------------------------------------------------------
// Purpose   : Compute the angle for a cartesian coordinate
// Parameters: 
// ================================================================================================
int Nevada_CFO_Angle(wiWORD b, wiWORD a)
{
...
// ================================================================================================
void Nevada_CFO( wiUWORD CFOEnB, wiUWORD CFOPathSel, wiUWORD CFOStart, wiUWORD CFOEnd,
                 ...
                 Nevada_RxState_t *pRX, NevadaRegisters_t *pReg )
{
    ...
    *CFODO1I = x1I[0].Q;
    ...
    if (p->state.Q.word == 5)
    {
        ...
        // Calculate Angle
        // ---------------
        phi.word   = Nevada_CFO_Angle(anginI, anginR);
    ...
    else if (p->state.Q.word == 10)
    {
        ...
        // calculate angle
        // ---------------
        phi.word        = Nevada_CFO_Angle(anginI, anginR)/4;
...
// ================================================================================================
// FUNCTION  : Nevada_FS_UIntToFloat
// ------------------------------------------------------------------------------------------------
// Purpose   : Convert an unsigned integer to floating point
// Parameters: 
// ================================================================================================
void Nevada_FS_UIntToFloat(wiUWORD *significand, wiWORD *exponent, unsigned int t)
{
    int i;
    if (t > 1023)
    {
...
// end of Nevada_FS_UIntToFloat() 

// ================================================================================================
// FUNCTION  : Nevada_FrameSync
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
void Nevada_FrameSync( wiUWORD FSEnB, wiUWORD FSPathSel, wiUWORD FSAGCDone, wiUWORD FSPeakSearch,
                       wiWORD FSDI0R, wiWORD FSDI0I, wiWORD FSDI1R, wiWORD FSDI1I, wiUWORD *FSPeakFound, 
                       wiUWORD *FSSigDetValid, Nevada_RxState_t *pRX, NevadaRegisters_t *pReg )
{
    ...
    Nevada_FrameSync_t *p = &(pRX->FrameSync);
    ...
    // ---------------------------------------------------
    Nevada_FS_UIntToFloat(&AxRSig, &AxRExp, abs(p->AcmAxR.Q.w20));
    Nevada_FS_UIntToFloat(&AxISig, &AxIExp, abs(p->AcmAxI.Q.w20));
    Nevada_FS_UIntToFloat(&Pw1Sig, &Pw1Exp, p->AcmPw0.Q.w20);
    Nevada_FS_UIntToFloat(&Pw2Sig, &Pw2Exp, p->AcmPw1.Q.w20);

    // ------------------------------------------------   
    // Compute the Test Metric Denominator Terms: Power
    // ------------------------------------------------   
    Nevada_FS_UIntToFloat(&(p->PwSig.D), &PwSigExp, Pw1Sig.w11*Pw2Sig.w11); 
    p->PwExp.D.word = PwSigExp.w7 + Pw1Exp.w6 + Pw2Exp.w6;

    // -----------------------------------------------------------------
    // Compute the Test Metric Numerator Terms: Autocorrelation w/Lag=16
    // -----------------------------------------------------------------
    Nevada_FS_UIntToFloat(&Ax1Sig, &Ax1Exp, (AxRSig.w11 * AxRSig.w11));
    Nevada_FS_UIntToFloat(&Ax2Sig, &Ax2Exp, (AxISig.w11 * AxISig.w11));
    ...
    if (p->PwSig.Q.w11 > 0)
    {
        Nevada_FS_UIntToFloat(&(p->RatioSig.D), &RatioSigExp, ((p->AxSig.Q.w11<<11)/p->PwSig.Q.w11)); 
...
// end of Nevada_Linear_to_dB4()

// ================================================================================================
// FUNCTION  : Nevada_DigitalAmp_Core
// ------------------------------------------------------------------------------------------------
// Purpose   : Core functionality for Nevada_DigitalAmp
...
// ================================================================================================
void Nevada_DigitalAmp_Core(wiBoolean DAEnB, wiWORD  DADI, wiWORD *DADO, wiUWORD Gain, wiReg *OutReg)
{
...
// end of Nevada_DigitalAmp_Core()

// ================================================================================================
// FUNCTION  : Nevada_DigitalAmp
// ------------------------------------------------------------------------------------------------
                         ...
                         Nevada_RxState_t *pRX)
{
    ...
    // ------------------------------
    Nevada_DigitalAmp_Core(DAEnB, DADI0R, DADO0R, A, pRX->DigitalAmpOut+0); // path 0, real
    Nevada_DigitalAmp_Core(DAEnB, DADI0I, DADO0I, A, pRX->DigitalAmpOut+1); // path 0, imaginary
    Nevada_DigitalAmp_Core(DAEnB, DADI1R, DADO1R, A, pRX->DigitalAmpOut+2); // path 1, real
    Nevada_DigitalAmp_Core(DAEnB, DADI1I, DADO1I, A, pRX->DigitalAmpOut+3); // path 1, imaginary
}
// end of Nevada_DigitalAmp()
...
// ================================================================================================
void Nevada_AGC( wiUWORD AGCEnB, wiUWORD AGCDAEnB, wiUWORD AGCPathSel, wiUWORD AGCInitGains, 
                 ...
                 NevadaRegisters_t *pReg, Nevada_RxState_t *pRX, Nevada_aSigState_t *SigOut)
{
    Nevada_AGC_t *p = &(pRX->AGC);
    ...
    // ---------------------------
    Nevada_DigitalAmp( p->DAEnB.Q.b0, p->DGain.Q, p->DI0R, p->DI0I, p->DI1R, p->DI1I, 
    ...
    if (p->PathSel.Q.b0) // Path 0 //////////////////////////////////////////////////////////
    {
        ...
        Nevada_Linear_to_dB4(p->AcmPwr0.Q, &msrpwr0);
    }
    if (p->PathSel.Q.b1) // Path 1 //////////////////////////////////////////////////////////
    {
        ...
        Nevada_Linear_to_dB4(p->AcmPwr1.Q, &msrpwr1);
    }
    /////////////////////////////////////////////////////////////////////////////////////
    //
    ...
    else if (AGCUpdateGains.b0)
    {
        ...
        //
        {
            ...
            if (!GainLimit && AGCLgSigAFE)           pRX->NonRTL.GainUpdateType[i] = NEVADA_GAIN_UPDATE_LGSIGAFE;
            else if (!GainLimit && msrpwr.w7 > pReg->ThSigSmall.w7)
            {
                if (msrpwr.w7 > pReg->ThSigLarge.w7) pRX->NonRTL.GainUpdateType[i] = NEVADA_GAIN_UPDATE_THSIGLARGE;
                else if (wPeakDet)                   pRX->NonRTL.GainUpdateType[i] = NEVADA_GAIN_UPDATE_THSMALLPEAK;
                else                                 pRX->NonRTL.GainUpdateType[i] = NEVADA_GAIN_UPDATE_THSIGSMALL;
            }
            else                                     pRX->NonRTL.GainUpdateType[i] = NEVADA_GAIN_UPDATE_LINEAR;
        }
    }
...
//             pReg      -- Pointer to modeled register file (model input)
// ================================================================================================
void Nevada_SignalDetect(wiUWORD SDEnB, wiUWORD SDPathSel, wiWORD SDDI0R, wiWORD SDDI0I, wiWORD SDDI1R, 
                          wiWORD SDDI1I, wiUWORD *SDSigDet, Nevada_RxState_t *pRX, NevadaRegisters_t *pReg)
{
    ...
    Nevada_SignalDetect_t *p = &(pRX->SignalDetect);
...
// end of Nevada_SignalDetect()

// ================================================================================================
// FUNCTION  : Nevada_DigitalFrontEnd
// ------------------------------------------------------------------------------------------------
// ================================================================================================
void Nevada_DigitalFrontEnd( wiBoolean DFEEnB, wiBoolean DFERestart, wiBoolean CheckRIFS, wiBoolean HoldGain,
                             ...
                             wiBoolean RestartRX, Nevada_aSigState_t *SigOut, Nevada_aSigState_t SigIn )
{
    Nevada_DigitalFrontEnd_t *p = &(pRX->DigitalFrontEnd);
    ...
    // ----------------------
    Nevada_AGC( p->AGCEnB, p->DAEnB, DFEPathSel, p->InitGains, p->UpdateGains, p->ForceLinear, 
    ...
    // Signal Detector
    // ---------------
    Nevada_SignalDetect(p->SDEnB, DFEPathSel, w0R, w0I, w1R, w1I, &p->SigDet, pRX, pReg);

    // ---------------------
    // Frame Synchronization
    // ---------------------
    Nevada_FrameSync(p->FSEnB, DFEPathSel, p->AGCdone, p->PeakSearch, s0R, s0I, s1R, s1I, &p->PeakFound, &p->SigDetValid, pRX, pReg);

    // ------------------------
    // Carrier Frequency Offset
    // ------------------------
    Nevada_CFO(p->CFOEnB, DFEPathSel, p->StartCFO, p->PeakFound, s0R, s0I, s1R, s1I, DFEOut0R, DFEOut0I, DFEOut1R, DFEOut1I, pRX, pReg);
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    //
    //  DSSS/CCK Demodulation
    //  Signal added to version 1b (DMW96B) to indicate the DFE is processing a DSSS/CCK
    //  packet. This signal is routed to the top level and output via the PA50 signal mux.
    //  The signal is not registered before the multiplexer, so the "input" signal of the
    //  signal state is used at the top-level.
    //
    SigOut->RxIsCCK = (p->State.Q.w6 == 21 || p->State.Q.w6 == 22 || p->State.Q.w6 == 28) ? 1 : 0;
