...
//=================================================================================================
static Nevada_TxState_t        TX[WISE_MAX_THREADS] = {{0}};  // transmitter state
static Nevada_RxState_t        RX[WISE_MAX_THREADS] = {{0}};  // receiver state
static Nevada_bRxState_t      bRX[WISE_MAX_THREADS] = {{0}};  // receiver state
static Nevada_DvState_t        DV[WISE_MAX_THREADS] = {{0}};  // design verification parameters
static NevadaRegisters_t Register[WISE_MAX_THREADS] ={{{0}}}; // baseband configuration registers

static Nevada_LookupTable_t *pLookupTable           = NULL;   // lookup tables (set at initialization)
...
// ================================================================================================
// FUNCTION  : Nevada_InitializeThread()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_InitializeThread(unsigned int ThreadIndex)
{
    ...
    else
    {
        ...
        memcpy(      TX + ThreadIndex,       TX+0, sizeof(Nevada_TxState_t) );
        memcpy(      RX + ThreadIndex,       RX+0, sizeof(Nevada_RxState_t) );
        memcpy(     bRX + ThreadIndex,      bRX+0, sizeof(Nevada_bRxState_t));
        memcpy(      DV + ThreadIndex,       DV+0, sizeof(Nevada_DvState_t) );
        memcpy(Register + ThreadIndex, Register+0, sizeof(NevadaRegisters_t));

        Nevada_TX_FreeMemory ( TX + ThreadIndex, 1);
        Nevada_RX_FreeMemory ( RX + ThreadIndex, 1);
        Nevada_bRX_FreeMemory(bRX + ThreadIndex, 1);

        TX[ThreadIndex].LastAllocation.Fn = NULL; // force allocation on next TX

        for (i=0; i<NEVADA_MAX_DV_TX; i++)
            DV[ThreadIndex].MAC_TX_D[i] = NULL;
    }    
    return WI_SUCCESS;
}
// end of Nevada_InitializeThread()

// ================================================================================================
// FUNCTION  : Nevada_CloseThread()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_CloseThread(unsigned int ThreadIndex)
{
    if (InvalidRange(ThreadIndex, 1, WISE_MAX_THREADS)) 
        return WI_ERROR_PARAMETER1;
    else
    {
        int i;
        XSTATUS( Nevada_TX_FreeMemory ( TX + ThreadIndex, 0) )
        XSTATUS( Nevada_RX_FreeMemory ( RX + ThreadIndex, 0) )
        XSTATUS( Nevada_bRX_FreeMemory(bRX + ThreadIndex, 0) );
...
// end of Nevada_CloseThread()

// ================================================================================================
// FUNCTION  : Nevada_ReadRegisterMap()
// ------------------------------------------------------------------------------------------------
...
wiStatus Nevada_ReadRegisterMap(wiUWORD *RegMap)
{
    ...
    {
        wiWORD Zero = {0};
        Nevada_DCX(-1, Zero, NULL, 0, Nevada_RxState(), pReg);
    }
    pReg->ID.word = NEVADA_PART_ID; // force the part ID field

    return WI_SUCCESS;
}
// end of Nevada_ReadRegisterMap()

// ================================================================================================
// FUNCTION  : Nevada_WriteRegisterMap()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
wiStatus Nevada_WriteRegisterMap(wiUWORD *RegMap)
{
    NevadaRegisters_t *pReg = Nevada_Registers();

    if (wiProcess_ThreadIndex() != 0) return STATUS(WI_ERROR_INVALID_FOR_THREAD);

    pReg->ID.word = NEVADA_PART_ID; // force the part ID value
    memset( RegMap, 0, 1024 );

    //  Matlab Generated Code from Nevada_GetRegisterMap.m
    // ----------------------------------------------------------------------------------
...
// end of Nevada_WriteRegisterMap()

// ================================================================================================
// FUNCTION  : Nevada_ReadRegister()
// ------------------------------------------------------------------------------------------------
// Purpose   : Update and return the internal register map
// Parameters: pRegMap -- pointer (by reference) to the register map
// ================================================================================================
wiUInt Nevada_ReadRegister(wiUInt Addr)
{
    if (Addr < 1024)
    {
        STATUS( Nevada_WriteRegisterMap(RegMap) );      // copy parameters to register map
        return (wiUInt)(RegMap[Addr].w8);
    }
    else return 0; // address not valid
}
// end of Nevada_ReadRegister()

// ================================================================================================
// FUNCTION  : Nevada_WriteRegister()
// ------------------------------------------------------------------------------------------------
// Purpose   : Write a register by address
// ================================================================================================
wiStatus Nevada_WriteRegister(wiUInt Addr, wiUInt Data)
{
    if (wiProcess_ThreadIndex() != 0)  return STATUS(WI_ERROR_INVALID_FOR_THREAD);
    ...
    {
        XSTATUS(Nevada_WriteRegisterMap(RegMap)); // copy parameters to register map
        RegMap[Addr].word = Data;                 // write data to register map
        XSTATUS(Nevada_ReadRegisterMap(RegMap));  // load parameters from register map
    }
    return WI_SUCCESS;
}
// end of Nevada_WriteRegister()

// ================================================================================================
// FUNCTION  : Nevada_LoadRegisters()
// ------------------------------------------------------------------------------------------------
// Purpose   : Save a register map to a file
// Parameters: RegMap   -- pointer to the register map to which to parameters are written
//             Filename -- name of the file to write
// ================================================================================================
wiStatus Nevada_LoadRegisters(wiUWORD *RegMap, wiString Filename)
{
...
// end of Nevada_LoadRegisters()

// ================================================================================================
// FUNCTION  : Nevada_SaveRegisters()
// ------------------------------------------------------------------------------------------------
// Purpose   : Save a register map to a file
// Parameters: RegMap   -- pointer to the register map to which to parameters are written
//             Filename -- name of the file to write
// ================================================================================================
wiStatus Nevada_SaveRegisters(wiUWORD *RegMap, wiString Filename)
{
    int i; time_t t;
    FILE *F;
    ...
    fprintf(F,"// ----------------------------------------------------------------------------\n");
    fprintf(F,"// Settings from WiSE::Nevada: %s",ctime(&t));
    fprintf(F,"// ----------------------------------------------------------------------------\n");
...
// end of Nevada_SaveRegisters()

// ================================================================================================
// FUNCTION  : Nevada_GetRegisterMap()
// ------------------------------------------------------------------------------------------------
// Purpose   : Update and return the internal register map
// Parameters: pRegMap -- pointer (by reference) to the register map
// ================================================================================================
wiStatus Nevada_GetRegisterMap(wiUWORD **pRegMap)
{
    XSTATUS( Nevada_WriteRegisterMap(RegMap) );
    *pRegMap = RegMap;
    return WI_SUCCESS;
}
// end of Nevada_GetRegisterMap()

// ================================================================================================
// FUNCTION  : Nevada_SetRegisterMap()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
wiStatus Nevada_SetRegisterMap(wiUInt RegData[], unsigned int NumRegs)
{
    unsigned int i;
    for (i=0; i<NumRegs; i++) 
        RegMap[i].word = RegData[i];

    XSTATUS( Nevada_ReadRegisterMap(RegMap) );
    return WI_SUCCESS;
}
// end of Nevada_SetRegisterMap()

// ================================================================================================
// FUNCTION : Nevada_Init()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_Init(void)
{
	pLookupTable = (Nevada_LookupTable_t *)wiMalloc(sizeof(Nevada_LookupTable_t));
	if (!pLookupTable) return STATUS(WI_ERROR_MEMORY_ALLOCATION);	

    XSTATUS( Nevada_TX_Init() );
    XSTATUS( Nevada_RX_Init() );
    XSTATUS( Nevada_b_Init()  );
    XSTATUS( Nevada_TX_DPD_Init() );
    XSTATUS( Nevada_RESET()   ); // power-on reset
...
// ================================================================================================
// FUNCTION : Nevada_Close()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_Close(void)
{
    unsigned int i, n;

    for (n=0; n<WISE_MAX_THREADS; n++)
    {
        XSTATUS( Nevada_TX_FreeMemory ( TX+n, 0)  );
        XSTATUS( Nevada_RX_FreeMemory ( RX+n, 0)  );
        XSTATUS( Nevada_bRX_FreeMemory(bRX+n, 0) );
        for (i=0; i<NEVADA_MAX_DV_TX; i++) 
        {
...
// ================================================================================================
// FUNCTION : Nevada_DvCopyTxData()
// ------------------------------------------------------------------------------------------------
// Purpose  : Copy the current PHY_TX_D array to pDV->PHY_TX_RD[n]
// ================================================================================================
wiStatus Nevada_DvCopyTxData(wiUInt n)
{
    Nevada_TxState_t *pTX = Nevada_TxState();
    Nevada_DvState_t *pDV = Nevada_DvState();

    if (n > NEVADA_MAX_DV_TX) return WI_ERROR_PARAMETER1;
...
// end of Nevada_DvSetTxDataByte()

// ================================================================================================
// FUNCTION : Nevada_DvCopyTxHeader()
// ------------------------------------------------------------------------------------------------
// Purpose  : Copy the PHY header from pDV->PHY_TX_RD[n] to the TxHeader registers
// ================================================================================================
wiStatus Nevada_DvCopyTxHeader(wiUInt n)
{
    Nevada_DvState_t  *pDV  = Nevada_DvState();
    NevadaRegisters_t *pReg = Nevada_Registers();
...
// end of Nevada_DvCopyTxHeader()

// ================================================================================================
// FUNCTION : Nevada_RESET()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_RESET(void)
{
    /////////////////////////////////////////////////////////////////////////////////////
    //
    ...
    Nevada_TX_FreeMemory(Nevada_TxState(), 1);

    /////////////////////////////////////////////////////////////////////////////////////
    //
    //  Set Default Register Values
    //
    XSTATUS( Nevada_SetRegisterMap(DefaultRegisters, sizeof(DefaultRegisters)/sizeof(wiUWORD)) );
    ...
    {
        wiWORD Zero = {0};
        Nevada_DCX(-1, Zero, NULL, 0, Nevada_RxState(), Nevada_Registers());
    }
    return WI_SUCCESS;
}
// end of Nevada_RESET()
...
// ================================================================================================
// FUNCTION : Nevada_DvState
// ------------------------------------------------------------------------------------------------
// ================================================================================================
Nevada_DvState_t* Nevada_DvState(void)
{
    return DV + wiProcess_ThreadIndex();
}
// end of Nevada_DvState()

// ================================================================================================
// FUNCTION : Nevada_TxState
// ------------------------------------------------------------------------------------------------
// ================================================================================================
Nevada_TxState_t* Nevada_TxState(void)
{
    int ThreadIndex = wiProcess_ThreadIndex();
    Nevada_TxState_t *pTX = &(TX[ThreadIndex]);

//wiPrintf("Nevada_TxState [%d] = @x%08X [@x%08X, @x%08X, @x%08X] @x%08X\n",ThreadIndex,pTX,TX+0,TX+1,TX+2,TX+3);

    return pTX;
}
// end of Nevada_TxState()

// ================================================================================================
// FUNCTION : Nevada_RxState
// ------------------------------------------------------------------------------------------------
// ================================================================================================
Nevada_RxState_t* Nevada_RxState(void)
{
    return RX + wiProcess_ThreadIndex();
}
// end of Nevada_RxState()

// ================================================================================================
// FUNCTION : Nevada_bRxState()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
Nevada_bRxState_t * Nevada_bRxState(void)
{
    return bRX + wiProcess_ThreadIndex();
}
// end of Nevada_bRxState()

// ================================================================================================
// FUNCTION : Nevada_LookupTable()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
Nevada_LookupTable_t* Nevada_LookupTable(void)
{
    return pLookupTable;
}
// end of Nevada_LookupTable()

// ================================================================================================
// FUNCTION : Nevada_Registers
// ------------------------------------------------------------------------------------------------
// ================================================================================================
NevadaRegisters_t* Nevada_Registers(void)
{
    return Register + wiProcess_ThreadIndex();
}
// end of Nevada_Registers()

// ================================================================================================
// FUNCTION : Nevada_GetArrayByName
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_GetArrayByName(wiString Name, unsigned int n, void **ptr)
{
         if (!strcmp(Name, "TX.uReal"))          *ptr = (void *)Nevada_TxState()->uReal;
    else if (!strcmp(Name, "TX.uImag"))          *ptr = (void *)Nevada_TxState()->uImag;
    else if (!strcmp(Name, "RX.tReal"))          *ptr = (void *)Nevada_RxState()->tReal[n];
    else if (!strcmp(Name, "RX.tImag"))          *ptr = (void *)Nevada_RxState()->tImag[n];
    else if (!strcmp(Name, "RX.uReal"))          *ptr = (void *)Nevada_RxState()->uReal[n];
    else if (!strcmp(Name, "RX.uImag"))          *ptr = (void *)Nevada_RxState()->uImag[n];
    else if (!strcmp(Name, "RX.vReal"))          *ptr = (void *)Nevada_RxState()->vReal[n];
    else if (!strcmp(Name, "RX.vImag"))          *ptr = (void *)Nevada_RxState()->vImag[n];
    else if (!strcmp(Name, "RX.wReal"))          *ptr = (void *)Nevada_RxState()->wReal[n];
    else if (!strcmp(Name, "RX.wImag"))          *ptr = (void *)Nevada_RxState()->wImag[n];
    else if (!strcmp(Name, "RX.xReal"))          *ptr = (void *)Nevada_RxState()->xReal[n];
    else if (!strcmp(Name, "RX.xImag"))          *ptr = (void *)Nevada_RxState()->xImag[n];
    else if (!strcmp(Name, "RX.yReal"))          *ptr = (void *)Nevada_RxState()->yReal[n];
    else if (!strcmp(Name, "RX.yImag"))          *ptr = (void *)Nevada_RxState()->yImag[n];
    else if (!strcmp(Name, "RX.zReal"))          *ptr = (void *)Nevada_RxState()->zReal[n];
    else if (!strcmp(Name, "RX.zImag"))          *ptr = (void *)Nevada_RxState()->zImag[n];
    else if (!strcmp(Name, "RX.a"))              *ptr = (void *)Nevada_RxState()->a;
    else if (!strcmp(Name, "RX.b"))              *ptr = (void *)Nevada_RxState()->b;
    else if (!strcmp(Name, "RX.Lp"))             *ptr = (void *)Nevada_RxState()->Lp;
    else if (!strcmp(Name, "RX.Lc"))             *ptr = (void *)Nevada_RxState()->Lc;
    else if (!strcmp(Name, "RX.cA"))             *ptr = (void *)Nevada_RxState()->cA;
    else if (!strcmp(Name, "RX.cB"))             *ptr = (void *)Nevada_RxState()->cB;
    else if (!strcmp(Name, "RX.AGain"))          *ptr = (void *)Nevada_RxState()->AGain;
    else if (!strcmp(Name, "RX.DGain"))          *ptr = (void *)Nevada_RxState()->DGain;
    else if (!strcmp(Name, "RX.LNAGain"))        *ptr = (void *)Nevada_RxState()->LNAGain;
    else if (!strcmp(Name, "RX.DFEState"))       *ptr = (void *)Nevada_RxState()->DFEState;
    else if (!strcmp(Name, "RX.DFEFlags"))       *ptr = (void *)Nevada_RxState()->DFEFlags;
    else if (!strcmp(Name, "RX.AdvDly"))         *ptr = (void *)Nevada_RxState()->AdvDly;
    else if (!strcmp(Name, "RX.aSigOut"))        *ptr = (void *)Nevada_RxState()->aSigOut;
    else if (!strcmp(Name, "RX.bSigOut"))        *ptr = (void *)Nevada_RxState()->bSigOut;
    else if (!strcmp(Name, "RX.traceRxClock"))   *ptr = (void *)Nevada_RxState()->traceRxClock;
    else if (!strcmp(Name, "RX.traceRxState"))   *ptr = (void *)Nevada_RxState()->traceRxState;
    else if (!strcmp(Name, "RX.traceRxControl")) *ptr = (void *)Nevada_RxState()->traceRxControl;
    else if (!strcmp(Name, "RX.traceDFE"))       *ptr = (void *)Nevada_RxState()->traceDFE;
    else if (!strcmp(Name, "RX.traceAGC"))       *ptr = (void *)Nevada_RxState()->traceAGC;
    else if (!strcmp(Name, "RX.traceRSSI"))      *ptr = (void *)Nevada_RxState()->traceRSSI;
    else if (!strcmp(Name, "RX.traceDFS"))       *ptr = (void *)Nevada_RxState()->traceDFS;
    else if (!strcmp(Name, "RX.traceSigDet"))    *ptr = (void *)Nevada_RxState()->traceSigDet;
    else if (!strcmp(Name, "RX.traceFrameSync")) *ptr = (void *)Nevada_RxState()->traceFrameSync;
    else if (!strcmp(Name, "RX.traceRadioIO"))   *ptr = (void *)Nevada_RxState()->traceRadioIO;
    else if (!strcmp(Name, "RX.traceDataConv"))  *ptr = (void *)Nevada_RxState()->traceDataConv;
    else if (!strcmp(Name, "RX.traceRX2"))       *ptr = (void *)Nevada_RxState()->traceRX2;
    else if (!strcmp(Name, "RX.traceRx"))        *ptr = (void *)Nevada_RxState()->traceRx;
    else if (!strcmp(Name, "RX.traceImgDet"))    *ptr = (void *)Nevada_RxState()->traceImgDet;
    else if (!strcmp(Name, "RX.traceIQCal"))     *ptr = (void *)Nevada_RxState()->traceIQCal;
    else if (!strcmp(Name, "RX.PHY_RX_D"))       *ptr = (void *)Nevada_RxState()->PHY_RX_D;
    else if (!strcmp(Name, "bRX.xReal"))         *ptr = (void *)Nevada_bRxState()->xReal;
    else if (!strcmp(Name, "bRX.xImag"))         *ptr = (void *)Nevada_bRxState()->xImag;
    else if (!strcmp(Name, "bRX.uReal"))         *ptr = (void *)Nevada_bRxState()->uReal;
    else if (!strcmp(Name, "bRX.uImag"))         *ptr = (void *)Nevada_bRxState()->uImag;
    else if (!strcmp(Name, "bRX.traceCCK"))      *ptr = (void *)Nevada_bRxState()->traceCCK;
    else if (!strcmp(Name, "bRX.traceState"))    *ptr = (void *)Nevada_bRxState()->traceState;
    else if (!strcmp(Name, "bRX.traceControl"))  *ptr = (void *)Nevada_bRxState()->traceControl;
    else if (!strcmp(Name, "bRX.traceFrontReg")) *ptr = (void *)Nevada_bRxState()->traceFrontReg;
    else if (!strcmp(Name, "bRX.traceDPLL"))     *ptr = (void *)Nevada_bRxState()->traceDPLL;
    else if (!strcmp(Name, "bRX.traceBarker"))   *ptr = (void *)Nevada_bRxState()->traceBarker;
    else if (!strcmp(Name, "bRX.traceDSSS"))     *ptr = (void *)Nevada_bRxState()->traceDSSS;
    else if (!strcmp(Name, "bRX.traceCCKInput")) *ptr = (void *)Nevada_bRxState()->traceCCKInput;
    else if (!strcmp(Name, "bRX.traceCCKTVMF"))  *ptr = (void *)Nevada_bRxState()->traceCCKTVMF;
    else if (!strcmp(Name, "bRX.traceCCKFWT"))   *ptr = (void *)Nevada_bRxState()->traceCCKFWT;
    else if (!strcmp(Name, "bRX.traceResample")) *ptr = (void *)Nevada_bRxState()->traceResample;
    else return WI_ERROR_PARAMETER1;

    return WI_SUCCESS;
}
// end of Nevada_GetArrayByName()

// ================================================================================================
// FUNCTION : Nevada_InvertArrayBit
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_InvertArrayBit(wiString Name, unsigned int n, unsigned int k, unsigned int m)
{
    void *ptr;
    wiUInt *pU;

    if (n > 1)                               return STATUS(WI_ERROR_PARAMETER2);
    if (k > (unsigned)Nevada_RxState()->N80) return STATUS(WI_ERROR_PARAMETER3);
    if (m > 31)                              return STATUS(WI_ERROR_PARAMETER4);

    XSTATUS( Nevada_GetArrayByName(Name, n, &ptr) );
...
// end of Nevada_InvertArrayBit()

// ================================================================================================
// FUNCTION : Nevada_SetArrayWord
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_SetArrayWord(wiString Name, unsigned int n, unsigned int k, unsigned int x)
{
    void *ptr;
    wiUInt *pU;
...
// end of Nevada_SetArrayWord()

// ================================================================================================
// FUNCTION : Nevada_WriteConfig()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_WriteConfig(wiMessageFn MsgFunc)
{
    ...
    NevadaRegisters_t *p   = Nevada_Registers();
...
// end of Nevada_WriteConfig()

// ================================================================================================
// FUNCTION : Nevada_ParseLine()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Nevada_ParseLine(wiString text)
{
    if (strstr(text, "Nevada"))
    {
        wiStatus status;
        ...
        //  Matlab Generated Code from Nevada_GetRegisterMap(16)
        // ------------------------------------------------------------------------------
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ID",              &(pReg->ID.word),               0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.PathSel",         &(pReg->PathSel.word),          0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RXMode",          &(pReg->RXMode.word),           0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TestMode",        &(pReg->TestMode.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.UpmixConj",       &(pReg->UpmixConj.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DownmixConj",     &(pReg->DownmixConj.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.UpmixDown",       &(pReg->UpmixDown.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DownmixUp",       &(pReg->DownmixUp.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.UpmixOff",        &(pReg->UpmixOff.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DownmixOff",      &(pReg->DownmixOff.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxIQSrcR",        &(pReg->TxIQSrcR.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxIQSrcI",        &(pReg->TxIQSrcI.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.PrdDACSrcR",      &(pReg->PrdDACSrcR.word),       0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.PrdDACSrcI",      &(pReg->PrdDACSrcI.word),       0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aTxExtend",       &(pReg->aTxExtend.word),        0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aTxDelay",        &(pReg->aTxDelay.word),         0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bTxExtend",       &(pReg->bTxExtend.word),        0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bTxDelay",        &(pReg->bTxDelay.word),         0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxIQSrcRun",      &(pReg->TxIQSrcRun.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxSmoothing",     &(pReg->TxSmoothing.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TX_SERVICE",      &(pReg->TX_SERVICE.word),       0, 65535));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TX_PRBS",         &(pReg->TX_PRBS.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RX_SERVICE",      &(pReg->RX_SERVICE.word),       0, 65535));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RSSI0",           &(pReg->RSSI0.word),            0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RSSI1",           &(pReg->RSSI1.word),            0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.maxLENGTH",       &(pReg->maxLENGTH.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.NotSounding",     &(pReg->NotSounding.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.nReserved1",      &(pReg->nReserved1.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Reserved0",       &(pReg->Reserved0.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.maxLENGTHnH",     &(pReg->maxLENGTHnH.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.maxLENGTHnL",     &(pReg->maxLENGTHnL.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aPPDUMaxTimeH",   &(pReg->aPPDUMaxTimeH.word),    0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aPPDUMaxTimeL",   &(pReg->aPPDUMaxTimeL.word),    0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.PilotPLLEn",      &(pReg->PilotPLLEn.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aC",              &(pReg->aC.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bC",              &(pReg->bC.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.cC",              &(pReg->cC.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Sa",              &(pReg->Sa.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Sb",              &(pReg->Sb.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Ca",              &(pReg->Ca.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Cb",              &(pReg->Cb.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.sTxIQ",           &(pReg->sTxIQ.word),            0, 1));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.TxIQa11",         &(pReg->TxIQa11.word),         -16, 15));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.TxIQa22",         &(pReg->TxIQa22.word),         -16, 15));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.TxIQa12",         &(pReg->TxIQa12.word),         -16, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.sRxIQ",           &(pReg->sRxIQ.word),            0, 3));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.RxIQ0a11",        &(pReg->RxIQ0a11.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.RxIQ0a22",        &(pReg->RxIQ0a22.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.RxIQ0a12",        &(pReg->RxIQ0a12.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.RxIQ1a11",        &(pReg->RxIQ1a11.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.RxIQ1a22",        &(pReg->RxIQ1a22.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.RxIQ1a12",        &(pReg->RxIQ1a12.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.ImgDetCoefRH",    &(pReg->ImgDetCoefRH.word),    -8, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetCoefRL",    &(pReg->ImgDetCoefRL.word),     0, 255));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.ImgDetCoefIH",    &(pReg->ImgDetCoefIH.word),    -8, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetCoefIL",    &(pReg->ImgDetCoefIL.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetLength",    &(pReg->ImgDetLength.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetEn",        &(pReg->ImgDetEn.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetLengthH",   &(pReg->ImgDetLengthH.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetPostShift", &(pReg->ImgDetPostShift.word),  0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetPreShift",  &(pReg->ImgDetPreShift.word),   0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetDelay",     &(pReg->ImgDetDelay.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetAvgNum",    &(pReg->ImgDetAvgNum.word),     0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetPowerH",    &(pReg->ImgDetPowerH.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetPowerL",    &(pReg->ImgDetPowerL.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalDone",         &(pReg->CalDone.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalResultType",   &(pReg->CalResultType.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ImgDetDone",      &(pReg->ImgDetDone.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalRun",          &(pReg->CalRun.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalPwrAdjust",    &(pReg->CalPwrAdjust.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalMode",         &(pReg->CalMode.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalCoefUpdate",   &(pReg->CalCoefUpdate.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalFineStep",     &(pReg->CalFineStep.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalCoarseStep",   &(pReg->CalCoarseStep.word),    0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalPwrRatio",     &(pReg->CalPwrRatio.word),      0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalIterationTh",  &(pReg->CalIterationTh.word),   0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalDelay",        &(pReg->CalDelay.word),         0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalGain",         &(pReg->CalGain.word),          0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CalPhase",        &(pReg->CalPhase.word),         0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RefPwrDig",       &(pReg->RefPwrDig.word),        0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.InitAGain",       &(pReg->InitAGain.word),        0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.AbsPwrL",         &(pReg->AbsPwrL.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.AbsPwrH",         &(pReg->AbsPwrH.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThSigLarge",      &(pReg->ThSigLarge.word),       0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThSigSmall",      &(pReg->ThSigSmall.word),       0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.StepLgSigDet",    &(pReg->StepLgSigDet.word),     0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.StepLarge",       &(pReg->StepLarge.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.StepSmall",       &(pReg->StepSmall.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThSwitchLNA",     &(pReg->ThSwitchLNA.word),      0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.StepLNA",         &(pReg->StepLNA.word),          0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThSwitchLNA2",    &(pReg->ThSwitchLNA2.word),     0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.StepLNA2",        &(pReg->StepLNA2.word),         0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Pwr100dBm",       &(pReg->Pwr100dBm.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.StepSmallPeak",   &(pReg->StepSmallPeak.word),    0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.FixedGain",       &(pReg->FixedGain.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.InitLNAGain",     &(pReg->InitLNAGain.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.InitLNAGain2",    &(pReg->InitLNAGain2.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.UpdateOnLNA",     &(pReg->UpdateOnLNA.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.MaxUpdateCount",  &(pReg->MaxUpdateCount.word),   0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThStepUpRefPwr",  &(pReg->ThStepUpRefPwr.word),   0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThStepUp",        &(pReg->ThStepUp.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThStepDown",      &(pReg->ThStepDown.word),       0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThDFSUp",         &(pReg->ThDFSUp.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThDFSDn",         &(pReg->ThDFSDn.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThStepDownMsrPwr",&(pReg->ThStepDownMsrPwr.word), 0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSOn",           &(pReg->DFSOn.word),            0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFS2Candidates",  &(pReg->DFS2Candidates.word),   0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSSmooth",       &(pReg->DFSSmooth.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSMACFilter",    &(pReg->DFSMACFilter.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSHdrFilter",    &(pReg->DFSHdrFilter.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSIntDet",       &(pReg->DFSIntDet.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSPattern",      &(pReg->DFSPattern.word),       0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSminPPB",       &(pReg->DFSminPPB.word),        0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSdTPRF",        &(pReg->DFSdTPRF.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSmaxWidth",     &(pReg->DFSmaxWidth.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSmaxTPRF",      &(pReg->DFSmaxTPRF.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSminTPRF",      &(pReg->DFSminTPRF.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSminGap",       &(pReg->DFSminGap.word),        0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSPulseSR",      &(pReg->DFSPulseSR.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.detRadar",        &(pReg->detRadar.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.detHeader",       &(pReg->detHeader.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.detPreamble",     &(pReg->detPreamble.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.detStepDown",     &(pReg->detStepDown.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.detStepUp",       &(pReg->detStepUp.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.WaitAGC",         &(pReg->WaitAGC.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayAGC",        &(pReg->DelayAGC.word),         0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SigDetWindow",    &(pReg->SigDetWindow.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SyncShift",       &(pReg->SyncShift.word),        0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayRestart",    &(pReg->DelayRestart.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.OFDMSwDDelay",    &(pReg->OFDMSwDDelay.word),     0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.OFDMSwDTh",       &(pReg->OFDMSwDTh.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CFOFilter",       &(pReg->CFOFilter.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.UpdateGainMF",    &(pReg->UpdateGainMF.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ArmLgSigDet3",    &(pReg->ArmLgSigDet3.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.LgSigAFEValid",   &(pReg->LgSigAFEValid.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.IdleFSEnB",       &(pReg->IdleFSEnB.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.IdleSDEnB",       &(pReg->IdleSDEnB.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.IdleDAEnB",       &(pReg->IdleDAEnB.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SDEnB17",         &(pReg->SDEnB17.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayCFO",        &(pReg->DelayCFO.word),         0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayRSSI",       &(pReg->DelayRSSI.word),        0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayRIFS",       &(pReg->DelayRIFS.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.OFDMSwDEn",       &(pReg->OFDMSwDEn.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.OFDMSwDSave",     &(pReg->OFDMSwDSave.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.OFDMSwDInit",     &(pReg->OFDMSwDInit.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.UpdateGain11b",   &(pReg->UpdateGain11b.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.OFDMSwDThLow",    &(pReg->OFDMSwDThLow.word),     0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SigDetTh1",       &(pReg->SigDetTh1.word),        0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SigDetTh2H",      &(pReg->SigDetTh2H.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SigDetTh2L",      &(pReg->SigDetTh2L.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SigDetDelay",     &(pReg->SigDetDelay.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SigDetFilter",    &(pReg->SigDetFilter.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SyncFilter",      &(pReg->SyncFilter.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SyncMag",         &(pReg->SyncMag.word),          0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SyncThSig",       &(pReg->SyncThSig.word),        0, 15));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.SyncThExp",       &(pReg->SyncThExp.word),       -8, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SyncDelay",       &(pReg->SyncDelay.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.MinShift",        &(pReg->MinShift.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ChEstShift0",     &(pReg->ChEstShift0.word),      0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ChEstShift1",     &(pReg->ChEstShift1.word),      0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ChTracking",      &(pReg->ChTracking.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ChTrackGain1",    &(pReg->ChTrackGain1.word),     0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ChTrackGain2",    &(pReg->ChTrackGain2.word),     0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aC1",             &(pReg->aC1.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bC1",             &(pReg->bC1.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aS1",             &(pReg->aS1.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bS1",             &(pReg->bS1.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aC2",             &(pReg->aC2.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bC2",             &(pReg->bC2.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aS2",             &(pReg->aS2.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bS2",             &(pReg->bS2.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxToneDiv",       &(pReg->TxToneDiv.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxWordRH",        &(pReg->TxWordRH.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxWordRL",        &(pReg->TxWordRL.word),         0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxWordIH",        &(pReg->TxWordIH.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxWordIL",        &(pReg->TxWordIL.word),         0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DCXSelect",       &(pReg->DCXSelect.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DCXFastLoad",     &(pReg->DCXFastLoad.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DCXHoldAcc",      &(pReg->DCXHoldAcc.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DCXShift",        &(pReg->DCXShift.word),         0, 7));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.DCXAcc0RH",       &(pReg->DCXAcc0RH.word),       -16, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DCXAcc0RL",       &(pReg->DCXAcc0RL.word),        0, 255));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.DCXAcc0IH",       &(pReg->DCXAcc0IH.word),       -16, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DCXAcc0IL",       &(pReg->DCXAcc0IL.word),        0, 255));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.DCXAcc1RH",       &(pReg->DCXAcc1RH.word),       -16, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DCXAcc1RL",       &(pReg->DCXAcc1RL.word),        0, 255));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.DCXAcc1IH",       &(pReg->DCXAcc1IH.word),       -16, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DCXAcc1IL",       &(pReg->DCXAcc1IL.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.INITdelay",       &(pReg->INITdelay.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.EDwindow",        &(pReg->EDwindow.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CQwindow",        &(pReg->CQwindow.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SSwindow",        &(pReg->SSwindow.word),         0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CQthreshold",     &(pReg->CQthreshold.word),      0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.EDthreshold",     &(pReg->EDthreshold.word),      0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bWaitAGC",        &(pReg->bWaitAGC.word),         0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bAGCdelay",       &(pReg->bAGCdelay.word),        0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bRefPwr",         &(pReg->bRefPwr.word),          0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bInitAGain",      &(pReg->bInitAGain.word),       0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bThSigLarge",     &(pReg->bThSigLarge.word),      0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bStepLarge",      &(pReg->bStepLarge.word),       0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bThSwitchLNA",    &(pReg->bThSwitchLNA.word),     0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bStepLNA",        &(pReg->bStepLNA.word),         0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bThSwitchLNA2",   &(pReg->bThSwitchLNA2.word),    0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bStepLNA2",       &(pReg->bStepLNA2.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bPwr100dBm",      &(pReg->bPwr100dBm.word),       0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DAmpGainRange",   &(pReg->DAmpGainRange.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bMaxUpdateCount", &(pReg->bMaxUpdateCount.word),  0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CSMode",          &(pReg->CSMode.word),           0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bPwrStepDet",     &(pReg->bPwrStepDet.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bCFOQual",        &(pReg->bCFOQual.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bAGCBounded4",    &(pReg->bAGCBounded4.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bLgSigAFEValid",  &(pReg->bLgSigAFEValid.word),   0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bSwDEn",          &(pReg->bSwDEn.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bSwDSave",        &(pReg->bSwDSave.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bSwDInit",        &(pReg->bSwDInit.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bSwDDelay",       &(pReg->bSwDDelay.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bSwDTh",          &(pReg->bSwDTh.word),           0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bThStepUpRefPwr", &(pReg->bThStepUpRefPwr.word),  0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bThStepUp",       &(pReg->bThStepUp.word),        0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bThStepDown",     &(pReg->bThStepDown.word),      0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bRestartDelay",   &(pReg->bRestartDelay.word),    0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SymHDRCE",        &(pReg->SymHDRCE.word),         0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CENSym1",         &(pReg->CENSym1.word),          0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CENSym2",         &(pReg->CENSym2.word),          0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.hThreshold1",     &(pReg->hThreshold1.word),      0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.hThreshold2",     &(pReg->hThreshold2.word),      0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.hThreshold3",     &(pReg->hThreshold3.word),      0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SFDWindow",       &(pReg->SFDWindow.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.AntSel",          &(pReg->AntSel.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DW_PHYEnB",       &(pReg->DW_PHYEnB.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TopState",        &(pReg->TopState.word),         0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.CFO",             &(pReg->CFO.word),              0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxFaultCount",    &(pReg->RxFaultCount.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RestartCount",    &(pReg->RestartCount.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.NuisanceCount",   &(pReg->NuisanceCount.word),    0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.sPHYEnB",         &(pReg->sPHYEnB.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.PHYEnB",          &(pReg->PHYEnB.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.NoStepUpAgg",     &(pReg->NoStepUpAgg.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RSSIdB",          &(pReg->RSSIdB.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.UnsupportedGF",   &(pReg->UnsupportedGF.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ClrOnWake",       &(pReg->ClrOnWake.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ClrOnHdr",        &(pReg->ClrOnHdr.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ClrNow",          &(pReg->ClrNow.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Filter64QAM",     &(pReg->Filter64QAM.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Filter16QAM",     &(pReg->Filter16QAM.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.FilterQPSK",      &(pReg->FilterQPSK.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.FilterBPSK",      &(pReg->FilterBPSK.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.FilterUndecoded", &(pReg->FilterUndecoded.word),  0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.FilterAllTypes",  &(pReg->FilterAllTypes.word),   0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.AddrFilter",      &(pReg->AddrFilter.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.STA0",            &(pReg->STA0.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.STA1",            &(pReg->STA1.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.STA2",            &(pReg->STA2.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.STA3",            &(pReg->STA3.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.STA4",            &(pReg->STA4.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.STA5",            &(pReg->STA5.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.minRSSI",         &(pReg->minRSSI.word),          0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.StepUpRestart",   &(pReg->StepUpRestart.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.StepDownRestart", &(pReg->StepDownRestart.word),  0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.NoStepAfterSync", &(pReg->NoStepAfterSync.word),  0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.NoStepAfterHdr",  &(pReg->NoStepAfterHdr.word),   0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxRIFS",          &(pReg->RxRIFS.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ForceRestart",    &(pReg->ForceRestart.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.KeepCCA_New",     &(pReg->KeepCCA_New.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.KeepCCA_Lost",    &(pReg->KeepCCA_Lost.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.PktTimePhase80",  &(pReg->PktTimePhase80.word),   0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.WakeupTimeH",     &(pReg->WakeupTimeH.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.WakeupTimeL",     &(pReg->WakeupTimeL.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayStdby",      &(pReg->DelayStdby.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayBGEnB",      &(pReg->DelayBGEnB.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayADC1",       &(pReg->DelayADC1.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayADC2",       &(pReg->DelayADC2.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayModem",      &(pReg->DelayModem.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayDFE",        &(pReg->DelayDFE.word),         0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayRFSW",       &(pReg->DelayRFSW.word),        0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxRxTime",        &(pReg->TxRxTime.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayFCAL1",      &(pReg->DelayFCAL1.word),       0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayPA",         &(pReg->DelayPA.word),          0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayFCAL2",      &(pReg->DelayFCAL2.word),       0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayState51",    &(pReg->DelayState51.word),     0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayShutdown",   &(pReg->DelayShutdown.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelaySleep",      &(pReg->DelaySleep.word),       0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TimeExtend",      &(pReg->TimeExtend.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SquelchTime",     &(pReg->SquelchTime.word),      0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelaySquelchRF",  &(pReg->DelaySquelchRF.word),   0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DelayClearDCX",   &(pReg->DelayClearDCX.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RadioMC",         &(pReg->RadioMC.word),          0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DACOPM",          &(pReg->DACOPM.word),           0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCAOPM",         &(pReg->ADCAOPM.word),          0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCBOPM",         &(pReg->ADCBOPM.word),          0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RFSW1",           &(pReg->RFSW1.word),            0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RFSW2",           &(pReg->RFSW2.word),            0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.LNAEnB",          &(pReg->LNAEnB.word),           0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCBCTRLR",       &(pReg->ADCBCTRLR.word),        0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCBCTRL",        &(pReg->ADCBCTRL.word),         0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.sPA50",           &(pReg->sPA50.word),            0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.sPA24",           &(pReg->sPA24.word),            0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCFSCTRL",       &(pReg->ADCFSCTRL.word),        0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCCHSEL",        &(pReg->ADCCHSEL.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCOE",           &(pReg->ADCOE.word),            0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.LNAxxBOnSW2",     &(pReg->LNAxxBOnSW2.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCOFCEN",        &(pReg->ADCOFCEN.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCDCREN",        &(pReg->ADCDCREN.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCMUX0",         &(pReg->ADCMUX0.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCARDY",         &(pReg->ADCARDY.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCBRDY",         &(pReg->ADCBRDY.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.PPSEL_MODE",      &(pReg->PPSEL_MODE.word),       0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCFCAL2",        &(pReg->ADCFCAL2.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCFCAL1",        &(pReg->ADCFCAL1.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCFCAL",         &(pReg->ADCFCAL.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.VCMEN",           &(pReg->VCMEN.word),            0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DACIFS_CTRL",     &(pReg->DACIFS_CTRL.word),      0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DACFG_CTRL0",     &(pReg->DACFG_CTRL0.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DACFG_CTRL1",     &(pReg->DACFG_CTRL1.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RESETB",          &(pReg->RESETB.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxLNAGAIN2",      &(pReg->TxLNAGAIN2.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SREdge",          &(pReg->SREdge.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SRFreq",          &(pReg->SRFreq.word),           0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.sLNA24A",         &(pReg->sLNA24A.word),          0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.sLNA24B",         &(pReg->sLNA24B.word),          0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.sLNA50A",         &(pReg->sLNA50A.word),          0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.sLNA50B",         &(pReg->sLNA50B.word),          0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SelectCCA",       &(pReg->SelectCCA.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThCCA_SigDet",    &(pReg->ThCCA_SigDet.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThCCA_RSSI1",     &(pReg->ThCCA_RSSI1.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThCCA_RSSI2",     &(pReg->ThCCA_RSSI2.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThCCA_GF1",       &(pReg->ThCCA_GF1.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ThCCA_GF2",       &(pReg->ThCCA_GF2.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ExtendBusyGF",    &(pReg->ExtendBusyGF.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ExtendRxError",   &(pReg->ExtendRxError.word),    0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ExtendRxError1",  &(pReg->ExtendRxError1.word),   0, 255));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t0_r_s0", &(pReg->P1_coef_t0_r_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t0_i_s0", &(pReg->P1_coef_t0_i_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t1_r_s0", &(pReg->P1_coef_t1_r_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t1_i_s0", &(pReg->P1_coef_t1_i_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t2_r_s0", &(pReg->P1_coef_t2_r_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t2_i_s0", &(pReg->P1_coef_t2_i_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t3_r_s0", &(pReg->P1_coef_t3_r_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t3_i_s0", &(pReg->P1_coef_t3_i_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t4_r_s0", &(pReg->P1_coef_t4_r_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t4_i_s0", &(pReg->P1_coef_t4_i_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t0_r_s0", &(pReg->P3_coef_t0_r_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t0_i_s0", &(pReg->P3_coef_t0_i_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t1_r_s0", &(pReg->P3_coef_t1_r_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t1_i_s0", &(pReg->P3_coef_t1_i_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t2_r_s0", &(pReg->P3_coef_t2_r_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t2_i_s0", &(pReg->P3_coef_t2_i_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t0_r_s0", &(pReg->P5_coef_t0_r_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t0_i_s0", &(pReg->P5_coef_t0_i_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t1_r_s0", &(pReg->P5_coef_t1_r_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t1_i_s0", &(pReg->P5_coef_t1_i_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t2_r_s0", &(pReg->P5_coef_t2_r_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t2_i_s0", &(pReg->P5_coef_t2_i_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t0_r_s1", &(pReg->P1_coef_t0_r_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t0_i_s1", &(pReg->P1_coef_t0_i_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t1_r_s1", &(pReg->P1_coef_t1_r_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t1_i_s1", &(pReg->P1_coef_t1_i_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t2_r_s1", &(pReg->P1_coef_t2_r_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t2_i_s1", &(pReg->P1_coef_t2_i_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t3_r_s1", &(pReg->P1_coef_t3_r_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t3_i_s1", &(pReg->P1_coef_t3_i_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t4_r_s1", &(pReg->P1_coef_t4_r_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t4_i_s1", &(pReg->P1_coef_t4_i_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t0_r_s1", &(pReg->P3_coef_t0_r_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t0_i_s1", &(pReg->P3_coef_t0_i_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t1_r_s1", &(pReg->P3_coef_t1_r_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t1_i_s1", &(pReg->P3_coef_t1_i_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t2_r_s1", &(pReg->P3_coef_t2_r_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t2_i_s1", &(pReg->P3_coef_t2_i_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t0_r_s1", &(pReg->P5_coef_t0_r_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t0_i_s1", &(pReg->P5_coef_t0_i_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t1_r_s1", &(pReg->P5_coef_t1_r_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t1_i_s1", &(pReg->P5_coef_t1_i_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t2_r_s1", &(pReg->P5_coef_t2_r_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t2_i_s1", &(pReg->P5_coef_t2_i_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t0_r_s2", &(pReg->P1_coef_t0_r_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t0_i_s2", &(pReg->P1_coef_t0_i_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t1_r_s2", &(pReg->P1_coef_t1_r_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t1_i_s2", &(pReg->P1_coef_t1_i_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t2_r_s2", &(pReg->P1_coef_t2_r_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t2_i_s2", &(pReg->P1_coef_t2_i_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t3_r_s2", &(pReg->P1_coef_t3_r_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t3_i_s2", &(pReg->P1_coef_t3_i_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t4_r_s2", &(pReg->P1_coef_t4_r_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t4_i_s2", &(pReg->P1_coef_t4_i_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t0_r_s2", &(pReg->P3_coef_t0_r_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t0_i_s2", &(pReg->P3_coef_t0_i_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t1_r_s2", &(pReg->P3_coef_t1_r_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t1_i_s2", &(pReg->P3_coef_t1_i_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t2_r_s2", &(pReg->P3_coef_t2_r_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t2_i_s2", &(pReg->P3_coef_t2_i_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t0_r_s2", &(pReg->P5_coef_t0_r_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t0_i_s2", &(pReg->P5_coef_t0_i_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t1_r_s2", &(pReg->P5_coef_t1_r_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t1_i_s2", &(pReg->P5_coef_t1_i_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t2_r_s2", &(pReg->P5_coef_t2_r_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t2_i_s2", &(pReg->P5_coef_t2_i_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t0_r_s3", &(pReg->P1_coef_t0_r_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t0_i_s3", &(pReg->P1_coef_t0_i_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t1_r_s3", &(pReg->P1_coef_t1_r_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t1_i_s3", &(pReg->P1_coef_t1_i_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t2_r_s3", &(pReg->P1_coef_t2_r_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t2_i_s3", &(pReg->P1_coef_t2_i_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t3_r_s3", &(pReg->P1_coef_t3_r_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t3_i_s3", &(pReg->P1_coef_t3_i_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t4_r_s3", &(pReg->P1_coef_t4_r_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P1_coef_t4_i_s3", &(pReg->P1_coef_t4_i_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t0_r_s3", &(pReg->P3_coef_t0_r_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t0_i_s3", &(pReg->P3_coef_t0_i_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t1_r_s3", &(pReg->P3_coef_t1_r_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t1_i_s3", &(pReg->P3_coef_t1_i_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t2_r_s3", &(pReg->P3_coef_t2_r_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P3_coef_t2_i_s3", &(pReg->P3_coef_t2_i_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t0_r_s3", &(pReg->P5_coef_t0_r_s3.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t0_i_s3", &(pReg->P5_coef_t0_i_s3.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t1_r_s3", &(pReg->P5_coef_t1_r_s3.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t1_i_s3", &(pReg->P5_coef_t1_i_s3.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t2_r_s3", &(pReg->P5_coef_t2_r_s3.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Nevada.Register.P5_coef_t2_i_s3", &(pReg->P5_coef_t2_i_s3.word), -2048, 2047));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.cross_pwr1",      &(pReg->cross_pwr1.word),       0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.cross_pwr2",      &(pReg->cross_pwr2.word),       0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.cross_pwr3",      &(pReg->cross_pwr3.word),       0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Tx_Rate_OFDM_HT", &(pReg->Tx_Rate_OFDM_HT.word),  0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Tx_Rate_OFDM_LL", &(pReg->Tx_Rate_OFDM_LL.word),  0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Tx_Rate_OFDM_LM", &(pReg->Tx_Rate_OFDM_LM.word),  0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DPD_spare",       &(pReg->DPD_spare.word),        0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DPD_LOW_TMP_DIS", &(pReg->DPD_LOW_TMP_DIS.word),  0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DPD_PA_tmp",      &(pReg->DPD_PA_tmp.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DPD_LOW_RATE_DIS",&(pReg->DPD_LOW_RATE_DIS.word), 0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DPD_coef_interspersion",&(pReg->DPD_coef_interspersion.word), 0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Cbc_coef_sel",    &(pReg->Cbc_coef_sel.word),     0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.cnfg_eovr",       &(pReg->cnfg_eovr.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Coef_sel_vovr",   &(pReg->Coef_sel_vovr.word),    0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Vfb_byp_vovr",    &(pReg->Vfb_byp_vovr.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Pa_tmp_ext",      &(pReg->Pa_tmp_ext.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTDone",        &(pReg->BISTDone.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTADCSel",      &(pReg->BISTADCSel.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTNoPrechg",    &(pReg->BISTNoPrechg.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTFGSel",       &(pReg->BISTFGSel.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTMode",        &(pReg->BISTMode.word),         0, 7));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.StartBIST",       &(pReg->StartBIST.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTPipe",        &(pReg->BISTPipe.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTN_val",       &(pReg->BISTN_val.word),        0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTOfsR",        &(pReg->BISTOfsR.word),         0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTOfsI",        &(pReg->BISTOfsI.word),         0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTTh1",         &(pReg->BISTTh1.word),          0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTTh2",         &(pReg->BISTTh2.word),          0, 65535));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTFG_CTRLI",    &(pReg->BISTFG_CTRLI.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTFG_CTRLR",    &(pReg->BISTFG_CTRLR.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTStatus",      &(pReg->BISTStatus.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTAccR",        &(pReg->BISTAccR.word),         0, 16777215));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTAccI",        &(pReg->BISTAccI.word),         0, 16777215));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTRangeLimH",   &(pReg->BISTRangeLimH.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.BISTRangeLimL",   &(pReg->BISTRangeLimL.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSIntEOB",       &(pReg->DFSIntEOB.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSIntFIFO",      &(pReg->DFSIntFIFO.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSThFIFO",       &(pReg->DFSThFIFO.word),        0, 127));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSDepthFIFO",    &(pReg->DFSDepthFIFO.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSReadFIFO",     &(pReg->DFSReadFIFO.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.DFSSyncFIFO",     &(pReg->DFSSyncFIFO.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxHeader1",       &(pReg->RxHeader1.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxHeader2",       &(pReg->RxHeader2.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxHeader3",       &(pReg->RxHeader3.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxHeader4",       &(pReg->RxHeader4.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxHeader5",       &(pReg->RxHeader5.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxHeader6",       &(pReg->RxHeader6.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxHeader7",       &(pReg->RxHeader7.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxHeader8",       &(pReg->RxHeader8.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxHeader1",       &(pReg->TxHeader1.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxHeader2",       &(pReg->TxHeader2.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxHeader3",       &(pReg->TxHeader3.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxHeader4",       &(pReg->TxHeader4.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxHeader5",       &(pReg->TxHeader5.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxHeader6",       &(pReg->TxHeader6.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxHeader7",       &(pReg->TxHeader7.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxHeader8",       &(pReg->TxHeader8.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TestSigEn",       &(pReg->TestSigEn.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCAI",           &(pReg->ADCAI.word),            0, 1023));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCAQ",           &(pReg->ADCAQ.word),            0, 1023));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCBI",           &(pReg->ADCBI.word),            0, 1023));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ADCBQ",           &(pReg->ADCBQ.word),            0, 1023));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bHeaderValid",    &(pReg->bHeaderValid.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bSyncFound",      &(pReg->bSyncFound.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bAGCDone",        &(pReg->bAGCDone.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.bSigDet",         &(pReg->bSigDet.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aHeaderValid",    &(pReg->aHeaderValid.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aSyncFound",      &(pReg->aSyncFound.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aAGCDone",        &(pReg->aAGCDone.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.aSigDet",         &(pReg->aSigDet.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SelectSignal2",   &(pReg->SelectSignal2.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SelectSignal1",   &(pReg->SelectSignal1.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SelectTrigger2",  &(pReg->SelectTrigger2.word),   0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.SelectTrigger1",  &(pReg->SelectTrigger1.word),   0, 3));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TriggerState1",   &(pReg->TriggerState1.word),    0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TriggerState2",   &(pReg->TriggerState2.word),    0, 63));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TriggerRSSI",     &(pReg->TriggerRSSI.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.LgSigAFE",        &(pReg->LgSigAFE.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Trigger2",        &(pReg->Trigger2.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Trigger1",        &(pReg->Trigger1.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Signal2",         &(pReg->Signal2.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Signal1",         &(pReg->Signal1.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.ClearInt",        &(pReg->ClearInt.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.IntEvent2",       &(pReg->IntEvent2.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.IntEvent1",       &(pReg->IntEvent1.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.IntRxFault",      &(pReg->IntRxFault.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.IntRxAbortTx",    &(pReg->IntRxAbortTx.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.IntTxSleep",      &(pReg->IntTxSleep.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.IntTxAbort",      &(pReg->IntTxAbort.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.IntTxFault",      &(pReg->IntTxFault.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Interrupt",       &(pReg->Interrupt.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Event2",          &(pReg->Event2.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.Event1",          &(pReg->Event1.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxFault",         &(pReg->RxFault.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.RxAbortTx",       &(pReg->RxAbortTx.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxSleep",         &(pReg->TxSleep.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxAbort",         &(pReg->TxAbort.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.TxFault",         &(pReg->TxFault.word),          0, 1));
        // ------------------------------------------------------------------------------
        //  END: Matlab Generated Code

        PSTATUS(wiParse_Boolean(text,"Nevada.EnableModelIO",           &(pRX->EnableModelIO)));
        PSTATUS(wiParse_Boolean(text,"Nevada.EnableIQCal",             &(pRX->EnableIQCal)));
        PSTATUS(wiParse_UInt   (text,"Nevada.aPadFront",               &(pTX->aPadFront), 0,2000000));
        PSTATUS(wiParse_UInt   (text,"Nevada.aPadBack",                &(pTX->aPadBack),  0, 400000));
        PSTATUS(wiParse_UInt   (text,"Nevada.bPadFront",               &(pTX->bPadFront), 0,  20000));
        PSTATUS(wiParse_UInt   (text,"Nevada.bPadBack",                &(pTX->bPadBack),  0, 220000));
        PSTATUS(wiParse_Int    (text,"Nevada.RX.InitialPhase",         &(pRX->clock.InitialPhase), -1, 80));
        PSTATUS(wiParse_Int    (text,"Nevada.RX.DownsamPhase",         &(pRX->DownsamPhase),     0,  1));
        PSTATUS(wiParse_Int    (text,"Nevada.RX.DownmixPhase",         &(pRX->DownmixPhase),     0,  3));
        PSTATUS(wiParse_Boolean(text,"Nevada.TX.SetPBCC",              &(pTX->SetPBCC)));

        PSTATUS(wiParse_Int    (text,"Nevada.ADC.Offset",              &(pRX->ADC.Offset), -256, 256));
        PSTATUS(wiParse_Int    (text,"Nevada.ADC.nEff",                &(pRX->ADC.nEff), 2, 10));
        PSTATUS(wiParse_Real   (text,"Nevada.ADC.Vpp",                 &(pRX->ADC.Vpp),  0.5, 3.0));
        PSTATUS(wiParse_Int    (text,"Nevada.DAC.nEff",                &(pTX->DAC.nEff), 2, 10));
        PSTATUS(wiParse_Real   (text,"Nevada.DAC.Vpp",                 &(pTX->DAC.Vpp),  0.5, 3.0));
        PSTATUS(wiParse_Real   (text,"Nevada.DAC.IQdB",                &(pTX->DAC.IQdB),-6.0, 6.0));
        PSTATUS(wiParse_Int    (text,"Nevada.TX.bResamplePhase",       &(pTX->bResamplePhase),  -1,19));
        PSTATUS(wiParse_Int    (text,"Nevada.TX.UpmixPhase",           &(pTX->UpmixPhase), 0, 7));
        PSTATUS(wiParse_Int    (text,"Nevada.TX.TxTonePhase",          &(pTX->TxTonePhase), 0, 159));

		PSTATUS(wiParse_Boolean(text,"Nevada.RX.Nuisance.Enabled",   &(pRX->Nuisance.Enabled)));
		PSTATUS(wiParse_UInt   (text,"Nevada.RX.Nuisance.kNuisance", &(pRX->Nuisance.kNuisance), 0, 1<<20));

        PSTATUS(wiParse_Boolean(text,"Nevada.RX.NonRTL.Enabled",       &(pRX->NonRTL.Enabled)));
        PSTATUS(wiParse_UInt   (text,"Nevada.RX.NonRTL.MaxSamplesCFO", &(pRX->NonRTL.MaxSamplesCFO), 0, 160));
        PSTATUS(wiParse_Boolean(text,"Nevada.RX.NonRTL.DisableCFO",    &(pRX->NonRTL.DisableCFO)));
        PSTATUS(wiParse_Boolean(text,"Nevada.RX.NonRTL.DisablePLL",    &(pRX->NonRTL.DisablePLL)));
        PSTATUS(wiParse_Boolean(text,"Nevada.RX.NonRTL.MojaveFrontCtrlEOP",&(pRX->NonRTL.MojaveFrontCtrlEOP)));

        // legacy versions following block
        PSTATUS(wiParse_UInt   (text,"Nevada.RX.TX.k1",                &(pRX->TX.k1[0]),      0, 1<<20));
        PSTATUS(wiParse_UInt   (text,"Nevada.RX.TX.k2",                &(pRX->TX.k2[0]),      0, 1<<20));
        PSTATUS(wiParse_UInt   (text,"Nevada.RX.TX.kTxDone",           &(pRX->TX.kTxDone[0]), 0, 1<<20));
        PSTATUS(wiParse_UInt   (text,"Nevada.RX.TX.kTxFault",          &(pRX->TX.kTxFault),   0, 1<<20));

        PSTATUS(wiParse_Boolean  (text,"Nevada.RX.TX.Enabled",         &(pRX->TX.Enabled)));
        PSTATUS(wiParse_Boolean  (text,"Nevada.RX.TX.Usek2Delay",      &(pRX->TX.Usek2Delay)));
        PSTATUS(wiParse_UInt     (text,"Nevada.RX.TX.k2Delay",         &(pRX->TX.k2Delay), 0,  8000));
        PSTATUS(wiParse_UIntArray(text,"Nevada.RX.TX.k1",                pRX->TX.k1,      3, 0, 1<<20));
        PSTATUS(wiParse_UIntArray(text,"Nevada.RX.TX.k2",                pRX->TX.k2,      3, 0, 1<<20));
        PSTATUS(wiParse_UIntArray(text,"Nevada.RX.TX.kTxDone",           pRX->TX.kTxDone, 3, 0, 1<<20));

        PSTATUS(wiParse_Boolean(text,"Nevada.TX.NoTxHeaderUpdate",     &(pTX->NoTxHeaderUpdate)                ));
        PSTATUS(wiParse_Boolean(text,"Nevada.TX.UseForcedbSFD",        &(pTX->UseForcedbSFD)                   ));
        PSTATUS(wiParse_Boolean(text,"Nevada.TX.UseForcedbSIGNAL",     &(pTX->UseForcedbSIGNAL)                ));
        PSTATUS(wiParse_Boolean(text,"Nevada.TX.UseForcedbSERVICE",    &(pTX->UseForcedbSERVICE)               ));
        PSTATUS(wiParse_Boolean(text,"Nevada.TX.UseForcedbLENGTH",     &(pTX->UseForcedbLENGTH)                ));
        PSTATUS(wiParse_Boolean(text,"Nevada.TX.UseForcedbCRC",        &(pTX->UseForcedbCRC)                   ));
        PSTATUS(wiParse_Boolean(text,"Nevada.TX.UseForcedSIGNAL",      &(pTX->UseForcedSIGNAL)                 ));
        PSTATUS(wiParse_Boolean(text,"Nevada.TX.UseForcedHTSIG",       &(pTX->UseForcedHTSIG)                  ));
        PSTATUS(wiParse_Boolean(text,"Nevada.TX.UseForcedHTSIG_CRC",   &(pTX->UseForcedHTSIG_CRC)              ));

        PSTATUS(wiParse_UInt   (text,"Nevada.TX.ForcedbSFD",           &(pTX->ForcedbSFD.word),     0,    65535));
        PSTATUS(wiParse_UInt   (text,"Nevada.TX.ForcedbSIGNAL",        &(pTX->ForcedbSIGNAL.word),  0,      255));
        PSTATUS(wiParse_UInt   (text,"Nevada.TX.ForcedbSERVICE",       &(pTX->ForcedbSERVICE.word), 0,      255));
        PSTATUS(wiParse_UInt   (text,"Nevada.TX.ForcedbLENGTH",        &(pTX->ForcedbLENGTH.word),  0,    65535));
        PSTATUS(wiParse_UInt   (text,"Nevada.TX.ForcedbCRC",           &(pTX->ForcedbCRC.word),     0,    65535));
        PSTATUS(wiParse_UInt   (text,"Nevada.TX.ForcedSIGNAL",         &(pTX->ForcedSIGNAL.word),   0, 16777215));
        PSTATUS(wiParse_UInt   (text,"Nevada.TX.ForcedHTSIG1",         &(pTX->ForcedHTSIG1.word),   0, 16777215));
        PSTATUS(wiParse_UInt   (text,"Nevada.TX.ForcedHTSIG2",         &(pTX->ForcedHTSIG2.word),   0,   262143));
        PSTATUS(wiParse_Boolean(text,"Nevada.RX.MAC_NOFILTER",         &(pRX->MAC_NOFILTER)                    ));

        PSTATUS(wiParse_Boolean(text,"Nevada.TX.Checksum.Enable",      &(pTX->Checksum.Enable)) );

        PSTATUS(wiParse_UIntArray(text,"Nevada.TX.Trace.kMarker", pTX->Trace.kMarker, 5, 0, 0x7FFFFFFF));
        PSTATUS(wiParse_UIntArray(text,"Nevada.RX.Trace.kMarker", pRX->Trace.kMarker, 5, 0, 0x7FFFFFFF));
        PSTATUS(wiParse_UIntArray(text,"Nevada.RX.Trace.kStart",  pRX->Trace.kStart,  9, 0, 0x7FFFFFFF));
        PSTATUS(wiParse_UIntArray(text,"Nevada.RX.Trace.kEnd",    pRX->Trace.kEnd,    9, 0, 0x7FFFFFFF));

        PSTATUS(wiParse_Boolean  (text,"Nevada.TX.LgSigAFE",           &(pTX->LgSigAFE)));
        PSTATUS(wiParse_Boolean  (text,"Nevada.TX.FixedTimingShift",   &(pTX->FixedTimingShift)));
        PSTATUS(wiParse_Real     (text,"Nevada.TX.TimingShift",        &(pTX->TimingShift), -1.0, 1.0));

        PSTATUS(wiParse_Boolean  (text,"Nevada.DV.ExpectPacketFail",   &(pDV->ExpectPacketFail)) );
        PSTATUS(wiParse_Int      (text,"Nevada.DV.VerilogChecker",     &(pDV->VerilogChecker),    0, 0xFFFF));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.NumCheckRegisters",  &(pDV->NumCheckRegisters), 0, 512));
        PSTATUS(wiParse_UIntArray(text,"Nevada.DV.CheckRegAddr",         pDV->CheckRegAddr, 512, 0, 1023));
        PSTATUS(wiParse_Boolean  (text,"Nevada.DV.FixedTxTimingLimits",&(pDV->FixedTxTimingLimits)));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.aTxStart",           &(pDV->aTxStart),      0,  800));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.bTxStart",           &(pDV->bTxStart),      0,  800));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.aTxEnd",             &(pDV->aTxEnd),        0,  800));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.bTxEnd",             &(pDV->bTxEnd),        0,  800));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.aTxStartTolerance",  &(pDV->aTxStartTolerance), 0, 80));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.bTxStartTolerance",  &(pDV->bTxStartTolerance), 0, 80));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.aTxEndTolerance",    &(pDV->aTxEndTolerance),   0, 80));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.bTxEndTolerance",    &(pDV->bTxEndTolerance),   0, 80));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.TxStartMinT80",      &(pDV->TxStartMinT80), 0, 8000));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.TxStartMaxT80",      &(pDV->TxStartMaxT80), 0, 8000));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.TxEndMinT80",        &(pDV->TxEndMinT80),   0, 8000));
        PSTATUS(wiParse_UInt     (text,"Nevada.DV.TxEndMaxT80",        &(pDV->TxEndMaxT80),   0, 8000));
        PSTATUS(wiParse_Boolean  (text,"Nevada.DV.LongSimulation",     &(pDV->LongSimulation)));
        PSTATUS(wiParse_Boolean  (text,"Nevada.DV.BreakCheck",         &(pDV->BreakCheck)));
        PSTATUS(wiParse_Int      (text,"Nevada.DV.TestLimit",          &(pDV->TestLimit),-2147483647 ,2147483647));
        PSTATUS(wiParse_Int      (text,"Nevada.DV.DelayT80",           &(pDV->DelayT80), -8000000, 8000000));
        PSTATUS(wiParse_Boolean  (text,"Nevada.DV.NoRegMapUpdateOnTx", &(pDV->NoRegMapUpdateOnTx)));

        PSTATUS(wiParse_Boolean(text,"Nevada.TraceKeyOp",     NULL)); // parameter removed
        PSTATUS(wiParse_Boolean(text,"Nevada.EnableTraceCCK", NULL)); // parameter removed

        // -----------------------
        // Simulation Result Trace
        // -----------------------
        status = WICHK(wiParse_Boolean(text,"Nevada.EnableTrace", &(pRX->EnableTrace)));
        if (status==WI_SUCCESS)
        {
            XSTATUS(Nevada_RX_AllocateMemory());  // make sure trace arrays are allocated
            return WI_SUCCESS;
        }
        ...
        // Array Sizes
        // ----------- 
        status = STATUS(wiParse_UInt(text, "Nevada.aLength", &(pRX->aLength), 1, 16+8*65535+6+287));
        if (status == WI_SUCCESS) {
            XSTATUS(Nevada_RX_AllocateMemory());
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_UInt(text, "Nevada.rLength", &(pRX->rLength), 1, 1000000));
        if (status == WI_SUCCESS) {
            XSTATUS(Nevada_RX_AllocateMemory());
            return WI_SUCCESS;
        }
        ...
        // PLL parameters
        // --------------
        status = STATUS(wiParse_UInt(text,"Nevada.DPLL.Sa_Sb",&(X.word), 0, 63));
        if (status == WI_SUCCESS) {
            pReg->Sa.word = (X.w6 >> 3) & 0x7;
            pReg->Sb.word = (X.w6 >> 0) & 0x7;
            return WI_SUCCESS;
        }
        XSTATUS(status = wiParse_UInt(text,"Nevada.DPLL.Ca_Cb",&(X.word), 0, 63));
        if (status == WI_SUCCESS) {
            pReg->Ca.word = (X.w6 >> 3) & 0x7;
            pReg->Cb.word = (X.w6 >> 0) & 0x7;
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Command(text,"Nevada_RESET()"));
        if (status == WI_SUCCESS) {
            STATUS(Nevada_RESET());
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Nevada_SaveRegisters(%255s)",&buf));
        if (status == WI_SUCCESS) {
            XSTATUS(Nevada_WriteRegisterMap(RegMap));
            XSTATUS(Nevada_SaveRegisters(RegMap, buf));
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Nevada_LoadRegisters(%255s)",&buf));
        if (status == WI_SUCCESS) {
            XSTATUS(Nevada_WriteRegisterMap(RegMap));
            XSTATUS(Nevada_LoadRegisters(RegMap, buf));
            XSTATUS(Nevada_ReadRegisterMap(RegMap));
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Nevada_WriteRegister(%d,%d)",&(X.word), &(Y.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS(Nevada_WriteRegister(X.word, Y.word));
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Nevada_DvCopyTxData(%d)", &(X.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS( Nevada_DvCopyTxData(X.word) );
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Nevada_DvSetTxDataByte(%d, %d, %d)", &dummy, &(X.word), &(Y.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS( Nevada_DvSetTxDataByte((wiUInt)dummy, X.word, Y.word) );
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Nevada_DvCopyTxHeader(%d)", &(X.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS( Nevada_DvCopyTxHeader(X.word) );
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Command(text,"Nevada_WriteDvRegMap()"));
        if (status == WI_SUCCESS)
        {
            Nevada_DvState_t *pDV = Nevada_DvState();
            XSTATUS( Nevada_WriteRegisterMap( pDV->RegMap ) );
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Nevada_InvertArrayBit(%255s, %d, %d, %d)", &buf, &(X.word), &(Y.word), &(Z.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS( Nevada_InvertArrayBit(buf, X.word, Y.word, Z.word) );
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Nevada_SetArrayWord(%255s, %d, %d, %d)", &buf, &(X.word), &(Y.word), &(Z.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS( Nevada_SetArrayWord(buf, X.word, Y.word, Z.word) );
            return WI_SUCCESS;
        }
        /////////////////////////////////////////////////////////////////////////////////
        //
        ...
        //
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.FixedLNAGain",    &(pReg->InitLNAGain.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Nevada.Register.FixedLNA2Gain",   &(pReg->InitLNAGain2.word),    0, 1));
        //
