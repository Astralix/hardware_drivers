...
//=================================================================================================
static Tamale_TxState_t        TX[WISE_MAX_THREADS] = {{0}};  // transmitter state
static Tamale_RxState_t        RX[WISE_MAX_THREADS] = {{0}};  // receiver state
static Tamale_bRxState_t      bRX[WISE_MAX_THREADS] = {{0}};  // receiver state
static Tamale_DvState_t        DV[WISE_MAX_THREADS] = {{0}};  // design verification parameters
static TamaleRegisters_t Register[WISE_MAX_THREADS] ={{{0}}}; // baseband configuration registers

static Tamale_LookupTable_t *pLookupTable           = NULL;   // lookup tables (set at initialization)
...
// ================================================================================================
// FUNCTION  : Tamale_InitializeThread()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_InitializeThread(unsigned int ThreadIndex)
{
    ...
    else
    {
        ...
        memcpy(      TX + ThreadIndex,       TX+0, sizeof(Tamale_TxState_t) );
        memcpy(      RX + ThreadIndex,       RX+0, sizeof(Tamale_RxState_t) );
        memcpy(     bRX + ThreadIndex,      bRX+0, sizeof(Tamale_bRxState_t));
        memcpy(      DV + ThreadIndex,       DV+0, sizeof(Tamale_DvState_t) );
        memcpy(Register + ThreadIndex, Register+0, sizeof(TamaleRegisters_t));

        Tamale_TX_FreeMemory ( TX + ThreadIndex, 1);
        Tamale_RX_FreeMemory ( RX + ThreadIndex, 1);
        Tamale_bRX_FreeMemory(bRX + ThreadIndex, 1);

        TX[ThreadIndex].LastAllocation.Fn = NULL; // force allocation on next TX

        for (i=0; i<TAMALE_MAX_DV_TX; i++)
            DV[ThreadIndex].MAC_TX_D[i] = NULL;
    }    
    return WI_SUCCESS;
}
// end of Tamale_InitializeThread()

// ================================================================================================
// FUNCTION  : Tamale_CloseThread()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_CloseThread(unsigned int ThreadIndex)
{
    if (InvalidRange(ThreadIndex, 1, WISE_MAX_THREADS)) 
        return WI_ERROR_PARAMETER1;
    else
    {
        int i;
        XSTATUS( Tamale_TX_FreeMemory ( TX + ThreadIndex, 0) )
        XSTATUS( Tamale_RX_FreeMemory ( RX + ThreadIndex, 0) )
        XSTATUS( Tamale_bRX_FreeMemory(bRX + ThreadIndex, 0) );
...
// end of Tamale_CloseThread()

// ================================================================================================
// FUNCTION  : Tamale_ReadRegisterMap()
// ------------------------------------------------------------------------------------------------
...
wiStatus Tamale_ReadRegisterMap(wiUWORD *RegMap)
{
    ...
    {
        wiWORD Zero = {0};
        Tamale_DCX(-1, Zero, NULL, 0, Tamale_RxState(), pReg);
    }
    pReg->ID.word = TAMALE_PART_ID; // force the part ID field

    return WI_SUCCESS;
}
// end of Tamale_ReadRegisterMap()

// ================================================================================================
// FUNCTION  : Tamale_WriteRegisterMap()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
wiStatus Tamale_WriteRegisterMap(wiUWORD *RegMap)
{
    TamaleRegisters_t *pReg = Tamale_Registers();

    if (wiProcess_ThreadIndex() != 0) return STATUS(WI_ERROR_INVALID_FOR_THREAD);

    pReg->ID.word = TAMALE_PART_ID; // force the part ID value
    memset( RegMap, 0, 1024 );

    //  Matlab Generated Code from Tamale_GetRegisterMap.m
    // ----------------------------------------------------------------------------------
...
// end of Tamale_WriteRegisterMap()

// ================================================================================================
// FUNCTION  : Tamale_ReadRegister()
// ------------------------------------------------------------------------------------------------
// Purpose   : Update and return the internal register map
// Parameters: pRegMap -- pointer (by reference) to the register map
// ================================================================================================
wiUInt Tamale_ReadRegister(wiUInt Addr)
{
    if (Addr < 1024)
    {
        STATUS( Tamale_WriteRegisterMap(RegMap) );      // copy parameters to register map
        return (wiUInt)(RegMap[Addr].w8);
    }
    else return 0; // address not valid
}
// end of Tamale_ReadRegister()

// ================================================================================================
// FUNCTION  : Tamale_WriteRegister()
// ------------------------------------------------------------------------------------------------
// Purpose   : Write a register by address
// ================================================================================================
wiStatus Tamale_WriteRegister(wiUInt Addr, wiUInt Data)
{
    if (wiProcess_ThreadIndex() != 0)  return STATUS(WI_ERROR_INVALID_FOR_THREAD);
    ...
    {
        XSTATUS(Tamale_WriteRegisterMap(RegMap)); // copy parameters to register map
        RegMap[Addr].word = Data;                 // write data to register map
        XSTATUS(Tamale_ReadRegisterMap(RegMap));  // load parameters from register map
    }
    return WI_SUCCESS;
}
// end of Tamale_WriteRegister()

// ================================================================================================
// FUNCTION  : Tamale_LoadRegisters()
// ------------------------------------------------------------------------------------------------
// Purpose   : Save a register map to a file
// Parameters: RegMap   -- pointer to the register map to which to parameters are written
//             Filename -- name of the file to write
// ================================================================================================
wiStatus Tamale_LoadRegisters(wiUWORD *RegMap, wiString Filename)
{
...
// end of Tamale_LoadRegisters()

// ================================================================================================
// FUNCTION  : Tamale_SaveRegisters()
// ------------------------------------------------------------------------------------------------
// Purpose   : Save a register map to a file
// Parameters: RegMap   -- pointer to the register map to which to parameters are written
//             Filename -- name of the file to write
// ================================================================================================
wiStatus Tamale_SaveRegisters(wiUWORD *RegMap, wiString Filename)
{
    int i; time_t t;
    FILE *F;
    ...
    fprintf(F,"// ----------------------------------------------------------------------------\n");
    fprintf(F,"// Settings from WiSE::Tamale: %s",ctime(&t));
    fprintf(F,"// ----------------------------------------------------------------------------\n");
...
// end of Tamale_SaveRegisters()

// ================================================================================================
// FUNCTION  : Tamale_GetRegisterMap()
// ------------------------------------------------------------------------------------------------
// Purpose   : Update and return the internal register map
// Parameters: pRegMap -- pointer (by reference) to the register map
// ================================================================================================
wiStatus Tamale_GetRegisterMap(wiUWORD **pRegMap)
{
    XSTATUS( Tamale_WriteRegisterMap(RegMap) );
    *pRegMap = RegMap;
    return WI_SUCCESS;
}
// end of Tamale_GetRegisterMap()

// ================================================================================================
// FUNCTION  : Tamale_SetRegisterMap()
// ------------------------------------------------------------------------------------------------
...
// ================================================================================================
wiStatus Tamale_SetRegisterMap(wiUInt RegData[], unsigned int NumRegs)
{
    unsigned int i;
    for (i=0; i<NumRegs; i++) 
        RegMap[i].word = RegData[i];

    XSTATUS( Tamale_ReadRegisterMap(RegMap) );
    return WI_SUCCESS;
}
// end of Tamale_SetRegisterMap()

// ================================================================================================
// FUNCTION : Tamale_Init()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_Init(void)
{
	pLookupTable = (Tamale_LookupTable_t *)wiMalloc(sizeof(Tamale_LookupTable_t));
	if (!pLookupTable) return STATUS(WI_ERROR_MEMORY_ALLOCATION);	

    XSTATUS( Tamale_TX_Init() );
    XSTATUS( Tamale_RX_Init() );
    XSTATUS( Tamale_b_Init()  );
    XSTATUS( Tamale_TX_DPD_Init() );
    XSTATUS( Tamale_RESET()   ); // power-on reset
...
// ================================================================================================
// FUNCTION : Tamale_Close()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_Close(void)
{
    unsigned int i, n;

    for (n=0; n<WISE_MAX_THREADS; n++)
    {
        XSTATUS( Tamale_TX_FreeMemory ( TX+n, 0)  );
        XSTATUS( Tamale_RX_FreeMemory ( RX+n, 0)  );
        XSTATUS( Tamale_bRX_FreeMemory(bRX+n, 0) );
        for (i=0; i<TAMALE_MAX_DV_TX; i++) 
        {
...
// end of Tamale_DvSetTxDataByte()

// ================================================================================================
// FUNCTION : Tamale_DvCopyTxHeader()
// ------------------------------------------------------------------------------------------------
// Purpose  : Copy the PHY header from pDV->PHY_TX_RD[n] to the TxHeader registers
// ================================================================================================
wiStatus Tamale_DvCopyTxHeader(wiUInt n)
{
    Tamale_DvState_t  *pDV  = Tamale_DvState();
    TamaleRegisters_t *pReg = Tamale_Registers();
...
// end of Tamale_DvCopyTxHeader()

// ================================================================================================
// FUNCTION : Tamale_RESET()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_RESET(void)
{
    /////////////////////////////////////////////////////////////////////////////////////
    //
    ...
    Tamale_TX_FreeMemory(Tamale_TxState(), 1);

    /////////////////////////////////////////////////////////////////////////////////////
    //
    //  Set Default Register Values
    //
    XSTATUS( Tamale_SetRegisterMap(DefaultRegisters, sizeof(DefaultRegisters)/sizeof(wiUWORD)) );
    ...
    {
        wiWORD Zero = {0};
        Tamale_DCX(-1, Zero, NULL, 0, Tamale_RxState(), Tamale_Registers());
    }
    return WI_SUCCESS;
}
// end of Tamale_RESET()
...
// ================================================================================================
// FUNCTION : Tamale_DvState
// ------------------------------------------------------------------------------------------------
// ================================================================================================
Tamale_DvState_t* Tamale_DvState(void)
{
    return DV + wiProcess_ThreadIndex();
}
// end of Tamale_DvState()

// ================================================================================================
// FUNCTION : Tamale_TxState
// ------------------------------------------------------------------------------------------------
// ================================================================================================
Tamale_TxState_t* Tamale_TxState(void)
{
    int ThreadIndex = wiProcess_ThreadIndex();
    Tamale_TxState_t *pTX = &(TX[ThreadIndex]);

//wiPrintf("Tamale_TxState [%d] = @x%08X [@x%08X, @x%08X, @x%08X] @x%08X\n",ThreadIndex,pTX,TX+0,TX+1,TX+2,TX+3);

    return pTX;
}
// end of Tamale_TxState()

// ================================================================================================
// FUNCTION : Tamale_RxState
// ------------------------------------------------------------------------------------------------
// ================================================================================================
Tamale_RxState_t* Tamale_RxState(void)
{
    return RX + wiProcess_ThreadIndex();
}
// end of Tamale_RxState()

// ================================================================================================
// FUNCTION : Tamale_bRxState()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
Tamale_bRxState_t * Tamale_bRxState(void)
{
    return bRX + wiProcess_ThreadIndex();
}
// end of Tamale_bRxState()

// ================================================================================================
// FUNCTION : Tamale_LookupTable()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
Tamale_LookupTable_t* Tamale_LookupTable(void)
{
    return pLookupTable;
}
// end of Tamale_LookupTable()

// ================================================================================================
// FUNCTION : Tamale_Registers
// ------------------------------------------------------------------------------------------------
// ================================================================================================
TamaleRegisters_t* Tamale_Registers(void)
{
    return Register + wiProcess_ThreadIndex();
}
// end of Tamale_Registers()

// ================================================================================================
// FUNCTION : Tamale_GetArrayByName
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_GetArrayByName(wiString Name, unsigned int n, void **ptr)
{
         if (!strcmp(Name, "TX.uReal"))          *ptr = (void *)Tamale_TxState()->uReal;
    else if (!strcmp(Name, "TX.uImag"))          *ptr = (void *)Tamale_TxState()->uImag;
    else if (!strcmp(Name, "RX.tReal"))          *ptr = (void *)Tamale_RxState()->tReal[n];
    else if (!strcmp(Name, "RX.tImag"))          *ptr = (void *)Tamale_RxState()->tImag[n];
    else if (!strcmp(Name, "RX.uReal"))          *ptr = (void *)Tamale_RxState()->uReal[n];
    else if (!strcmp(Name, "RX.uImag"))          *ptr = (void *)Tamale_RxState()->uImag[n];
    else if (!strcmp(Name, "RX.vReal"))          *ptr = (void *)Tamale_RxState()->vReal[n];
    else if (!strcmp(Name, "RX.vImag"))          *ptr = (void *)Tamale_RxState()->vImag[n];
    else if (!strcmp(Name, "RX.wReal"))          *ptr = (void *)Tamale_RxState()->wReal[n];
    else if (!strcmp(Name, "RX.wImag"))          *ptr = (void *)Tamale_RxState()->wImag[n];
    else if (!strcmp(Name, "RX.xReal"))          *ptr = (void *)Tamale_RxState()->xReal[n];
    else if (!strcmp(Name, "RX.xImag"))          *ptr = (void *)Tamale_RxState()->xImag[n];
    else if (!strcmp(Name, "RX.yReal"))          *ptr = (void *)Tamale_RxState()->yReal[n];
    else if (!strcmp(Name, "RX.yImag"))          *ptr = (void *)Tamale_RxState()->yImag[n];
    else if (!strcmp(Name, "RX.zReal"))          *ptr = (void *)Tamale_RxState()->zReal[n];
    else if (!strcmp(Name, "RX.zImag"))          *ptr = (void *)Tamale_RxState()->zImag[n];
    else if (!strcmp(Name, "RX.a"))              *ptr = (void *)Tamale_RxState()->a;
    else if (!strcmp(Name, "RX.b"))              *ptr = (void *)Tamale_RxState()->b;
    else if (!strcmp(Name, "RX.Lp"))             *ptr = (void *)Tamale_RxState()->Lp;
    else if (!strcmp(Name, "RX.Lc"))             *ptr = (void *)Tamale_RxState()->Lc;
    else if (!strcmp(Name, "RX.cA"))             *ptr = (void *)Tamale_RxState()->cA;
    else if (!strcmp(Name, "RX.cB"))             *ptr = (void *)Tamale_RxState()->cB;
    else if (!strcmp(Name, "RX.AGain"))          *ptr = (void *)Tamale_RxState()->AGain;
    else if (!strcmp(Name, "RX.DGain"))          *ptr = (void *)Tamale_RxState()->DGain;
    else if (!strcmp(Name, "RX.LNAGain"))        *ptr = (void *)Tamale_RxState()->LNAGain;
    else if (!strcmp(Name, "RX.DFEState"))       *ptr = (void *)Tamale_RxState()->DFEState;
    else if (!strcmp(Name, "RX.DFEFlags"))       *ptr = (void *)Tamale_RxState()->DFEFlags;
    else if (!strcmp(Name, "RX.AdvDly"))         *ptr = (void *)Tamale_RxState()->AdvDly;
    else if (!strcmp(Name, "RX.aSigOut"))        *ptr = (void *)Tamale_RxState()->aSigOut;
    else if (!strcmp(Name, "RX.bSigOut"))        *ptr = (void *)Tamale_RxState()->bSigOut;
    else if (!strcmp(Name, "RX.traceRxClock"))   *ptr = (void *)Tamale_RxState()->traceRxClock;
    else if (!strcmp(Name, "RX.traceRxState"))   *ptr = (void *)Tamale_RxState()->traceRxState;
    else if (!strcmp(Name, "RX.traceRxControl")) *ptr = (void *)Tamale_RxState()->traceRxControl;
    else if (!strcmp(Name, "RX.traceDFE"))       *ptr = (void *)Tamale_RxState()->traceDFE;
    else if (!strcmp(Name, "RX.traceAGC"))       *ptr = (void *)Tamale_RxState()->traceAGC;
    else if (!strcmp(Name, "RX.traceRSSI"))      *ptr = (void *)Tamale_RxState()->traceRSSI;
    else if (!strcmp(Name, "RX.traceDFS"))       *ptr = (void *)Tamale_RxState()->traceDFS;
    else if (!strcmp(Name, "RX.traceSigDet"))    *ptr = (void *)Tamale_RxState()->traceSigDet;
    else if (!strcmp(Name, "RX.traceFrameSync")) *ptr = (void *)Tamale_RxState()->traceFrameSync;
    else if (!strcmp(Name, "RX.traceRadioIO"))   *ptr = (void *)Tamale_RxState()->traceRadioIO;
    else if (!strcmp(Name, "RX.traceDataConv"))  *ptr = (void *)Tamale_RxState()->traceDataConv;
    else if (!strcmp(Name, "RX.traceRX2"))       *ptr = (void *)Tamale_RxState()->traceRX2;
    else if (!strcmp(Name, "RX.traceRx"))        *ptr = (void *)Tamale_RxState()->traceRx;
    else if (!strcmp(Name, "RX.traceImgDet"))    *ptr = (void *)Tamale_RxState()->traceImgDet;
    else if (!strcmp(Name, "RX.traceIQCal"))     *ptr = (void *)Tamale_RxState()->traceIQCal;
    else if (!strcmp(Name, "RX.PHY_RX_D"))       *ptr = (void *)Tamale_RxState()->PHY_RX_D;
    else if (!strcmp(Name, "bRX.xReal"))         *ptr = (void *)Tamale_bRxState()->xReal;
    else if (!strcmp(Name, "bRX.xImag"))         *ptr = (void *)Tamale_bRxState()->xImag;
    else if (!strcmp(Name, "bRX.uReal"))         *ptr = (void *)Tamale_bRxState()->uReal;
    else if (!strcmp(Name, "bRX.uImag"))         *ptr = (void *)Tamale_bRxState()->uImag;
    else if (!strcmp(Name, "bRX.traceCCK"))      *ptr = (void *)Tamale_bRxState()->traceCCK;
    else if (!strcmp(Name, "bRX.traceState"))    *ptr = (void *)Tamale_bRxState()->traceState;
    else if (!strcmp(Name, "bRX.traceControl"))  *ptr = (void *)Tamale_bRxState()->traceControl;
    else if (!strcmp(Name, "bRX.traceFrontReg")) *ptr = (void *)Tamale_bRxState()->traceFrontReg;
    else if (!strcmp(Name, "bRX.traceDPLL"))     *ptr = (void *)Tamale_bRxState()->traceDPLL;
    else if (!strcmp(Name, "bRX.traceBarker"))   *ptr = (void *)Tamale_bRxState()->traceBarker;
    else if (!strcmp(Name, "bRX.traceDSSS"))     *ptr = (void *)Tamale_bRxState()->traceDSSS;
    else if (!strcmp(Name, "bRX.traceCCKInput")) *ptr = (void *)Tamale_bRxState()->traceCCKInput;
    else if (!strcmp(Name, "bRX.traceCCKTVMF"))  *ptr = (void *)Tamale_bRxState()->traceCCKTVMF;
    else if (!strcmp(Name, "bRX.traceCCKFWT"))   *ptr = (void *)Tamale_bRxState()->traceCCKFWT;
    else if (!strcmp(Name, "bRX.traceResample")) *ptr = (void *)Tamale_bRxState()->traceResample;
    else return WI_ERROR_PARAMETER1;

    return WI_SUCCESS;
}
// end of Tamale_GetArrayByName()

// ================================================================================================
// FUNCTION : Tamale_InvertArrayBit
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_InvertArrayBit(wiString Name, unsigned int n, unsigned int k, unsigned int m)
{
    void *ptr;
    wiUInt *pU;

    if (n > 1)                               return STATUS(WI_ERROR_PARAMETER2);
    if (k > (unsigned)Tamale_RxState()->N80) return STATUS(WI_ERROR_PARAMETER3);
    if (m > 31)                              return STATUS(WI_ERROR_PARAMETER4);

    XSTATUS( Tamale_GetArrayByName(Name, n, &ptr) );
...
// end of Tamale_InvertArrayBit()

// ================================================================================================
// FUNCTION : Tamale_SetArrayWord
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_SetArrayWord(wiString Name, unsigned int n, unsigned int k, unsigned int x)
{
    void *ptr;
    wiUInt *pU;
...
// end of Tamale_SetArrayWord()

// ================================================================================================
// FUNCTION : Tamale_WriteConfig()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_WriteConfig(wiMessageFn MsgFunc)
{
    ...
    TamaleRegisters_t *p   = Tamale_Registers();
...
// end of Tamale_WriteConfig()

// ================================================================================================
// FUNCTION : Tamale_ParseLine()
// ------------------------------------------------------------------------------------------------
// ================================================================================================
wiStatus Tamale_ParseLine(wiString text)
{
    if (strstr(text, "Tamale"))
    {
        wiStatus status;
        ...
        //  Matlab Generated Code from Tamale_GetRegisterMap(16)
        // ------------------------------------------------------------------------------
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ID",              &(pReg->ID.word),               0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.PathSel",         &(pReg->PathSel.word),          0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RXMode",          &(pReg->RXMode.word),           0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TestMode",        &(pReg->TestMode.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.UpmixConj",       &(pReg->UpmixConj.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DownmixConj",     &(pReg->DownmixConj.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.UpmixDown",       &(pReg->UpmixDown.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DownmixUp",       &(pReg->DownmixUp.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.UpmixOff",        &(pReg->UpmixOff.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DownmixOff",      &(pReg->DownmixOff.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxIQSrcR",        &(pReg->TxIQSrcR.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxIQSrcI",        &(pReg->TxIQSrcI.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.PrdDACSrcR",      &(pReg->PrdDACSrcR.word),       0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.PrdDACSrcI",      &(pReg->PrdDACSrcI.word),       0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aTxExtend",       &(pReg->aTxExtend.word),        0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aTxDelay",        &(pReg->aTxDelay.word),         0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bTxExtend",       &(pReg->bTxExtend.word),        0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bTxDelay",        &(pReg->bTxDelay.word),         0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxIQSrcRun",      &(pReg->TxIQSrcRun.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxSmoothing",     &(pReg->TxSmoothing.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TX_SERVICE",      &(pReg->TX_SERVICE.word),       0, 65535));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TX_PRBS",         &(pReg->TX_PRBS.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RX_SERVICE",      &(pReg->RX_SERVICE.word),       0, 65535));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RSSI0",           &(pReg->RSSI0.word),            0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RSSI1",           &(pReg->RSSI1.word),            0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.maxLENGTH",       &(pReg->maxLENGTH.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.NotSounding",     &(pReg->NotSounding.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.nReserved1",      &(pReg->nReserved1.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Reserved0",       &(pReg->Reserved0.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.maxLENGTHnH",     &(pReg->maxLENGTHnH.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.maxLENGTHnL",     &(pReg->maxLENGTHnL.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aPPDUMaxTimeH",   &(pReg->aPPDUMaxTimeH.word),    0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aPPDUMaxTimeL",   &(pReg->aPPDUMaxTimeL.word),    0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.PilotPLLEn",      &(pReg->PilotPLLEn.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aC",              &(pReg->aC.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bC",              &(pReg->bC.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.cC",              &(pReg->cC.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Sa",              &(pReg->Sa.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Sb",              &(pReg->Sb.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Ca",              &(pReg->Ca.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Cb",              &(pReg->Cb.word),               0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.sTxIQ",           &(pReg->sTxIQ.word),            0, 1));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.TxIQa11",         &(pReg->TxIQa11.word),         -16, 15));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.TxIQa22",         &(pReg->TxIQa22.word),         -16, 15));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.TxIQa12",         &(pReg->TxIQa12.word),         -16, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.sRxIQ",           &(pReg->sRxIQ.word),            0, 3));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.RxIQ0a11",        &(pReg->RxIQ0a11.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.RxIQ0a22",        &(pReg->RxIQ0a22.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.RxIQ0a12",        &(pReg->RxIQ0a12.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.RxIQ1a11",        &(pReg->RxIQ1a11.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.RxIQ1a22",        &(pReg->RxIQ1a22.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.RxIQ1a12",        &(pReg->RxIQ1a12.word),        -16, 15));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.ImgDetCoefRH",    &(pReg->ImgDetCoefRH.word),    -8, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetCoefRL",    &(pReg->ImgDetCoefRL.word),     0, 255));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.ImgDetCoefIH",    &(pReg->ImgDetCoefIH.word),    -8, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetCoefIL",    &(pReg->ImgDetCoefIL.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetLength",    &(pReg->ImgDetLength.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetEn",        &(pReg->ImgDetEn.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetLengthH",   &(pReg->ImgDetLengthH.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetPostShift", &(pReg->ImgDetPostShift.word),  0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetPreShift",  &(pReg->ImgDetPreShift.word),   0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetDelay",     &(pReg->ImgDetDelay.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetAvgNum",    &(pReg->ImgDetAvgNum.word),     0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetPowerH",    &(pReg->ImgDetPowerH.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetPowerL",    &(pReg->ImgDetPowerL.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalDone",         &(pReg->CalDone.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalResultType",   &(pReg->CalResultType.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ImgDetDone",      &(pReg->ImgDetDone.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalRun",          &(pReg->CalRun.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalPwrAdjust",    &(pReg->CalPwrAdjust.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalMode",         &(pReg->CalMode.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalCoefUpdate",   &(pReg->CalCoefUpdate.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalFineStep",     &(pReg->CalFineStep.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalCoarseStep",   &(pReg->CalCoarseStep.word),    0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalPwrRatio",     &(pReg->CalPwrRatio.word),      0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalIterationTh",  &(pReg->CalIterationTh.word),   0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalDelay",        &(pReg->CalDelay.word),         0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalGain",         &(pReg->CalGain.word),          0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CalPhase",        &(pReg->CalPhase.word),         0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RefPwrDig",       &(pReg->RefPwrDig.word),        0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.InitAGain",       &(pReg->InitAGain.word),        0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.AbsPwrL",         &(pReg->AbsPwrL.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.AbsPwrH",         &(pReg->AbsPwrH.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThSigLarge",      &(pReg->ThSigLarge.word),       0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThSigSmall",      &(pReg->ThSigSmall.word),       0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.StepLgSigDet",    &(pReg->StepLgSigDet.word),     0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.StepLarge",       &(pReg->StepLarge.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.StepSmall",       &(pReg->StepSmall.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThSwitchLNA",     &(pReg->ThSwitchLNA.word),      0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.StepLNA",         &(pReg->StepLNA.word),          0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThSwitchLNA2",    &(pReg->ThSwitchLNA2.word),     0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.StepLNA2",        &(pReg->StepLNA2.word),         0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Pwr100dBm",       &(pReg->Pwr100dBm.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.StepSmallPeak",   &(pReg->StepSmallPeak.word),    0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.FixedGain",       &(pReg->FixedGain.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.InitLNAGain",     &(pReg->InitLNAGain.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.InitLNAGain2",    &(pReg->InitLNAGain2.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.UpdateOnLNA",     &(pReg->UpdateOnLNA.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.MaxUpdateCount",  &(pReg->MaxUpdateCount.word),   0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThStepUpRefPwr",  &(pReg->ThStepUpRefPwr.word),   0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThStepUp",        &(pReg->ThStepUp.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThStepDown",      &(pReg->ThStepDown.word),       0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThDFSUp",         &(pReg->ThDFSUp.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThDFSDn",         &(pReg->ThDFSDn.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThStepDownMsrPwr",&(pReg->ThStepDownMsrPwr.word), 0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSOn",           &(pReg->DFSOn.word),            0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFS2Candidates",  &(pReg->DFS2Candidates.word),   0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSSmooth",       &(pReg->DFSSmooth.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSMACFilter",    &(pReg->DFSMACFilter.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSHdrFilter",    &(pReg->DFSHdrFilter.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSIntDet",       &(pReg->DFSIntDet.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSPattern",      &(pReg->DFSPattern.word),       0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSminPPB",       &(pReg->DFSminPPB.word),        0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSdTPRF",        &(pReg->DFSdTPRF.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSmaxWidth",     &(pReg->DFSmaxWidth.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSmaxTPRF",      &(pReg->DFSmaxTPRF.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSminTPRF",      &(pReg->DFSminTPRF.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSminGap",       &(pReg->DFSminGap.word),        0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSPulseSR",      &(pReg->DFSPulseSR.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.detRadar",        &(pReg->detRadar.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.detHeader",       &(pReg->detHeader.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.detPreamble",     &(pReg->detPreamble.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.detStepDown",     &(pReg->detStepDown.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.detStepUp",       &(pReg->detStepUp.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.WaitAGC",         &(pReg->WaitAGC.word),          0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayAGC",        &(pReg->DelayAGC.word),         0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SigDetWindow",    &(pReg->SigDetWindow.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SyncShift",       &(pReg->SyncShift.word),        0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayRestart",    &(pReg->DelayRestart.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.OFDMSwDDelay",    &(pReg->OFDMSwDDelay.word),     0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.OFDMSwDTh",       &(pReg->OFDMSwDTh.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CFOFilter",       &(pReg->CFOFilter.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.UpdateGainMF",    &(pReg->UpdateGainMF.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ArmLgSigDet3",    &(pReg->ArmLgSigDet3.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.LgSigAFEValid",   &(pReg->LgSigAFEValid.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.IdleFSEnB",       &(pReg->IdleFSEnB.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.IdleSDEnB",       &(pReg->IdleSDEnB.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.IdleDAEnB",       &(pReg->IdleDAEnB.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SDEnB17",         &(pReg->SDEnB17.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayCFO",        &(pReg->DelayCFO.word),         0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayRSSI",       &(pReg->DelayRSSI.word),        0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayRIFS",       &(pReg->DelayRIFS.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.OFDMSwDEn",       &(pReg->OFDMSwDEn.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.OFDMSwDSave",     &(pReg->OFDMSwDSave.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.OFDMSwDInit",     &(pReg->OFDMSwDInit.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.UpdateGain11b",   &(pReg->UpdateGain11b.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.OFDMSwDThLow",    &(pReg->OFDMSwDThLow.word),     0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SigDetTh1",       &(pReg->SigDetTh1.word),        0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SigDetTh2H",      &(pReg->SigDetTh2H.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SigDetTh2L",      &(pReg->SigDetTh2L.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SigDetDelay",     &(pReg->SigDetDelay.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SigDetFilter",    &(pReg->SigDetFilter.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SyncFilter",      &(pReg->SyncFilter.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SyncMag",         &(pReg->SyncMag.word),          0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SyncThSig",       &(pReg->SyncThSig.word),        0, 15));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.SyncThExp",       &(pReg->SyncThExp.word),       -8, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SyncDelay",       &(pReg->SyncDelay.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.MinShift",        &(pReg->MinShift.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ChEstShift0",     &(pReg->ChEstShift0.word),      0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ChEstShift1",     &(pReg->ChEstShift1.word),      0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ChTracking",      &(pReg->ChTracking.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ChTrackGain1",    &(pReg->ChTrackGain1.word),     0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ChTrackGain2",    &(pReg->ChTrackGain2.word),     0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aC1",             &(pReg->aC1.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bC1",             &(pReg->bC1.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aS1",             &(pReg->aS1.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bS1",             &(pReg->bS1.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aC2",             &(pReg->aC2.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bC2",             &(pReg->bC2.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aS2",             &(pReg->aS2.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bS2",             &(pReg->bS2.word),              0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxToneDiv",       &(pReg->TxToneDiv.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxWordRH",        &(pReg->TxWordRH.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxWordRL",        &(pReg->TxWordRL.word),         0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxWordIH",        &(pReg->TxWordIH.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxWordIL",        &(pReg->TxWordIL.word),         0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DCXSelect",       &(pReg->DCXSelect.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DCXFastLoad",     &(pReg->DCXFastLoad.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DCXHoldAcc",      &(pReg->DCXHoldAcc.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DCXShift",        &(pReg->DCXShift.word),         0, 7));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.DCXAcc0RH",       &(pReg->DCXAcc0RH.word),       -16, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DCXAcc0RL",       &(pReg->DCXAcc0RL.word),        0, 255));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.DCXAcc0IH",       &(pReg->DCXAcc0IH.word),       -16, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DCXAcc0IL",       &(pReg->DCXAcc0IL.word),        0, 255));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.DCXAcc1RH",       &(pReg->DCXAcc1RH.word),       -16, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DCXAcc1RL",       &(pReg->DCXAcc1RL.word),        0, 255));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.DCXAcc1IH",       &(pReg->DCXAcc1IH.word),       -16, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DCXAcc1IL",       &(pReg->DCXAcc1IL.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.INITdelay",       &(pReg->INITdelay.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.EDwindow",        &(pReg->EDwindow.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CQwindow",        &(pReg->CQwindow.word),         0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SSwindow",        &(pReg->SSwindow.word),         0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CQthreshold",     &(pReg->CQthreshold.word),      0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.EDthreshold",     &(pReg->EDthreshold.word),      0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bWaitAGC",        &(pReg->bWaitAGC.word),         0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bAGCdelay",       &(pReg->bAGCdelay.word),        0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bRefPwr",         &(pReg->bRefPwr.word),          0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bInitAGain",      &(pReg->bInitAGain.word),       0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bThSigLarge",     &(pReg->bThSigLarge.word),      0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bStepLarge",      &(pReg->bStepLarge.word),       0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bThSwitchLNA",    &(pReg->bThSwitchLNA.word),     0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bStepLNA",        &(pReg->bStepLNA.word),         0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bThSwitchLNA2",   &(pReg->bThSwitchLNA2.word),    0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bStepLNA2",       &(pReg->bStepLNA2.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bPwr100dBm",      &(pReg->bPwr100dBm.word),       0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DAmpGainRange",   &(pReg->DAmpGainRange.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bMaxUpdateCount", &(pReg->bMaxUpdateCount.word),  0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CSMode",          &(pReg->CSMode.word),           0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bPwrStepDet",     &(pReg->bPwrStepDet.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bCFOQual",        &(pReg->bCFOQual.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bAGCBounded4",    &(pReg->bAGCBounded4.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bLgSigAFEValid",  &(pReg->bLgSigAFEValid.word),   0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bSwDEn",          &(pReg->bSwDEn.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bSwDSave",        &(pReg->bSwDSave.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bSwDInit",        &(pReg->bSwDInit.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bSwDDelay",       &(pReg->bSwDDelay.word),        0, 31));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bSwDTh",          &(pReg->bSwDTh.word),           0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bThStepUpRefPwr", &(pReg->bThStepUpRefPwr.word),  0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bThStepUp",       &(pReg->bThStepUp.word),        0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bThStepDown",     &(pReg->bThStepDown.word),      0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bRestartDelay",   &(pReg->bRestartDelay.word),    0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SymHDRCE",        &(pReg->SymHDRCE.word),         0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CENSym1",         &(pReg->CENSym1.word),          0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CENSym2",         &(pReg->CENSym2.word),          0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.hThreshold1",     &(pReg->hThreshold1.word),      0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.hThreshold2",     &(pReg->hThreshold2.word),      0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.hThreshold3",     &(pReg->hThreshold3.word),      0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SFDWindow",       &(pReg->SFDWindow.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.AntSel",          &(pReg->AntSel.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DW_PHYEnB",       &(pReg->DW_PHYEnB.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TopState",        &(pReg->TopState.word),         0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.CFO",             &(pReg->CFO.word),              0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxFaultCount",    &(pReg->RxFaultCount.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RestartCount",    &(pReg->RestartCount.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.NuisanceCount",   &(pReg->NuisanceCount.word),    0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.sPHYEnB",         &(pReg->sPHYEnB.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.PHYEnB",          &(pReg->PHYEnB.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.NoStepUpAgg",     &(pReg->NoStepUpAgg.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RSSIdB",          &(pReg->RSSIdB.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.UnsupportedGF",   &(pReg->UnsupportedGF.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ClrOnWake",       &(pReg->ClrOnWake.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ClrOnHdr",        &(pReg->ClrOnHdr.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ClrNow",          &(pReg->ClrNow.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Filter64QAM",     &(pReg->Filter64QAM.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Filter16QAM",     &(pReg->Filter16QAM.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.FilterQPSK",      &(pReg->FilterQPSK.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.FilterBPSK",      &(pReg->FilterBPSK.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.FilterUndecoded", &(pReg->FilterUndecoded.word),  0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.FilterAllTypes",  &(pReg->FilterAllTypes.word),   0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.AddrFilter",      &(pReg->AddrFilter.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.STA0",            &(pReg->STA0.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.STA1",            &(pReg->STA1.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.STA2",            &(pReg->STA2.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.STA3",            &(pReg->STA3.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.STA4",            &(pReg->STA4.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.STA5",            &(pReg->STA5.word),             0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.minRSSI",         &(pReg->minRSSI.word),          0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.StepUpRestart",   &(pReg->StepUpRestart.word),    0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.StepDownRestart", &(pReg->StepDownRestart.word),  0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.NoStepAfterSync", &(pReg->NoStepAfterSync.word),  0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.NoStepAfterHdr",  &(pReg->NoStepAfterHdr.word),   0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxRIFS",          &(pReg->RxRIFS.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ForceRestart",    &(pReg->ForceRestart.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.KeepCCA_New",     &(pReg->KeepCCA_New.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.KeepCCA_Lost",    &(pReg->KeepCCA_Lost.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.PktTimePhase80",  &(pReg->PktTimePhase80.word),   0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.WakeupTimeH",     &(pReg->WakeupTimeH.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.WakeupTimeL",     &(pReg->WakeupTimeL.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayStdby",      &(pReg->DelayStdby.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayBGEnB",      &(pReg->DelayBGEnB.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayADC1",       &(pReg->DelayADC1.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayADC2",       &(pReg->DelayADC2.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayModem",      &(pReg->DelayModem.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayDFE",        &(pReg->DelayDFE.word),         0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayRFSW",       &(pReg->DelayRFSW.word),        0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxRxTime",        &(pReg->TxRxTime.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayFCAL1",      &(pReg->DelayFCAL1.word),       0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayPA",         &(pReg->DelayPA.word),          0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayFCAL2",      &(pReg->DelayFCAL2.word),       0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayState51",    &(pReg->DelayState51.word),     0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayShutdown",   &(pReg->DelayShutdown.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelaySleep",      &(pReg->DelaySleep.word),       0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TimeExtend",      &(pReg->TimeExtend.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SquelchTime",     &(pReg->SquelchTime.word),      0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelaySquelchRF",  &(pReg->DelaySquelchRF.word),   0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DelayClearDCX",   &(pReg->DelayClearDCX.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RadioMC",         &(pReg->RadioMC.word),          0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DACOPM",          &(pReg->DACOPM.word),           0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCAOPM",         &(pReg->ADCAOPM.word),          0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCBOPM",         &(pReg->ADCBOPM.word),          0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RFSW1",           &(pReg->RFSW1.word),            0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RFSW2",           &(pReg->RFSW2.word),            0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.LNAEnB",          &(pReg->LNAEnB.word),           0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCBCTRLR",       &(pReg->ADCBCTRLR.word),        0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCBCTRL",        &(pReg->ADCBCTRL.word),         0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.sPA50",           &(pReg->sPA50.word),            0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.sPA24",           &(pReg->sPA24.word),            0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCFSCTRL",       &(pReg->ADCFSCTRL.word),        0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCCHSEL",        &(pReg->ADCCHSEL.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCOE",           &(pReg->ADCOE.word),            0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.LNAxxBOnSW2",     &(pReg->LNAxxBOnSW2.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCOFCEN",        &(pReg->ADCOFCEN.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCDCREN",        &(pReg->ADCDCREN.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCMUX0",         &(pReg->ADCMUX0.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCARDY",         &(pReg->ADCARDY.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCBRDY",         &(pReg->ADCBRDY.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.PPSEL_MODE",      &(pReg->PPSEL_MODE.word),       0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCFCAL2",        &(pReg->ADCFCAL2.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCFCAL1",        &(pReg->ADCFCAL1.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCFCAL",         &(pReg->ADCFCAL.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.VCMEN",           &(pReg->VCMEN.word),            0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DACIFS_CTRL",     &(pReg->DACIFS_CTRL.word),      0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DACFG_CTRL0",     &(pReg->DACFG_CTRL0.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DACFG_CTRL1",     &(pReg->DACFG_CTRL1.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RESETB",          &(pReg->RESETB.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxLNAGAIN2",      &(pReg->TxLNAGAIN2.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SREdge",          &(pReg->SREdge.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SRFreq",          &(pReg->SRFreq.word),           0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.sLNA24A",         &(pReg->sLNA24A.word),          0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.sLNA24B",         &(pReg->sLNA24B.word),          0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.sLNA50A",         &(pReg->sLNA50A.word),          0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.sLNA50B",         &(pReg->sLNA50B.word),          0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SelectCCA",       &(pReg->SelectCCA.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThCCA_SigDet",    &(pReg->ThCCA_SigDet.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThCCA_RSSI1",     &(pReg->ThCCA_RSSI1.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThCCA_RSSI2",     &(pReg->ThCCA_RSSI2.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThCCA_GF1",       &(pReg->ThCCA_GF1.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ThCCA_GF2",       &(pReg->ThCCA_GF2.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ExtendBusyGF",    &(pReg->ExtendBusyGF.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ExtendRxError",   &(pReg->ExtendRxError.word),    0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ExtendRxError1",  &(pReg->ExtendRxError1.word),   0, 255));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t0_r_s0", &(pReg->P1_coef_t0_r_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t0_i_s0", &(pReg->P1_coef_t0_i_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t1_r_s0", &(pReg->P1_coef_t1_r_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t1_i_s0", &(pReg->P1_coef_t1_i_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t2_r_s0", &(pReg->P1_coef_t2_r_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t2_i_s0", &(pReg->P1_coef_t2_i_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t3_r_s0", &(pReg->P1_coef_t3_r_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t3_i_s0", &(pReg->P1_coef_t3_i_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t4_r_s0", &(pReg->P1_coef_t4_r_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t4_i_s0", &(pReg->P1_coef_t4_i_s0.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t0_r_s0", &(pReg->P3_coef_t0_r_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t0_i_s0", &(pReg->P3_coef_t0_i_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t1_r_s0", &(pReg->P3_coef_t1_r_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t1_i_s0", &(pReg->P3_coef_t1_i_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t2_r_s0", &(pReg->P3_coef_t2_r_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t2_i_s0", &(pReg->P3_coef_t2_i_s0.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t0_r_s0", &(pReg->P5_coef_t0_r_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t0_i_s0", &(pReg->P5_coef_t0_i_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t1_r_s0", &(pReg->P5_coef_t1_r_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t1_i_s0", &(pReg->P5_coef_t1_i_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t2_r_s0", &(pReg->P5_coef_t2_r_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t2_i_s0", &(pReg->P5_coef_t2_i_s0.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t0_r_s1", &(pReg->P1_coef_t0_r_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t0_i_s1", &(pReg->P1_coef_t0_i_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t1_r_s1", &(pReg->P1_coef_t1_r_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t1_i_s1", &(pReg->P1_coef_t1_i_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t2_r_s1", &(pReg->P1_coef_t2_r_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t2_i_s1", &(pReg->P1_coef_t2_i_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t3_r_s1", &(pReg->P1_coef_t3_r_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t3_i_s1", &(pReg->P1_coef_t3_i_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t4_r_s1", &(pReg->P1_coef_t4_r_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t4_i_s1", &(pReg->P1_coef_t4_i_s1.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t0_r_s1", &(pReg->P3_coef_t0_r_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t0_i_s1", &(pReg->P3_coef_t0_i_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t1_r_s1", &(pReg->P3_coef_t1_r_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t1_i_s1", &(pReg->P3_coef_t1_i_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t2_r_s1", &(pReg->P3_coef_t2_r_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t2_i_s1", &(pReg->P3_coef_t2_i_s1.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t0_r_s1", &(pReg->P5_coef_t0_r_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t0_i_s1", &(pReg->P5_coef_t0_i_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t1_r_s1", &(pReg->P5_coef_t1_r_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t1_i_s1", &(pReg->P5_coef_t1_i_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t2_r_s1", &(pReg->P5_coef_t2_r_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t2_i_s1", &(pReg->P5_coef_t2_i_s1.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t0_r_s2", &(pReg->P1_coef_t0_r_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t0_i_s2", &(pReg->P1_coef_t0_i_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t1_r_s2", &(pReg->P1_coef_t1_r_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t1_i_s2", &(pReg->P1_coef_t1_i_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t2_r_s2", &(pReg->P1_coef_t2_r_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t2_i_s2", &(pReg->P1_coef_t2_i_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t3_r_s2", &(pReg->P1_coef_t3_r_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t3_i_s2", &(pReg->P1_coef_t3_i_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t4_r_s2", &(pReg->P1_coef_t4_r_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t4_i_s2", &(pReg->P1_coef_t4_i_s2.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t0_r_s2", &(pReg->P3_coef_t0_r_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t0_i_s2", &(pReg->P3_coef_t0_i_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t1_r_s2", &(pReg->P3_coef_t1_r_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t1_i_s2", &(pReg->P3_coef_t1_i_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t2_r_s2", &(pReg->P3_coef_t2_r_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t2_i_s2", &(pReg->P3_coef_t2_i_s2.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t0_r_s2", &(pReg->P5_coef_t0_r_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t0_i_s2", &(pReg->P5_coef_t0_i_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t1_r_s2", &(pReg->P5_coef_t1_r_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t1_i_s2", &(pReg->P5_coef_t1_i_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t2_r_s2", &(pReg->P5_coef_t2_r_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t2_i_s2", &(pReg->P5_coef_t2_i_s2.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t0_r_s3", &(pReg->P1_coef_t0_r_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t0_i_s3", &(pReg->P1_coef_t0_i_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t1_r_s3", &(pReg->P1_coef_t1_r_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t1_i_s3", &(pReg->P1_coef_t1_i_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t2_r_s3", &(pReg->P1_coef_t2_r_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t2_i_s3", &(pReg->P1_coef_t2_i_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t3_r_s3", &(pReg->P1_coef_t3_r_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t3_i_s3", &(pReg->P1_coef_t3_i_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t4_r_s3", &(pReg->P1_coef_t4_r_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P1_coef_t4_i_s3", &(pReg->P1_coef_t4_i_s3.word), -4096, 4095));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t0_r_s3", &(pReg->P3_coef_t0_r_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t0_i_s3", &(pReg->P3_coef_t0_i_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t1_r_s3", &(pReg->P3_coef_t1_r_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t1_i_s3", &(pReg->P3_coef_t1_i_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t2_r_s3", &(pReg->P3_coef_t2_r_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P3_coef_t2_i_s3", &(pReg->P3_coef_t2_i_s3.word), -16384, 16383));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t0_r_s3", &(pReg->P5_coef_t0_r_s3.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t0_i_s3", &(pReg->P5_coef_t0_i_s3.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t1_r_s3", &(pReg->P5_coef_t1_r_s3.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t1_i_s3", &(pReg->P5_coef_t1_i_s3.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t2_r_s3", &(pReg->P5_coef_t2_r_s3.word), -2048, 2047));
        PSTATUS(wiParse_Int    (text,"Tamale.Register.P5_coef_t2_i_s3", &(pReg->P5_coef_t2_i_s3.word), -2048, 2047));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.cross_pwr1",      &(pReg->cross_pwr1.word),       0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.cross_pwr2",      &(pReg->cross_pwr2.word),       0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.cross_pwr3",      &(pReg->cross_pwr3.word),       0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Tx_Rate_OFDM_HT", &(pReg->Tx_Rate_OFDM_HT.word),  0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Tx_Rate_OFDM_LL", &(pReg->Tx_Rate_OFDM_LL.word),  0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Tx_Rate_OFDM_LM", &(pReg->Tx_Rate_OFDM_LM.word),  0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DPD_spare",       &(pReg->DPD_spare.word),        0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DPD_LOW_TMP_DIS", &(pReg->DPD_LOW_TMP_DIS.word),  0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DPD_PA_tmp",      &(pReg->DPD_PA_tmp.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DPD_LOW_RATE_DIS",&(pReg->DPD_LOW_RATE_DIS.word), 0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DPD_coef_interspersion",&(pReg->DPD_coef_interspersion.word), 0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Cbc_coef_sel",    &(pReg->Cbc_coef_sel.word),     0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.cnfg_eovr",       &(pReg->cnfg_eovr.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Coef_sel_vovr",   &(pReg->Coef_sel_vovr.word),    0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Vfb_byp_vovr",    &(pReg->Vfb_byp_vovr.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Pa_tmp_ext",      &(pReg->Pa_tmp_ext.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTDone",        &(pReg->BISTDone.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTADCSel",      &(pReg->BISTADCSel.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTNoPrechg",    &(pReg->BISTNoPrechg.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTFGSel",       &(pReg->BISTFGSel.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTMode",        &(pReg->BISTMode.word),         0, 7));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.StartBIST",       &(pReg->StartBIST.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTPipe",        &(pReg->BISTPipe.word),         0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTN_val",       &(pReg->BISTN_val.word),        0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTOfsR",        &(pReg->BISTOfsR.word),         0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTOfsI",        &(pReg->BISTOfsI.word),         0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTTh1",         &(pReg->BISTTh1.word),          0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTTh2",         &(pReg->BISTTh2.word),          0, 65535));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTFG_CTRLI",    &(pReg->BISTFG_CTRLI.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTFG_CTRLR",    &(pReg->BISTFG_CTRLR.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTStatus",      &(pReg->BISTStatus.word),       0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTAccR",        &(pReg->BISTAccR.word),         0, 16777215));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTAccI",        &(pReg->BISTAccI.word),         0, 16777215));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTRangeLimH",   &(pReg->BISTRangeLimH.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.BISTRangeLimL",   &(pReg->BISTRangeLimL.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSIntEOB",       &(pReg->DFSIntEOB.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSIntFIFO",      &(pReg->DFSIntFIFO.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSThFIFO",       &(pReg->DFSThFIFO.word),        0, 127));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSDepthFIFO",    &(pReg->DFSDepthFIFO.word),     0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSReadFIFO",     &(pReg->DFSReadFIFO.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.DFSSyncFIFO",     &(pReg->DFSSyncFIFO.word),      0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxHeader1",       &(pReg->RxHeader1.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxHeader2",       &(pReg->RxHeader2.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxHeader3",       &(pReg->RxHeader3.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxHeader4",       &(pReg->RxHeader4.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxHeader5",       &(pReg->RxHeader5.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxHeader6",       &(pReg->RxHeader6.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxHeader7",       &(pReg->RxHeader7.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxHeader8",       &(pReg->RxHeader8.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxHeader1",       &(pReg->TxHeader1.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxHeader2",       &(pReg->TxHeader2.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxHeader3",       &(pReg->TxHeader3.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxHeader4",       &(pReg->TxHeader4.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxHeader5",       &(pReg->TxHeader5.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxHeader6",       &(pReg->TxHeader6.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxHeader7",       &(pReg->TxHeader7.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxHeader8",       &(pReg->TxHeader8.word),        0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TestSigEn",       &(pReg->TestSigEn.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCAI",           &(pReg->ADCAI.word),            0, 1023));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCAQ",           &(pReg->ADCAQ.word),            0, 1023));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCBI",           &(pReg->ADCBI.word),            0, 1023));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ADCBQ",           &(pReg->ADCBQ.word),            0, 1023));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bHeaderValid",    &(pReg->bHeaderValid.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bSyncFound",      &(pReg->bSyncFound.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bAGCDone",        &(pReg->bAGCDone.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.bSigDet",         &(pReg->bSigDet.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aHeaderValid",    &(pReg->aHeaderValid.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aSyncFound",      &(pReg->aSyncFound.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aAGCDone",        &(pReg->aAGCDone.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.aSigDet",         &(pReg->aSigDet.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SelectSignal2",   &(pReg->SelectSignal2.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SelectSignal1",   &(pReg->SelectSignal1.word),    0, 15));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SelectTrigger2",  &(pReg->SelectTrigger2.word),   0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.SelectTrigger1",  &(pReg->SelectTrigger1.word),   0, 3));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TriggerState1",   &(pReg->TriggerState1.word),    0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TriggerState2",   &(pReg->TriggerState2.word),    0, 63));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TriggerRSSI",     &(pReg->TriggerRSSI.word),      0, 255));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.LgSigAFE",        &(pReg->LgSigAFE.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Trigger2",        &(pReg->Trigger2.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Trigger1",        &(pReg->Trigger1.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Signal2",         &(pReg->Signal2.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Signal1",         &(pReg->Signal1.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.ClearInt",        &(pReg->ClearInt.word),         0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.IntEvent2",       &(pReg->IntEvent2.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.IntEvent1",       &(pReg->IntEvent1.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.IntRxFault",      &(pReg->IntRxFault.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.IntRxAbortTx",    &(pReg->IntRxAbortTx.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.IntTxSleep",      &(pReg->IntTxSleep.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.IntTxAbort",      &(pReg->IntTxAbort.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.IntTxFault",      &(pReg->IntTxFault.word),       0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Interrupt",       &(pReg->Interrupt.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Event2",          &(pReg->Event2.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.Event1",          &(pReg->Event1.word),           0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxFault",         &(pReg->RxFault.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.RxAbortTx",       &(pReg->RxAbortTx.word),        0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxSleep",         &(pReg->TxSleep.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxAbort",         &(pReg->TxAbort.word),          0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.TxFault",         &(pReg->TxFault.word),          0, 1));
        // ------------------------------------------------------------------------------
        //  END: Matlab Generated Code

        PSTATUS(wiParse_Boolean(text,"Tamale.EnableModelIO",           &(pRX->EnableModelIO)));
        PSTATUS(wiParse_Boolean(text,"Tamale.EnableIQCal",             &(pRX->EnableIQCal)));
        PSTATUS(wiParse_UInt   (text,"Tamale.aPadFront",               &(pTX->aPadFront), 0,2000000));
        PSTATUS(wiParse_UInt   (text,"Tamale.aPadBack",                &(pTX->aPadBack),  0, 400000));
        PSTATUS(wiParse_UInt   (text,"Tamale.bPadFront",               &(pTX->bPadFront), 0,  20000));
        PSTATUS(wiParse_UInt   (text,"Tamale.bPadBack",                &(pTX->bPadBack),  0, 220000));
        PSTATUS(wiParse_Int    (text,"Tamale.RX.InitialPhase",         &(pRX->clock.InitialPhase), -1, 80));
        PSTATUS(wiParse_Int    (text,"Tamale.RX.DownsamPhase",         &(pRX->DownsamPhase),     0,  1));
        PSTATUS(wiParse_Int    (text,"Tamale.RX.DownmixPhase",         &(pRX->DownmixPhase),     0,  3));
        PSTATUS(wiParse_Boolean(text,"Tamale.TX.SetPBCC",              &(pTX->SetPBCC)));

        PSTATUS(wiParse_Int    (text,"Tamale.ADC.Offset",              &(pRX->ADC.Offset), -256, 256));
        PSTATUS(wiParse_Int    (text,"Tamale.ADC.nEff",                &(pRX->ADC.nEff), 2, 10));
        PSTATUS(wiParse_Real   (text,"Tamale.ADC.Vpp",                 &(pRX->ADC.Vpp),  0.5, 3.0));
        PSTATUS(wiParse_Int    (text,"Tamale.DAC.nEff",                &(pTX->DAC.nEff), 2, 10));
        PSTATUS(wiParse_Real   (text,"Tamale.DAC.Vpp",                 &(pTX->DAC.Vpp),  0.5, 3.0));
        PSTATUS(wiParse_Real   (text,"Tamale.DAC.IQdB",                &(pTX->DAC.IQdB),-6.0, 6.0));
        PSTATUS(wiParse_Int    (text,"Tamale.TX.bResamplePhase",       &(pTX->bResamplePhase),  -1,19));
        PSTATUS(wiParse_Int    (text,"Tamale.TX.UpmixPhase",           &(pTX->UpmixPhase), 0, 7));
        PSTATUS(wiParse_Int    (text,"Tamale.TX.TxTonePhase",          &(pTX->TxTonePhase), 0, 159));

		PSTATUS(wiParse_Boolean(text,"Tamale.RX.Nuisance.Enabled",   &(pRX->Nuisance.Enabled)));
		PSTATUS(wiParse_UInt   (text,"Tamale.RX.Nuisance.kNuisance", &(pRX->Nuisance.kNuisance), 0, 1<<20));

        PSTATUS(wiParse_Boolean(text,"Tamale.RX.NonRTL.Enabled",       &(pRX->NonRTL.Enabled)));
        PSTATUS(wiParse_UInt   (text,"Tamale.RX.NonRTL.MaxSamplesCFO", &(pRX->NonRTL.MaxSamplesCFO), 0, 160));
        PSTATUS(wiParse_Boolean(text,"Tamale.RX.NonRTL.DisableCFO",    &(pRX->NonRTL.DisableCFO)));
        PSTATUS(wiParse_Boolean(text,"Tamale.RX.NonRTL.DisablePLL",    &(pRX->NonRTL.DisablePLL)));
        PSTATUS(wiParse_Boolean(text,"Tamale.RX.NonRTL.MojaveFrontCtrlEOP",&(pRX->NonRTL.MojaveFrontCtrlEOP)));

        // legacy versions following block
        PSTATUS(wiParse_UInt   (text,"Tamale.RX.TX.k1",                &(pRX->TX.k1[0]),      0, 1<<20));
        PSTATUS(wiParse_UInt   (text,"Tamale.RX.TX.k2",                &(pRX->TX.k2[0]),      0, 1<<20));
        PSTATUS(wiParse_UInt   (text,"Tamale.RX.TX.kTxDone",           &(pRX->TX.kTxDone[0]), 0, 1<<20));
        PSTATUS(wiParse_UInt   (text,"Tamale.RX.TX.kTxFault",          &(pRX->TX.kTxFault),   0, 1<<20));

        PSTATUS(wiParse_Boolean  (text,"Tamale.RX.TX.Enabled",         &(pRX->TX.Enabled)));
        PSTATUS(wiParse_Boolean  (text,"Tamale.RX.TX.Usek2Delay",      &(pRX->TX.Usek2Delay)));
        PSTATUS(wiParse_UInt     (text,"Tamale.RX.TX.k2Delay",         &(pRX->TX.k2Delay), 0,  8000));
        PSTATUS(wiParse_UIntArray(text,"Tamale.RX.TX.k1",                pRX->TX.k1,      3, 0, 1<<20));
        PSTATUS(wiParse_UIntArray(text,"Tamale.RX.TX.k2",                pRX->TX.k2,      3, 0, 1<<20));
        PSTATUS(wiParse_UIntArray(text,"Tamale.RX.TX.kTxDone",           pRX->TX.kTxDone, 3, 0, 1<<20));

        PSTATUS(wiParse_Boolean(text,"Tamale.TX.NoTxHeaderUpdate",     &(pTX->NoTxHeaderUpdate)                ));
        PSTATUS(wiParse_Boolean(text,"Tamale.TX.UseForcedbSFD",        &(pTX->UseForcedbSFD)                   ));
        PSTATUS(wiParse_Boolean(text,"Tamale.TX.UseForcedbSIGNAL",     &(pTX->UseForcedbSIGNAL)                ));
        PSTATUS(wiParse_Boolean(text,"Tamale.TX.UseForcedbSERVICE",    &(pTX->UseForcedbSERVICE)               ));
        PSTATUS(wiParse_Boolean(text,"Tamale.TX.UseForcedbLENGTH",     &(pTX->UseForcedbLENGTH)                ));
        PSTATUS(wiParse_Boolean(text,"Tamale.TX.UseForcedbCRC",        &(pTX->UseForcedbCRC)                   ));
        PSTATUS(wiParse_Boolean(text,"Tamale.TX.UseForcedSIGNAL",      &(pTX->UseForcedSIGNAL)                 ));
        PSTATUS(wiParse_Boolean(text,"Tamale.TX.UseForcedHTSIG",       &(pTX->UseForcedHTSIG)                  ));
        PSTATUS(wiParse_Boolean(text,"Tamale.TX.UseForcedHTSIG_CRC",   &(pTX->UseForcedHTSIG_CRC)              ));

        PSTATUS(wiParse_UInt   (text,"Tamale.TX.ForcedbSFD",           &(pTX->ForcedbSFD.word),     0,    65535));
        PSTATUS(wiParse_UInt   (text,"Tamale.TX.ForcedbSIGNAL",        &(pTX->ForcedbSIGNAL.word),  0,      255));
        PSTATUS(wiParse_UInt   (text,"Tamale.TX.ForcedbSERVICE",       &(pTX->ForcedbSERVICE.word), 0,      255));
        PSTATUS(wiParse_UInt   (text,"Tamale.TX.ForcedbLENGTH",        &(pTX->ForcedbLENGTH.word),  0,    65535));
        PSTATUS(wiParse_UInt   (text,"Tamale.TX.ForcedbCRC",           &(pTX->ForcedbCRC.word),     0,    65535));
        PSTATUS(wiParse_UInt   (text,"Tamale.TX.ForcedSIGNAL",         &(pTX->ForcedSIGNAL.word),   0, 16777215));
        PSTATUS(wiParse_UInt   (text,"Tamale.TX.ForcedHTSIG1",         &(pTX->ForcedHTSIG1.word),   0, 16777215));
        PSTATUS(wiParse_UInt   (text,"Tamale.TX.ForcedHTSIG2",         &(pTX->ForcedHTSIG2.word),   0,   262143));
        PSTATUS(wiParse_Boolean(text,"Tamale.RX.MAC_NOFILTER",         &(pRX->MAC_NOFILTER)                    ));

        PSTATUS(wiParse_Boolean(text,"Tamale.TX.Checksum.Enable",      &(pTX->Checksum.Enable)) );

        PSTATUS(wiParse_UIntArray(text,"Tamale.TX.Trace.kMarker", pTX->Trace.kMarker, 5, 0, 0x7FFFFFFF));
        PSTATUS(wiParse_UIntArray(text,"Tamale.RX.Trace.kMarker", pRX->Trace.kMarker, 5, 0, 0x7FFFFFFF));
        PSTATUS(wiParse_UIntArray(text,"Tamale.RX.Trace.kStart",  pRX->Trace.kStart,  9, 0, 0x7FFFFFFF));
        PSTATUS(wiParse_UIntArray(text,"Tamale.RX.Trace.kEnd",    pRX->Trace.kEnd,    9, 0, 0x7FFFFFFF));

        PSTATUS(wiParse_Boolean  (text,"Tamale.TX.LgSigAFE",           &(pTX->LgSigAFE)));
        PSTATUS(wiParse_Boolean  (text,"Tamale.TX.FixedTimingShift",   &(pTX->FixedTimingShift)));
        PSTATUS(wiParse_Real     (text,"Tamale.TX.TimingShift",        &(pTX->TimingShift), -1.0, 1.0));

        PSTATUS(wiParse_Boolean  (text,"Tamale.DV.ExpectPacketFail",   &(pDV->ExpectPacketFail)) );
        PSTATUS(wiParse_Int      (text,"Tamale.DV.VerilogChecker",     &(pDV->VerilogChecker),    0, 0xFFFF));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.NumCheckRegisters",  &(pDV->NumCheckRegisters), 0, 512));
        PSTATUS(wiParse_UIntArray(text,"Tamale.DV.CheckRegAddr",         pDV->CheckRegAddr, 512, 0, 1023));
        PSTATUS(wiParse_Boolean  (text,"Tamale.DV.FixedTxTimingLimits",&(pDV->FixedTxTimingLimits)));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.aTxStart",           &(pDV->aTxStart),      0,  800));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.bTxStart",           &(pDV->bTxStart),      0,  800));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.aTxEnd",             &(pDV->aTxEnd),        0,  800));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.bTxEnd",             &(pDV->bTxEnd),        0,  800));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.aTxStartTolerance",  &(pDV->aTxStartTolerance), 0, 80));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.bTxStartTolerance",  &(pDV->bTxStartTolerance), 0, 80));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.aTxEndTolerance",    &(pDV->aTxEndTolerance),   0, 80));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.bTxEndTolerance",    &(pDV->bTxEndTolerance),   0, 80));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.TxStartMinT80",      &(pDV->TxStartMinT80), 0, 8000));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.TxStartMaxT80",      &(pDV->TxStartMaxT80), 0, 8000));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.TxEndMinT80",        &(pDV->TxEndMinT80),   0, 8000));
        PSTATUS(wiParse_UInt     (text,"Tamale.DV.TxEndMaxT80",        &(pDV->TxEndMaxT80),   0, 8000));
        PSTATUS(wiParse_Boolean  (text,"Tamale.DV.LongSimulation",     &(pDV->LongSimulation)));
        PSTATUS(wiParse_Boolean  (text,"Tamale.DV.BreakCheck",         &(pDV->BreakCheck)));
        PSTATUS(wiParse_Int      (text,"Tamale.DV.TestLimit",          &(pDV->TestLimit),-2147483647 ,2147483647));
        PSTATUS(wiParse_Int      (text,"Tamale.DV.DelayT80",           &(pDV->DelayT80), -8000000, 8000000));
        PSTATUS(wiParse_Boolean  (text,"Tamale.DV.NoRegMapUpdateOnTx", &(pDV->NoRegMapUpdateOnTx)));

        PSTATUS(wiParse_Boolean(text,"Tamale.TraceKeyOp",     NULL)); // parameter removed
        PSTATUS(wiParse_Boolean(text,"Tamale.EnableTraceCCK", NULL)); // parameter removed

        // -----------------------
        // Simulation Result Trace
        // -----------------------
        status = WICHK(wiParse_Boolean(text,"Tamale.EnableTrace", &(pRX->EnableTrace)));
        if (status==WI_SUCCESS)
        {
            XSTATUS(Tamale_RX_AllocateMemory());  // make sure trace arrays are allocated
            return WI_SUCCESS;
        }
        ...
        // Array Sizes
        // ----------- 
        status = STATUS(wiParse_UInt(text, "Tamale.aLength", &(pRX->aLength), 1, 16+8*65535+6+287));
        if (status == WI_SUCCESS) {
            XSTATUS(Tamale_RX_AllocateMemory());
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_UInt(text, "Tamale.rLength", &(pRX->rLength), 1, 1000000));
        if (status == WI_SUCCESS) {
            XSTATUS(Tamale_RX_AllocateMemory());
            return WI_SUCCESS;
        }
        ...
        // PLL parameters
        // --------------
        status = STATUS(wiParse_UInt(text,"Tamale.DPLL.Sa_Sb",&(X.word), 0, 63));
        if (status == WI_SUCCESS) {
            pReg->Sa.word = (X.w6 >> 3) & 0x7;
            pReg->Sb.word = (X.w6 >> 0) & 0x7;
            return WI_SUCCESS;
        }
        XSTATUS(status = wiParse_UInt(text,"Tamale.DPLL.Ca_Cb",&(X.word), 0, 63));
        if (status == WI_SUCCESS) {
            pReg->Ca.word = (X.w6 >> 3) & 0x7;
            pReg->Cb.word = (X.w6 >> 0) & 0x7;
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Command(text,"Tamale_RESET()"));
        if (status == WI_SUCCESS) {
            STATUS(Tamale_RESET());
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Tamale_SaveRegisters(%255s)",&buf));
        if (status == WI_SUCCESS) {
            XSTATUS(Tamale_WriteRegisterMap(RegMap));
            XSTATUS(Tamale_SaveRegisters(RegMap, buf));
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Tamale_LoadRegisters(%255s)",&buf));
        if (status == WI_SUCCESS) {
            XSTATUS(Tamale_WriteRegisterMap(RegMap));
            XSTATUS(Tamale_LoadRegisters(RegMap, buf));
            XSTATUS(Tamale_ReadRegisterMap(RegMap));
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Tamale_WriteRegister(%d,%d)",&(X.word), &(Y.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS(Tamale_WriteRegister(X.word, Y.word));
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Tamale_DvCopyTxData(%d)", &(X.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS( Tamale_DvCopyTxData(X.word) );
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Tamale_DvSetTxDataByte(%d, %d, %d)", &dummy, &(X.word), &(Y.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS( Tamale_DvSetTxDataByte((wiUInt)dummy, X.word, Y.word) );
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Tamale_DvCopyTxHeader(%d)", &(X.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS( Tamale_DvCopyTxHeader(X.word) );
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Command(text,"Tamale_WriteDvRegMap()"));
        if (status == WI_SUCCESS)
        {
            Tamale_DvState_t *pDV = Tamale_DvState();
        ...
        status = STATUS(wiParse_Function(text,"Tamale_InvertArrayBit(%255s, %d, %d, %d)", &buf, &(X.word), &(Y.word), &(Z.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS( Tamale_InvertArrayBit(buf, X.word, Y.word, Z.word) );
            return WI_SUCCESS;
        }
        status = STATUS(wiParse_Function(text,"Tamale_SetArrayWord(%255s, %d, %d, %d)", &buf, &(X.word), &(Y.word), &(Z.word)));
        if (status == WI_SUCCESS)
        {
            XSTATUS( Tamale_SetArrayWord(buf, X.word, Y.word, Z.word) );
            return WI_SUCCESS;
        }
        /////////////////////////////////////////////////////////////////////////////////
        //
        ...
        //
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.FixedLNAGain",    &(pReg->InitLNAGain.word),     0, 1));
        PSTATUS(wiParse_UInt   (text,"Tamale.Register.FixedLNA2Gain",   &(pReg->InitLNAGain2.word),    0, 1));
        //
