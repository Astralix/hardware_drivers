//
//  Tamale.h -- 802.11n Single Spatial Stream Handheld Baseband
...
//  ------------------
//  TAMALE_FFT_OFFSET     - delay from peak found to the start of an FFT (RX control)
//  TAMALE_FFT_DELAY      - processing/pipeline delay for the (I)FFT
//  TAMALE_PLL_DELAY      - delay due to the PLL phase corrector
//  TAMALE_SIG_LATENCY    - latency from L-SIG FFT start to the decoded output
//  TAMALE_GF_SIG_LATENCY - latency from the HT-SIG FFT start to decoded output (Greenfield)
//  TAMALE_DPD_DELAY      - signal delay for DPD vs bypass
//
#define TAMALE_FFT_OFFSET      2
#define TAMALE_FFT_DELAY      82
#define TAMALE_PLL_DELAY       0
#define TAMALE_SIG_LATENCY    70
#define TAMALE_HTSIG_LATENCY  76 
#define TAMALE_DPD_DELAY       4

//////////////////////////////////////////////////////////////////////////////////////////////
//
//  ARRAY LENGTHS
//  -------------
//  TAMALE_MAX_N_SYM      - maximum number of OFDM symbols. 2560 --> 10.2 ms which is greater
//                          than any 802.11a packet or 802.11n aPPDUMaxTime (10.0 ms)
//  TAMALE_MAC_DV_TX      - maximum number of MAC_TX_D arrays stored in DvState
//
#define TAMALE_MAX_N_SYM 2560
#define TAMALE_MAX_DV_TX    4 

//////////////////////////////////////////////////////////////////////////////////////////////
//
//  Enumerate Subcarrier Type Labels
//
enum TamaleSubcarrierTypeE
{
    TAMALE_NULL_SUBCARRIER  = 0,
    TAMALE_DATA_SUBCARRIER  = 1,
    TAMALE_PILOT_SUBCARRIER = 2
};
...
typedef struct
{
    ...
    wiWORD CC110Real[64][8], CC110Imag[64][8]; // ...defined in Tamale_b_Init()

} Tamale_LookupTable_t;
...
typedef struct
{
    ...
    int      bResamplePhase;         // phase between 44/40 MHz for Tamale_b_TX_Resample
    ...
    Tamale_traceTxType *traceTx;     // trace for general purpose signals/markers
...
typedef union // ----------------- Registers in Top-Level Control -----------         
{
    ...
    struct
    {
        ...
        unsigned int bPathMux      : 1; // <19> mux control for bTamale input
        unsigned int bRXEnB        : 1; // <20> bTamale TopCtrl enable
...
typedef union // ----------------- RX Exception Event Flags ----------------------         
{
    ...
    struct
    {
        ...
        unsigned int rLimit_SIGNAL   : 1; // <  4> Overrun of array "r" detected in Tamale_RX2_Signal()
        unsigned int xLimit_SIGNAL   : 1; // <  5> Overrun of array "x" detected in Tamale_RX2_Signal()
        unsigned int xLimit_DataDemod: 1; // <  6> Overrun of array "x" detected in Tamale_RX2_Signal()
        unsigned int kLimit_FrontCtrl: 1; // <  7> Overrun of "k44" counter in Tamale_b_RX_Top()
        unsigned int Truncated_N_SYM : 1; // <  8> OFDM "N_SYM" forced to 1 to avoid array overrun in Tamale_RX2_Signal()
    };
} Tamale_RTL_ExceptionFlags_t;
...
typedef struct
{
    ...
    wiBoolean               BreakCheck;       // indicates TamaleDV_CheckScript should not continue to later waveforms
    ...
    wiUWORD   *MAC_TX_D[TAMALE_MAX_DV_TX];     // Pointer to data bytes to be transmitted
    wiUInt     N_PHY_TX_RD[TAMALE_MAX_DV_TX];  // Length of array in pMAC_TX_D (includes PHY header)
...
typedef struct  // ----------------- Tamale_Downmix State --------------------------
{
...
typedef struct  // ----------------- Tamale_DownsamFIR State -----------------------
{
...
typedef struct  // ----------------- Tamale_WeightedPilotPhaseTrack State ----------
{
...
} Tamale_WeightedPilotPhaseTrack_t;

typedef struct  // ----------------- Tamale_PLL State ------------------------------
{
...
typedef struct  // ----------------- Tamale_EqD State ------------------------------
{
...
typedef struct  // ----------------- Tamale_RX2 State ------------------------------
{
...
typedef struct  // ----------------- Tamale_RX_CCA State ---------------------------
{
...
typedef struct  // ----------------- Tamale_RX (Top) State -------------------------
{
    ...
    Tamale_aSigState_t aSigIn, aSigOut;  // signals from OFDM DFE to top level control
    Tamale_bSigState_t bSigIn, bSigOut;  // signals from CCK FrontCtrl to top level control
...
typedef struct
{
    ...
    struct
    {
        ...
        enum
        {
            TAMALE_GAIN_UPDATE_LINEAR      = 1,
            TAMALE_GAIN_UPDATE_THSIGSMALL  = 2,
            TAMALE_GAIN_UPDATE_THSMALLPEAK = 3,
            TAMALE_GAIN_UPDATE_THSIGLARGE  = 4,
            TAMALE_GAIN_UPDATE_LGSIGAFE    = 5
        } GainUpdateType[8];
    ...
    Tamale_RxClockState_t clock;     // clock generator state
    ...
    Tamale_Downmix_t                 Downmix;
    Tamale_DownsamFIR_t              DownsamFIR;
    Tamale_ImageDetector_t           ImageDetector;
    Tamale_IQCalibration_t           IQCalibration;
    Tamale_CFO_t                     CFOState;
    Tamale_FrameSync_t               FrameSync;
    wiReg                            DigitalAmpOut[4];
    Tamale_AGC_t                     AGC;
    Tamale_SignalDetect_t            SignalDetect;
    Tamale_DigitalFrontEnd_t         DigitalFrontEnd;
    Tamale_WeightedPilotPhaseTrack_t WeightedPilotPhaseTrack;
    Tamale_PLL_t                     PLL;
    Tamale_EqD_t                     EqD;
    Tamale_RX2_t                     RX2;
    Tamale_RX_CCA_t                  CCA;
    Tamale_RxTop_t                   RxTop;
    ...
    Tamale_RTL_ExceptionFlags_t RTL_Exception;  // exception event flags used for RTL verification  

    Tamale_aSigState_t         *aSigOut;        // trace "aSigState" register output
    Tamale_bSigState_t         *bSigOut;        // trace "bSigState" register output
    Tamale_traceRxClockType    *traceRxClock;   // trace RX clocks in top-level controller
    Tamale_traceRxStateType    *traceRxState;   // trace RX top state
    Tamale_traceRxControlType  *traceRxControl; // trace top-level RX control
    Tamale_traceDFEType        *traceDFE;       // trace OFDM Digital Front End
    Tamale_traceAGCType        *traceAGC;       // trace signals from OFDM AGC
    Tamale_traceRSSIType       *traceRSSI;      // trace power measurements from OFDM AGC/RSSI
    Tamale_traceDFSType        *traceDFS;       // trace DFS
    Tamale_traceSigDetType     *traceSigDet;    // trace OFDM signal detect
    Tamale_traceFrameSyncType  *traceFrameSync; // trace OFDM frame synchronization
    Tamale_RadioDigitalIO_t    *traceRadioIO;   // trace radio digital interface   
    Tamale_DataConvCtrlState_t *traceDataConv;  // trace data converter control interface
    Tamale_traceRX2Type        *traceRX2;       // trace for OFDM RX2 demodulation/decoding
    Tamale_traceRxType         *traceRx;        // trace for general purpose signals/markers
    Tamale_traceImgDetType     *traceImgDet;    // trace for image detector
    Tamale_traceIQCalType      *traceIQCal;     // trace for IQ calibration engine
...
typedef union // ----------------- Trace CCK Baseband Interface -------------
{
    ...
    struct
    {
        unsigned int bRXEnB     :1; //< 0> input to Tamale_b_RX_top
        unsigned int Run11b     :1; //< 1> input to Tamale_b_RX_top
        unsigned int LgSigAFE   :1; //< 2> input to Tamale_b_RX_top
        unsigned int bRXDone    :1; //< 3> output from Tamale_b_RX_top
        unsigned int AntSel     :1; //< 4> output from Tamale_b_RX_top
        unsigned int __unused5 : 1; //< 5>
        unsigned int CSCCK      :1; //< 6> output from Tamale_b_RX_top
        unsigned int Restart    :1; //< 7> input to Tamale_b_RX_top
        unsigned int RSSICCK    :8; //<8:15> output from Tamale_b_RX_top
        unsigned int LNAGain    :1; //<16> output from Tamale_b_RX_top
        unsigned int AGain      :6; //<17:22> output from Tamale_b_RX_top
        signed   int DGain      :5; //<23:27> output from AGC
        unsigned int LNAGain2   :1; //<28> output from Tamale_b_RX_top
    };
} Tamale_b_traceCCKType;
...
} Tamale_b_traceControlType;

typedef union // ----------------- Trace bTamale RX Interpolator Signals ----
...
typedef struct
{
    ...
    struct TAMALE_RAM_128x32_S { // 128x32, 2-port RAM
...
typedef struct
{
    ...
    Tamale_b_FrontCtrlRegType inReg, outReg; // packed register I/O
...
typedef struct
{
    ...
    wiWORD    dRxFilter[2][16];    // delay line for Tamale_bRX_Filter
    ...
    Tamale_bRX_Interpolate_t     Interpolate; // state for RX interpolator
    Tamale_bBarkerCorrelator_t   BarkerCorrelator;
    Tamale_bEnergyDetect_t       EnergyDetect;
    Tamale_bCorrelationQuality_t CorrelationQuality;
    Tamale_bAGC_t                AGC;
    Tamale_bEstimateCFO_t        EstimateCFO;
    Tamale_bCorrectCFO_t         CorrectCFO;
    Tamale_bChanEst_t            ChanEst;
    Tamale_bRX_SP_t              RX_SP;
    Tamale_bRX_CMF_t             CMF;
    Tamale_bRX_DSSS_t            RX_DSSS;
    Tamale_bRX_CCK_t             RX_CCK;
    Tamale_bRX_SFD_t             RX_SFD;
    Tamale_bRX_Header_t          RX_Header;
    Tamale_bRX_PSDU_t            RX_PSDU;
    Tamale_bRX_DPLL_t            RX_DPLL;
    Tamale_bRX_Descrambler_t     Descrambler;
    Tamale_b_Downsample_t        Downsample;
    Tamale_bRxTop_t              RxTop;

    Tamale_b_traceCCKType      *traceCCK;     // trace signals from CCK interface
    Tamale_b_traceStateType    *traceState;   // trace state from CCK controller
    Tamale_b_traceControlType  *traceControl; // trace signals from CCK controller
    Tamale_b_FrontCtrlRegType  *traceFrontReg;// trace "outReg" from Tamale_b_RX_top
    Tamale_b_traceDPLLType     *traceDPLL;    // trace signals from DPLL
    Tamale_b_traceBarkerType   *traceBarker;  // trace signals from Barker correlator
    Tamale_b_traceDSSSType     *traceDSSS;    // trace signals from DSSS demodulator
    Tamale_b_traceCCKFBFType   *traceCCKInput;// trace CCK input/DFE output
    Tamale_b_traceCCKTVMFType  *traceCCKTVMF; // trace CCK TVMF output
    Tamale_b_traceCCKFWTType   *traceCCKFWT;  // trace CCK FWT/metric output
    Tamale_b_traceResampleType *traceResample;// trace bRX Resample NCOs
...
wiStatus Tamale_SetRegisterMap(wiUInt pRegData[], unsigned int NumRegs);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Transmit functions in Tamale_TX.c
//
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Tamale_TX_DPD.c
//
void     Tamale_TX_DPD(int N, wiWORD xR[], wiWORD xI[], wiWORD *yR, wiWORD *yI);
wiStatus Tamale_TX_DPD_Init(void);
wiStatus Tamale_TX_DPD_Close(void);

///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Tamale_RX.c
//
wiStatus Tamale_RX( wiBoolean DW_PHYEnB, Tamale_RadioDigitalIO_t *Radio, 
                    Tamale_DataConvCtrlState_t *DataConvIn, Tamale_DataConvCtrlState_t DataConvOut, 
                    Tamale_CtrlState_t *inReg, Tamale_CtrlState_t outReg );
                   
wiStatus Tamale_RX_Init(void);
wiStatus Tamale_RX_Restart(void);
wiStatus Tamale_RX_FreeMemory(Tamale_RxState_t *pRX, wiBoolean ClearPointerOnly);
wiStatus Tamale_RX_AllocateMemory(void);
wiStatus Tamale_RX_EndOfWaveform(void);

void Tamale_RX_ClockGenerator(Tamale_RxState_t *pRX, wiBoolean Reset);

///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Tamale_RX1.c
//
...
void Tamale_PLL( wiWORD I1[], wiWORD Q1[], wiWORD H2[], wiWORD DI[], wiWORD DQ[], int iFlag, int HoldVCO, int UpdateH,
                 wiWORD *AdvDly, wiWORD *_cSP, wiWORD *_cCP, Tamale_RxState_t *pRX, TamaleRegisters_t *pReg );

void Tamale_WeightedPilotPhaseTrack( int FInit, int HoldVCO, wiUWORD PathSel, wiWORD v0R[], wiWORD v0I[],
                                     wiWORD v1R[], wiWORD v1I[], wiWORD h0R[], wiWORD h0I[],
                                     wiWORD h1R[], wiWORD h1I[], wiWORD cSP, wiWORD AdvDly, wiWORD *cCPP, 
                                     Tamale_RxState_t *pRX, TamaleRegisters_t *pReg, wiBoolean nMode, int i );

void Tamale_VitDec(int Nbits, wiUWORD VitDetLLR0[], wiUWORD VitDetLLR1[], wiUWORD VitDetOut[], Tamale_RxState_t *pRX);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Tamale_DFS.c
//
void Tamale_CheckFCS(wiUInt nBytes, wiUWORD X[], wiUInt *FCSPass);

void Tamale_DFS( wiBoolean DFSEnB, Tamale_aSigState_t SigIn, Tamale_aSigState_t *SigOut, 
                 Tamale_RxState_t *pRX, TamaleRegisters_t *pReg );

///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Tamale_DigitalFrontEnd.c
//
void Tamale_DigitalFrontEnd( wiBoolean DFEEnB, wiBoolean DFERestart, wiBoolean CheckRIFS, wiBoolean HoldGain,
                             ...
                             wiBoolean RestartRX, Tamale_aSigState_t *aSigOut, Tamale_aSigState_t aSigIn );

/////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Tamale_FFT.c
//
void Tamale_FFT( int TxBRx, wiWORD FFTTxDIR[], wiWORD FFTTxDII[], wiWORD FFTRxDIR[], wiWORD FFTRxDII[],
                 wiWORD *FFTTxDOR, wiWORD *FFTTxDOI, wiWORD *FFTRxDOR, wiWORD *FFTRxDOI );

///////////////////////////////////////////////////////////////////////////////////////////////////
//
// Functions in Tamale_b.c
//
wiStatus Tamale_bRX_AllocateMemory(void);
wiStatus Tamale_bRX_FreeMemory(Tamale_bRxState_t *pRX, wiBoolean ClearPointerOnly);
...
void Tamale_bRX_top( Tamale_RxState_t *pRX, Tamale_bRxState_t *RX, TamaleRegisters_t *pReg, wiBoolean bRXEnB, 
