function data = DwPhyTest_PERvCFO(varargin)
% data = DwPhyTest_PERvCFO(...)
%
...
for i=1:length(fcMHzOfs),
    DwPhyLab_SetRxFreqESG(fcMHz(1)+fcMHzOfs(i));
    ...
    while (data.Npkts(i) < MaxPkts) && (data.ErrCount(i) < MinFail) && ishandle(hBar),
        ...
        if ishandle(hBar)
            waitbar( i/Npts, hBar, { sprintf('%g Mbps, Pr = %1.1f dBm',Mbps,PrdBm),...
                sprintf('fcMHz = %g, PER = %1.2f%%',data.fcMHz(1)+fcMHzOfs(i),100*data.PER(i)) } );
        else
