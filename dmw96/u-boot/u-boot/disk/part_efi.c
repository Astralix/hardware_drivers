...
static int is_pte_env(gpt_entry * pte);
...
static int is_pte_env(gpt_entry * pte)
{
	static const efi_guid_t uboot_env_guid = PARTITION_UBOOT_ENV_GUID;

	return !memcmp(pte->partition_type_guid.b, uboot_env_guid.b, 16);
}
#endif
