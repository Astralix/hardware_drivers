...
again:
	...
	*last = on;
	if (on)
		bootmenu_abort_boot = 1;
	return on;
}

static int dmw_keypad_next_key(void)
{
	return dmw_keypad_scan(menu_next_keypad_row, menu_next_keypad_col);
}

static int dmw_keypad_enter_key(void)
{
	return dmw_keypad_scan(menu_enter_keypad_row, menu_enter_keypad_col);
}
...
static int aux_key(void *user)
{
	...
	return debounce(dmw_keypad_next_key, &last_aux);
}
...
static int on_key(void *user)
{
	...
	return debounce(dmw_keypad_enter_key, &last_on);
}
...
void dmw_bootmenu_hook(int activity)
{
	if (enter_bootmenu == 1) {
