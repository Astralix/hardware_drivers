...
int dfu_active = 0, dfu_entered = 0;
unsigned long dfu_idle_timeout, dfu_start_time;
...
int dfu_enter_dfu_mode(void)
{
	...
	tmp = getenv("dfu_idle_timeout");
	dfu_idle_timeout = tmp ? simple_strtoul(tmp, NULL, 10) : 0;
	dfu_idle_timeout *= 1000; /* convert to ms */
	dfu_entered = 1;
	dfu_start_time = get_timer(0);

	udc_disconnect();
#ifdef CONFIG_USBD_DFU
	device_instance[0].dfu_state = DFU_STATE_dfuIDLE;
#endif
	enable_interrupts();
	udc_connect();
...
int drv_usbtty_init (void)
{
	...
	usbtty_init_terminal_type(0); //strcmp(tt,"cdc_acm"));
...
static void usbtty_init_instances (void)
{
	...
	interface_instance->alternates = 2; //1;
...
{
#ifdef CONFIG_USBD_DFU
	if (device_instance[0].dfu_state != DFU_STATE_appIDLE)
		return 0;
#endif
...
#ifdef CONFIG_USBD_DFU
extern void dfu_poll(void);
#else
#define dfu_poll() do {} while (0)
#endif
...
void usbtty_poll (void)
{
	...
	if (dfu_entered && dfu_idle_timeout && !dfu_active) {
		if (get_timer(dfu_start_time) > dfu_idle_timeout) {
