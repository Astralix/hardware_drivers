				...
				cfiword_t cword)
{
	...
	ret = flash_full_status_check (info, sect, info->write_tout, "write");
				  ...
				  int len)
{
	...
	case CFI_CMDSET_SST: /* SST flashes don't support buffer write */
		retcode = ERR_INVAL;
		break;
...
#ifdef CONFIG_SYS_AMD_PPB_PROTECTION
/* for amd type flash, we use PPB Persistent Protection Bits */
void amd_query_protect (flash_info_t * info)
{
	int i;
	...
	flash_write_cmd (info, 0, info->addr_unlock1, AMD_CMD_PPB);

	for (i = 0; i < info->sector_count; i++)
		/* read the bit, chip has inverse logic */
		info->protect[i] = (flash_read_uchar(info,
			(info->start[i] - info->start[0]) / info->portwidth)) ? 0 : 1;

	/* exit PPB Command set */
	flash_write_cmd (info, 0, 0, AMD_CMD_EXIT);
	flash_write_cmd (info, 0, 0, AMD_CMD_EXIT_ACK);

}

/* Attention: this is a one-time-programmable bit.
 * We set this bit to the PPB mode to enable the PPB feature of this chip.
 * This function need to run only once.
 */
void amd_switch_protect(flash_info_t * info)
{
	static int i = 0;
	...
	flash_write_cmd (info, 0, info->addr_unlock1, AMD_CMD_UNLOCK_START);
	flash_write_cmd (info, 0, info->addr_unlock2, AMD_CMD_UNLOCK_ACK);
	flash_write_cmd (info, 0, info->addr_unlock1, AMD_CMD_LOCK_REG);

	/* read the bit, chip has inverse logic */
	status1 = flash_read_uchar(info, 0);
	if ((status1 & (1<<1)) != 0) {
	        flash_write_cmd (info, 0, 0, 0xa0);
		flash_write_cmd (info, 0, 0, ~(1<<1));
		do {
			/* read status twice */
			status1 = flash_read_uchar(info, 0x0);
			status2	= flash_read_uchar(info, 0x0);
		/* check for DQ6 toggle */
		} while (((status1 & (1<<6)) != (status2 & (1<<6)))
			&& ((status1 & (1<<5)) == 0));
		if ((status1 & (1<<6)) != (status2 & (1<<6)))
			debug("amd_switch_protect togglebit error\n");
		if (status1 & (1<<5))
			debug("amd_switch_protect internal timeout error\n");
	}

	/* exit Lock Register Command set */
	flash_write_cmd (info, 0, 0, AMD_CMD_EXIT);
	flash_write_cmd (info, 0, 0, AMD_CMD_EXIT_ACK);
	i=1;
}
...
int amd_set_page_protect(flash_info_t * info, long sector)
{
	unsigned char status1, status2;
	int retcode;
	...
	/* program PPB bit */
	flash_write_cmd (info, sector, 0, AMD_CMD_WRITE);
	flash_write_cmd (info, sector, 0, 0x00);
	...
	if ((status1 & (1<<5)) == 1) {
		debug("amd_real_protect timeout\n");
		retcode = -1;
	}
	...
	/* check programmed */
	status2 = flash_read_uchar(info, (info->start[sector] - info->start[0]) / info->portwidth);
	if ((status2 & (1<<0)) != 0) {
		printf("Sector not protected, 0x%08lx\n", info->start[sector]);
		retcode = -2;
	}
...
int amd_clear_page_protect(flash_info_t * info, long sector)
{
	int retcode = 0;
	long i;
	unsigned char status1, status2;
	...
	/* argh, we need to erase all protection bits
	 * and reprogram all protected sectors again */
	flash_write_cmd (info, 0, 0, AMD_CMD_ERASE_START);
	flash_write_cmd (info, 0, 0, AMD_CMD_ERASE_SECTOR);
	...
	/* all pages are now erased, we need protect the formerly
	 * protected pages */
	for (i = 0; i < info->sector_count; i++) {
		if (info->protect[i])
			retcode += amd_set_page_protect (info, i);
	}
	return retcode;
}


int amd_real_protect (flash_info_t * info, long sector, int prot)
{
	/* Update hardware info */
	amd_query_protect(info);

	/* ignore already correct protection */
	if (info->protect[sector] == prot)
		return 0;

	if (prot == 1) { /* protected */
		info->protect[sector] = 1;
		return amd_set_page_protect(info, sector);
	} else { /* not protected */
		info->protect[sector] = 0;
		return amd_clear_page_protect(info, sector);
	}
...
int flash_real_protect (flash_info_t * info, long sector, int prot)
{
	...
	if (info->vendor == CFI_CMDSET_AMD_STANDARD) {
	    retcode = amd_real_protect(info, sector, prot);
	} else {
#endif
	switch (info->vendor) {
		case CFI_CMDSET_INTEL_PROG_REGIONS:
		...
		case CFI_CMDSET_SST:
			default:
			/* No support for protection. Simply return 0 (=OK) */
			return retcode;
	};
...
static void cmdset_sst_read_jedec_ids(flash_info_t *info)
{
	flash_write_cmd(info, 0, 0, AMD_CMD_RESET);
	flash_unlock_seq(info, 0);
	flash_write_cmd(info, 0, info->addr_unlock1, FLASH_CMD_READ_ID);
	...
	info->manufacturer_id = flash_read_uchar (info,
					FLASH_OFFSET_MANUFACTURER_ID);
	/* The device ID is 16 bits from address 0x01 */
	info->device_id = flash_read_word (info,
						FLASH_OFFSET_DEVICE_ID);
	flash_write_cmd(info, 0, 0, AMD_CMD_RESET);
}

static int cmdset_sst_init(flash_info_t *info, struct cfi_qry *qry)
{
	info->cmd_reset = AMD_CMD_RESET;

	cmdset_sst_read_jedec_ids(info);
	flash_write_cmd(info, 0, info->cfi_offset, FLASH_CMD_CFI);
...
static int __flash_detect_cfi (flash_info_t * info, struct cfi_qry *qry)
{
	...
	for (cfi_offset=0;
	     ...
	     cfi_offset++) {
		/* Try first to "Unlock" the flash (SST needs it...) */
		info->addr_unlock1 = flash_offset_cfi[cfi_offset];
		info->addr_unlock2 = flash_offset_cfi[cfi_offset] >> 1;
		flash_unlock_seq(info, 0);
...
ulong flash_get_size (phys_addr_t base, int banknum)
{
	...
	if (flash_detect_cfi (info, &qry)) {
		...
		/* SST must get unlock-sequence before entering CFI mode... */
		if (info->vendor == CFI_CMDSET_SST) {
			if (info->portwidth == FLASH_CFI_8BIT) {
				info->addr_unlock1 = 0xaaaa;
				info->addr_unlock2 = 0x5555;
			} else {
				info->addr_unlock1 = 0x5555;
				info->addr_unlock2 = 0x2aaa;
			}
			//printf ("portwidth = 0x%x\n", info->portwidth);
			//printf ("cfi_offset = 0x%x\n", info->cfi_offset);
			//printf ("addr_unlock1 = 0x%x\n", info->addr_unlock1);
		...
		case CFI_CMDSET_SST:
			info->cmd_reset = AMD_CMD_RESET;
			cmdset_sst_init(info, &qry);

			/* SST has 2 difference erable units: Sectors and Blocks
			 * CFI information of SST is not according to the standard, since it declares about
			 * 2 erase regions:
			 * 1) Number of sectors and size of each sector
			 * 2) Number of blocks and size of each block
			 * Of course, this is not according to the standard (on which the size of all
			 * erase regions together should be the size of the device...)
			*/
			num_erase_regions = 1;
			break;
		default:
...
#ifdef CONFIG_SYS_AMD_PPB_PROTECTION
		if (info->vendor == CFI_CMDSET_AMD_STANDARD) {
			/* check and set PPB Mode */
			amd_switch_protect(info);
			/* update protection information from hardware */
			amd_query_protect (info);
		}
#endif
	...
	for (i = 0; i < CONFIG_SYS_MAX_FLASH_BANKS; ++i) {
		...
		if (flash_info[i].flash_id == FLASH_UNKNOWN) {
			...
			if (flash_info[i].legacy_unlock) {
				 ...
				 */
#ifdef CONFIG_SYS_AMD_PPB_PROTECTION
				if (flash_info[i].vendor == CFI_CMDSET_AMD_STANDARD) {
				/* read the status from the chip */
				amd_query_protect(&flash_info[i]);
				} else {
#endif
