...
#define virt_to_bus(x) ((unsigned long)(x) - 0x80000000)
#else
#define virt_to_bus(x) (x)
#endif

#if defined CONFIG_GMAC_DEBUG_NOTICE &&  !defined CONFIG_GMAC_DEBUG_WARNING && !defined CONFIG_GMAC_DEBUG_ERROR
	#define TR2(fmt, args...) printf("%s(%d) " fmt,__func__,__LINE__,##args)
	#define TR1(fmt, args...)
	#define TR0(fmt, args...)
#elif defined CONFIG_GMAC_DEBUG_WARNING && !defined CONFIG_GMAC_DEBUG_ERROR && !defined CONFIG_GMAC_DEBUG_NOTICE
	#define TR2(fmt, args...)
	#define TR1(fmt, args...) printf("%s(%d) " fmt,__func__,__LINE__,##args)
	#define TR0(fmt, args...)
#elif defined CONFIG_GMAC_DEBUG_ERROR && !defined CONFIG_GMAC_DEBUG_WARNING && !defined CONFIG_GMAC_DEBUG_NOTICE
	#define TR2(fmt, args...)
	#define TR1(fmt, args...)
	#define TR0(fmt, args...) printf("%s(%d) " fmt,__func__,__LINE__,##args)
#else
	#define TR2(fmt, args...)
	#define TR1(fmt, args...)
	#define TR0(fmt, args...)
...
#define DEFAULT_PHY_BASE PHY9		//We use First Phy
#define DEFAULT_PHY_UNDEFINE	255
...
#define RECEIVE_DESC_SIZE   64	//Rx Descriptors needed in the Descriptor pool/queue
...
#define RX_BUF_SIZE		1536 //MAX_ETHERNET_PAYLOAD + ETHERNET_HEADER + ETHERNET_CRC
...
typedef struct synopGMACDeviceStruct
{
	...
	//unsigned long TxDescDma;		 /* Dma-able address of first tx descriptor either in ring or chain mode, this is used by the GMAC device*/
	//unsigned long RxDescDma; 	 /* Dma-albe address of first rx descriptor either in ring or chain mode, this is used by the GMAC device*/ 
	dma_desc *TxDesc;               /* start address of TX descriptors ring or chain, this is used by the driver  */
	dma_desc *RxDesc;               /* start address of RX descriptors ring or chain, this is used by the driver  */
	...
	u32  TxBusy;                   /* index of the tx descriptor owned by DMA, is obtained by synop_gmac_get_tx_qptr()                */
	u32  TxNext;                   /* index of the tx descriptor next available with driver, given to DMA by synop_gmac_set_tx_qptr() */
	u32  RxBusy;                   /* index of the rx descriptor owned by DMA, obtained by synop_gmac_get_rx_qptr()                   */
	u32  RxNext;                   /* index of the rx descriptor next available with driver, given to DMA by synop_gmac_set_rx_qptr() */
	
	dma_desc * TxBusyDesc;          /* Tx Descriptor address corresponding to the index TxBusy */
	dma_desc * TxNextDesc;          /* Tx Descriptor address corresponding to the index TxNext */
	dma_desc * RxBusyDesc;          /* Rx Descriptor address corresponding to the index TxBusy */
	dma_desc * RxNextDesc;          /* Rx Descriptor address corresponding to the index RxNext */
...
// Bits on PHY Register 0 (Control) - RW:
#define PHY_Reg0                   0
#define PHY_Reg0_RESET        (1<<15) // WR: '1'-> SW Reset Phy. RD: Always '0'
#define PHY_Reg0_LOOPBACK     (1<<14) // RW: '0'- Disable; '1'- Enable Loopback
#define PHY_Reg0_SPEED        (1<<13) // RW: '0'- 10 Mbps; '1'- 100 Mbps
#define PHY_Reg0_AUTONEG      (1<<12) // RW: '0'- Disable; '1'- Enable Auto-Negotiation Process
#define PHY_Reg0_PWRDOWN      (1<<11) // RW: '0'- Disable; '1'- Enable Power Down mode
#define PHY_Reg0_ISOLATE      (1<<10) // RW: '0'- Disable; '1'- Enable PHY-MAC electrical Isolation
#define PHY_Reg0_AUTONEGRST   (1<< 9) // WR: '1'-> AutoNeg Restart. RD: Always '0'
#define PHY_Reg0_DUPLEX       (1<< 8) // RW: '0'- Half; '1'- Duplex Mode
#define PHY_Reg0_COLTEST      (1<< 7) // RW: '0'- Disable; '1'- Enable Collision Test
// bits 6 - 0 are Reserved, Always Read '0'.

// Bits on PHY Register 1 (LOCAL Side Status) - RO:
#define PHY_Reg1                   1
#define PHY_Reg1_CAPPRESUPP   (1<< 6) // RO: '1'- Capable of MDIO Preamble Suppression
#define PHY_Reg1_AUTONEGDONE  (1<< 5) // RO: '1'- Auto-Negitiation Process Completed
#define PHY_Reg1_RMTFAULT     (1<< 4) // RO: '1'- Remote Fault. Latched High untill read
#define PHY_Reg1_CAPAUTONEG   (1<< 3) // RO: '1'- Capable of Auto-Negotiation Operation
#define PHY_Reg1_LINK_STATUS  (1<< 2) // RO: '1'- Link OK; '0'-Link Down. Latched Low untill read


#define PHY_Reg2                   2 //PHY Identifier 0
#define PHY_Reg3                   3 //PHY Identifier 1

// Bits on PHY Registers 4 (Local  Side Auto-Negotiation Advertisment) - RW,
//                   and 5 (Remote Side Auto-Negotiation Advertisment) - RO.
// These Registers Are Send (4) and Received (5) during the Auto Negotiation Process,
// and Reflects each side Capabilities.
#define PHY_Reg4                    4
#define PHY_Reg5                    5
#define PHY_Reg4_5_PAUSE_OP    (1<<10)
#define PHY_Reg4_5_FD_100M     (1<< 8)
#define PHY_Reg4_5_HD_100M     (1<< 7)
#define PHY_Reg4_5_FD_10M      (1<< 6)
#define PHY_Reg4_5_HD_10M      (1<< 5)

#define PHY_Reg6                    6
#define PHY_Reg6_CapAutoNeg     (1<<0)

/***************************************************************************************/

#define PHY_SPEED_100M         ( PHY_Reg4_5_FD_100M | PHY_Reg4_5_HD_100M )
#define PHY_SPEED_10M          ( PHY_Reg4_5_FD_10M  | PHY_Reg4_5_HD_10M )

#define PHY_DUPLEX_FULL        ( PHY_Reg4_5_FD_100M  | PHY_Reg4_5_FD_10M )
#define PHY_DUPLEX_HALF        ( PHY_Reg4_5_HD_100M  | PHY_Reg4_5_HD_10M )
...
enum InitialRegisters
{
	...
	**********************************************/
	...
	DmaIntDisable           = 0,
	DmaIntRxOnly			= DmaIntErrorMask | DmaIntRxAbnMask | DmaIntRxNormMask | DmaIntRxStoppedMask,
	DmaIntTxOnly			= DmaIntErrorMask | DmaIntTxAbnMask | DmaIntTxNormMask | DmaIntTxStoppedMask
};
 ...
 * Common functions
 **********************************************************/
s32 synop_gmac_reset (synop_gmac_device * gmacdev );
void synop_gmac_rx_disable(synop_gmac_device * gmacdev);
void synop_gmac_tx_disable(synop_gmac_device * gmacdev);
s32 synop_gmac_attach (synop_gmac_device * gmacdev, u32 macBase, u32 dmaBase, u32 phyBase); 
s32 synop_gmac_set_tx_qptr(synop_gmac_device * gmacdev, u32 Buffer1, u32 Length1, u32 Data1, u32 Buffer2, u32 Length2, u32 Data2,u32 offload_needed);
void synop_gmac_resume_dma_tx(synop_gmac_device * gmacdev);
void synop_gmac_disable_dma_tx(synop_gmac_device * gmacdev);
void synop_gmac_disable_dma_rx(synop_gmac_device * gmacdev);
/*******************Ip checksum offloading APIs***************************************/
void synop_gmac_enable_rx_chksum_offload(synop_gmac_device *gmacdev);
