...
static int ep0_find_endpoint(struct usb_device_instance *dev, int epid)
{
	int i;

	for (i = 0; i < dev->bus->max_endpoints; i++) {
		if (dev->bus->endpoint_array[i].endpoint_address == epid)
			return i;
	}
...
int __udc_endpoint_feature(struct usb_device_instance *dev, int ep, int enable)
{
	return -1; /* not implemented */
}

int udc_endpoint_feature(struct usb_device_instance *dev, int ep, int enable)
	__attribute__((weak, alias("__udc_endpoint_feature")));
...
#else
			urb->buffer = (u8 *) device_descriptor;
...
#else
			urb->buffer = (u8 *) configuration_descriptor;
			urb->actual_length =
	...
	case USB_DESCRIPTOR_TYPE_HID:
#ifdef CONFIG_USBD_DFU
		{
			    ...
			    bNumInterface == CONFIG_USBD_DFU_INTERFACE) 
			{
				...
				//urb->buffer = (u8 *) &device->dfu_cfg_desc->func_dfu;
				//urb->actual_length = sizeof(struct usb_dfu_func_descriptor);

			   copy_config (urb, (void *)&device->dfu_cfg_desc->func_dfu, 
						cpu_to_le16(sizeof(struct usb_dfu_func_descriptor)), max);		
			} else
				return -1;
		}
...
int ep0_recv_setup (struct urb *urb)
{
	...
	/* handle the requests that do not return data */
		...
		case USB_REQ_SET_FEATURE:
			...
			case USB_REQ_RECIPIENT_ENDPOINT:
				...
				if (le16_to_cpu (request->wValue) == USB_ENDPOINT_HALT) {
					int endpoint = ep0_find_endpoint(device, le16_to_cpu(request->wIndex));
					if (endpoint >= 0)
						return udc_endpoint_feature(device, endpoint,
		...
		case USB_REQ_GET_INTERFACE:
		case USB_REQ_SYNCH_FRAME:	/* XXX should never see this (?) */
			dbg_ep0(2, "REQUEST: 0x%x", request->bRequest);
			return -1;
		}
	}
	return -1;
