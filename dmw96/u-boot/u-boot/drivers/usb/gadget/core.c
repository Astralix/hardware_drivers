...
struct usb_device_descriptor *usbd_device_device_descriptor (struct usb_device_instance *device, int port)
{
	...
	if (device->dfu_state != DFU_STATE_appIDLE) {
		return (struct usb_device_descriptor *)device->dfu_dev_desc;
	}
#endif
...
#ifdef CONFIG_USBD_DFU
	if (device->dfu_state != DFU_STATE_appIDLE)
		return (struct usb_configuration_descriptor *)(&device->dfu_cfg_desc->ucfg);
#endif
								   ...
								   *device, int port, int configuration, int interface, int alternate)
{
	...
	if (device->dfu_state != DFU_STATE_appIDLE) {
		...
		struct _dfu_desc *dfu_dev;
		struct _dfu_alt_desc *alt;
#endif
		if (alternate < 0 || alternate >= (int)&device->dfu_num_alternates)
			return NULL;

#ifdef CONFIG_USBD_DFUF_SI
		dfu_dev = (struct _dfu_desc *)device->dfu_cfg_desc;
		alt = dfu_dev->alternates + alternate;

		return (struct usb_interface_descriptor *)&alt->uif;
#else //!CONFIG_USBD_DFUF_SI
		return (struct usb_interface_descriptor *)
		       &device->dfu_cfg_desc->uif[alternate];
#endif //CONFIG_USBD_DFUF_SI
	}
#endif
