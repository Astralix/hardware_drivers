...
int do_mem_mtest (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	for (;;) {
		...
		/*
		 * Description: Test the integrity of a physical
		 *		memory device by writing a pseudo-random numbers
		 *		to the entire region, and then read and compare
		 */

		/*
		 * Fill memory with a known pattern using pseudo random generator
		 */
		random32_init(6271);
		for (offset = 0; offset < num_words; offset++) {
			WATCHDOG_RESET();
			start[offset] = random32();
		}
